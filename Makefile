...
libsubdirs: preparedirs
	[ -d ${AUTEWAREFS_DIR}/files/opt/lib/ebtables ] || mkdir ${AUTEWAREFS_DIR}/files/opt/lib/ebtables
...
export IPTABLES_DIR=${IPTABLES_MOD}"/debsrc/iptables-1.4.6/"
iptables: libsubdirs ipset ipp2p
	@echo "Building iptables ..."
	${IPTABLES_MOD}/configpkg
	$(MAKE) -C ${IPTABLES_DIR}
	cp ${IPTABLES_DIR}/.libs/iptables-multi ${AUTEWAREFS_DIR}/files/opt/bin/iptables-multi
	cp ${IPTABLES_DIR}/.libs/ip6tables-multi ${AUTEWAREFS_DIR}/files/opt/bin/ip6tables-multi
	cp ${IPTABLES_DIR}/.libs/libxtables.so.4.0.0 ${AUTEWAREFS_DIR}/files/opt/lib/
#	cp ${IPTABLES_DIR}/iptables-multi ${AUTEWAREFS_DIR}/files/opt/bin/iptables-multi
#	cp ${IPTABLES_DIR}/ip6tables-multi ${AUTEWAREFS_DIR}/files/opt/bin/ip6tables-multi
	...
	cd -)
#	cp ${IPTABLES_DIR}/iptables ${AUTEWAREFS_DIR}/files/opt/bin/
#	cp ${IPTABLES_DIR}/iptables-save ${AUTEWAREFS_DIR}/files/opt/bin/
#	cp ${IPTABLES_DIR}/iptables-restore ${AUTEWAREFS_DIR}/files/opt/bin/
#	cp ${IPTABLES_DIR}/ip6tables ${AUTEWAREFS_DIR}/files/opt/bin/
#	cp ${IPTABLES_DIR}/ip6tables-save ${AUTEWAREFS_DIR}/files/opt/bin/
#	cp ${IPTABLES_DIR}/ip6tables-restore ${AUTEWAREFS_DIR}/files/opt/bin/
#	cp ${IPTABLES_DIR}/extensions/lib*.so ${AUTEWAREFS_DIR}/files/opt/lib/iptables/
	cp ${IPTABLES_DIR}/libiptc/.libs/lib*.so.0.0.0 ${AUTEWAREFS_DIR}/files/opt/lib/
	(cd ${AUTEWAREFS_DIR}/files/opt/lib/;\
	ln -sf libip4tc.so.0.0.0 libip4tc.so;\
	ln -sf libip4tc.so.0.0.0 libip4tc.so.0;\
	ln -sf libip6tc.so.0.0.0 libip6tc.so;\
	ln -sf libip6tc.so.0.0.0 libip6tc.so.0;\
...
ipset: kmod libsubdirs
	@echo "Building ipset ..."
	$(MAKE) -C ${IPTABLES_MOD}/external/ipset-4.5/
...
cleaniptables:
	...
	$(MAKE) -C ${IPTABLES_MOD}/external/ipset-4.5 clean
