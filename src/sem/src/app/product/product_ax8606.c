...
#include "product_ax8606.h"
#include "board/ax81_smu.h"
#include "board/ax81_2x12g12s.h"
#include "board/netlink.h"

extern board_fix_param_t ax81_smu;
...
board_fix_param_t *ax8606_support_board_arr[] =
{
	[BOARD_TYPE_AX81_SMU] = &ax81_smu,
...
int ax_8606_get_product_sys_info(product_sysinfo_t *sys_info)
{
...
int ax8606_get_board_on_mask(unsigned int *mask, int slot_id, int default_slot_id)
{
	int ret;
	int reg_data;
	...
	reg_data = ((reg_data & 0xfc) << 2 | (reg_data & 0x3)) & 0x3f;
	*mask = reg_data ^ AX8606_DEFAULT_BOARD_ON_MASK;
	...
	if (reg_data)
	{
		...
		{
			*mask &= (~(1 << DISTRIBUTE_AX8606_SECOND_MASTER_SLOT));
			*mask |= (1 << DISTRIBUTE_AX8606_FIRST_MASTER_SLOT);
 ...
 */
int ax8606_is_remote_master_on(int *insert_state)
{
	int ret = 0;
	int reg_data = 0;
	ret = sem_read_cpld(CPLD_REMOTE_ON_STATE_REG, &reg_data);
...
int ax8606_force_active_master(void)
{
	int ret;
...
int ax8606_remote_master_state(int *state)
{
	int ret = 0;
	int reg_data = 0;
 ...
 *
 */
int ax8606_board_inserted(int slot_id)
{
 ...
 *
 */
int ax8606_board_removed(int slot_id)
{
...
int ax8606_tipc_init(board_fix_param_t *board)
{
	int zone = 3;
	int cluster = 1;
...
*************************************************************/
int ax8606_sync_send_file_s(int slot_id, char *filename, int syn_to_blk)
{
	int i = 0;
	int itemp_len = 0;
	int send_len = 0;
...
modify:    2011-05-26 <zhangdx@autelan.com>
*************************************************************/
int ax8606_sync_recv_file_s(char *recvbuf, int iflag, int slot_id)
{
	int i = 0;
	int iCount = 0;
	int syn_to_blk = 0;
 ...
 *
 */
int	ax8606_synchronise_software_version(int sd, board_fix_param_t *board)
{
	sem_pdu_head_t *head;
	sem_tlv_t *tlv_head;
	...
	while (1)
	{
		...
		if (timeout<0)
		{
			...
			if (abort_check_product_state_reboot())
			{
				...
				sleep(10);
				ax8606_hardware_reset_slot(mask, 0, RESET_CMD_FROM_SEM);
				break;
			}
			else
			{
...
int ax8606_master_active_or_stby_select(int *sd, board_fix_param_t *board)
{
	int ret;
		...
		if (insert_state)
		{
			...
			else
			{
				...
				if (ret == 0)
				{
					...
					product->active_master_slot_id = DISTRIBUTE_AX8606_SECOND_MASTER_SLOT;
					product->update_product_state();
...
int ax8606_active_stby_switch(void *arg)
{
	int ret;
...
int ax8606_syn_software_version_response(int sd, sem_tlv_t *temp_tlv_head, board_fix_param_t *board, int slot_id)
{
...
int ax8606_product_dbm_file_create(product_info_syn_t *product)
{
	FILE *fd;
	int i;
	char temp_buf[100];
 ...
 *
 */
int ax8606_update_product_state(void)
{
	FILE *fd;
	int i;
 ...
 *
 */
int ax8606_syn_product_info(board_info_syn_t *temp)
{
	int i, j;
	char send_buf[SEM_TIPC_SEND_BUF_LEN];
	sem_pdu_head_t *head;
...
int ax8606_hardware_reset_slot(unsigned int bit_mask, unsigned int flag, unsigned int operater_flag)
{
	int ret;
	int reg_data;
	...
	mask = (((mask & 0x30) >> 2) | (mask & 0x3)) & 0xff;
...
product_fix_param_t dt_ax8606_fix_param =
{
    .product_type = XXX_YYY_AX8606,
    .product_id = PRODUCT_AX8606,
    .name = "AX8606",
    .slotcount = DISTRIBUTE_AX8606_SLOT_NUM,
    .master_slot_count = DISTRIBUTE_AX8606_MASTER_SLOT_NUM,
    .master_slot_id = {DISTRIBUTE_AX8606_FIRST_MASTER_SLOT, DISTRIBUTE_AX8606_SECOND_MASTER_SLOT},
    .default_master_slot_id = DISTRIBUTE_AX8606_FIRST_MASTER_SLOT,
	.more_than_one_master_board_on = 1,
	.active_master_slot_id = -1,
	.fan_num = 4,
	.is_distributed = 1,
	.support_board_arr = ax8606_support_board_arr,
	.board_on_mask = AX8606_DEFAULT_BOARD_ON_MASK,
	.get_slot_id = get_slot_id,
	.is_remote_master_on = ax8606_is_remote_master_on,
	.force_active_master = ax8606_force_active_master,
	.remote_master_state = ax8606_remote_master_state,
	.get_board_on_mask = ax8606_get_board_on_mask,
	.get_product_sys_info = ax_8606_get_product_sys_info,
	.board_inserted = ax8606_board_inserted,
	.board_removed = ax8606_board_removed,
	.tipc_init = ax8606_tipc_init,
	.master_active_or_stby_select = ax8606_master_active_or_stby_select,
	.active_stby_switch = ax8606_active_stby_switch,
	.synchronise_software_version = ax8606_synchronise_software_version,
	.syn_software_version_response = ax8606_syn_software_version_response,
	.product_dbm_file_create = ax8606_product_dbm_file_create,
	.update_product_state = ax8606_update_product_state,
	.syn_product_info = ax8606_syn_product_info,	
	.sync_send_file = ax8606_sync_send_file_s,
	.sync_recv_file = ax8606_sync_recv_file_s,
	.hardware_reset_slot = ax8606_hardware_reset_slot
};
