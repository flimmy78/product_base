...
static int local_board_dbm_file_create(board_fix_param_t *board)
{
	...
	#else
	board->board_ap_max_counter = (__AP_MAX_COUNTER > AX81_AC8C_AP_MAX) ? AX81_AC8C_AP_MAX: __AP_MAX_COUNTER;
	fprintf(fd, "%d\n", board->board_ap_max_counter);
...
/*
 *
 *board ax81_ac8c hardware reset self
 *
 *
 */
 
int ax81_ac8c_hard_reset(void)
{
	int ret;
...
static int set_port_preferr_media(unsigned int port, unsigned int media)
{
	...
	switch(media)
	{
        case COMBO_PHY_MEDIA_PREFER_COPPER:
			 ...
			 */
			sem_oct_mdio_write(port, VSC_VSC8558_LED_MODE_SELECT, 0xaee1);
...
static int ax81_ac8c_show_system_environment(env_state_t *env_state)
{
...
board_fix_param_t ax81_ac8c =
{
	.board_code = AX81_AC8C_BOARD_CODE,
	.board_type = BOARD_TYPE_AX81_AC8C,
	.board_id = BOARD_SOFT_ID_AX81_AC8C,
	.name = "AX81_AC8C",
	.slot_id = UNKONWN_SLOT_ID,
	.is_master = NOT_MASTER,
	.cpu_num = SINGLE_CPU,
	.port_num_on_panel = 8,
    .port_arr = {{CPU, ETH_GTX, 4, 0, 0, AX81_AC8C_ETHPORT_DEFAULT_ATTR}, 
            	 {CPU, ETH_GTX, 5, 1, 1, AX81_AC8C_ETHPORT_DEFAULT_ATTR}, 
            	 {CPU, ETH_GTX, 6, 2, 2, AX81_AC8C_ETHPORT_DEFAULT_ATTR}, 
            	 {CPU, ETH_GTX, 7, 3, 3, AX81_AC8C_ETHPORT_DEFAULT_ATTR}, 
				 {ASIC, -1, -1, -1, -1, -1}, 
            	 {ASIC, -1, -1, -1, -1, -1}, 
            	 {ASIC, -1, -1, -1, -1, -1}, 
            	 {ASIC, -1, -1, -1, -1, -1}}, 				 	
	...
	.hard_self_reset = ax81_ac8c_hard_reset,
	.board_self_init = ax81_ac8c_init,
	...
	.show_system_environment = ax81_ac8c_show_system_environment
};
