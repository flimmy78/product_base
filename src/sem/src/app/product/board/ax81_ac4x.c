...
/*
 *
 *board ax81_4x hardware reset self
 *
 *
 */
 
int ax81_ac4x_hard_reset(void)
{
	int ret;
...
static int ax81_ac4x_show_system_environment(env_state_t *env_state)
{
	...
	if ((get_num_from_file(MASTER_TEMP1_INPUT, &master_temp1_input) >= 0)
		...
		if ((get_num_from_file(SLAVE_TEMP1_INPUT, &slave_temp1_input) >= 0)
			...
			sem_syslog_dbg("ax81_ac4x max1619 temperature read cpu temperature error! \n");			
			return -1;
		}
	...
	/*if use the temperature mointor tmp421*/
		...
		sem_syslog_dbg("ax81_4x read cpu temperature error! \n");
		return -1;	
	}	
...
board_fix_param_t ax81_ac4x =
{
	.board_code = AX81_AC4X_BOARD_CODE,
	.board_type = BOARD_TYPE_AX81_AC4X,
	.board_id = BOARD_SOFT_ID_AX81_AC4X,
	.name = "AX81_AC4X",
	...
	.hard_self_reset = ax81_ac4x_hard_reset,
	.board_self_init = ax81_ac4x_init,
	...
	.show_system_environment = ax81_ac4x_show_system_environment
};
