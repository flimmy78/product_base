...
extern int ax81_1x12g12s_hard_reset(void);
extern int ax81_1x12g12s_init(void);
...
static int local_board_dbm_file_create(board_fix_param_t *board)
{
	...
	#else
	board->board_ap_max_counter = (__AP_MAX_COUNTER > AX71_1X12G12S_AP_MAX) ? AX71_1X12G12S_AP_MAX: __AP_MAX_COUNTER;
	fprintf(fd, "%d\n", board->board_ap_max_counter);
...
int load_fpga(void)
{
	...
	if (0 == retval)
	{
		...
		//printf("FPGA bin file burning success\n");
	} 
	else 
	{
		printf("file burning ioctl fail!\n");
	}
...
int ax71_1x12g12s_init(void)
{
	...
	//fpga_abnormal_tipc = (struct fpga_abnormal_tipc_s *)malloc(sizeof(fpga_abnormal_tipc_t));
	//memset(fpga_abnormal_tipc,0,sizeof(fpga_abnormal_tipc_t));
...
static int ax81_1x12g12s_show_system_environment(env_state_t *env_state)
{
	...
	if ((get_num_from_file(SLAVE_TEMP1_INPUT, &master_temp1_input) >= 0)
		...
		sem_syslog_dbg("ax71_1x12g12s read cpu temperature error! \n");
	} 
	...
	if ((get_num_from_file(TMP421_SLAVE_TEMP1_INPUT, &master_temp1_input) >= 0)
		...
		sem_syslog_dbg("ax71_1x12g12s read fpga  temperature error! \n");
	}

	return 0;
}

board_fix_param_t ax71_1x12g12s =
{
	.board_code = AX71_1X12G12S_BOARD_CODE,
	.board_type = BOARD_TYPE_AX71_1X12G12S,
	.board_id = BOARD_SOFT_ID_AX7605I_1X12G12S,
	.name = "AX71_1X12G12S",
	...
	.port_arr = {{ASIC,-1,-1,-1,-1,-1}, {ASIC,-1,-1,-1,-1,-1}, {ASIC,-1,-1,-1,-1,-1},\
				 ...
				 {ASIC,-1,-1,-1,-1,-1}, {ASIC,-1,-1,-1,-1,-1}, {ASIC,-1,-1,-1,-1,-1}, {ASIC,-1,-1,-1,-1,-1},\
	             {ASIC,-1,-1,-1,-1,-1}, {ASIC,-1,-1,-1,-1,-1}, {ASIC,-1,-1,-1,-1,-1}, {ASIC,-1,-1,-1,-1,-1},\
                 {ASIC,-1,-1,-1,-1,-1}, {CPU,ETH_XGE_FIBER,-1,-1,-1,AX71_1X12G12S_ETHPORT_DEFAULT_ATTR}},
