...
board_fix_param_t *ax8800_support_board_arr[] =
{
...
int ax_8800_get_product_sys_info(product_sysinfo_t *sys_info)
{
 ...
 */
int ax8800_is_remote_master_on(int *insert_state)
{
	int ret = 0;
	int reg_data = 0;
	ret = sem_read_cpld(CPLD_REMOTE_ON_STATE_REG, &reg_data);
...
int ax8800_force_active_master(void)
{
	int ret;
...
int ax8800_remote_master_state(int *state)
{
	int ret = 0;
	int reg_data = 0;
 ...
 *
 */
int ax8800_board_inserted(int slot_id)
{
 ...
 *
 */
int ax8800_board_removed(int slot_id)
{
...
int ax8800_tipc_init(board_fix_param_t *board)
{
	int zone = 3;
	int cluster = 1;
...
*************************************************************/
int ax8800_sync_send_file_s(int slot_id, char *filename, int syn_to_blk)
{
	int i = 0;
	int itemp_len = 0;
	int send_len = 0;
...
modify:    2011-05-26 <zhangdx@autelan.com>
*************************************************************/
int ax8800_sync_recv_file_s(char *recvbuf, int iflag, int slot_id)
{
	int i = 0;
	int iCount = 0;
	int syn_to_blk = 0;
 ...
 *
 */
int	ax8800_synchronise_software_version(int sd, board_fix_param_t *board)
{
	sem_pdu_head_t *head;
	sem_tlv_t *tlv_head;
	...
	while (1)
	{
		...
		if (timeout<0)
		{
			...
			if (abort_check_product_state_reboot())
			{
				...
				sleep(10);
				ax8800_hardware_reset_slot(mask, 0, RESET_CMD_FROM_SEM);
				break;
			}
			else
			{
...
int ax8800_master_active_or_stby_select(int *sd, board_fix_param_t *board)
{
	int ret;
		...
		if (insert_state)
		{
			...
			else
			{
				...
				if (ret == 0)
				{
					...
					product->active_master_slot_id = DISTRIBUTE_AX8800_SECOND_MASTER_SLOT;
					product->update_product_state();
...
/*
  ...
  *
  */
int ax8800_active_stby_switch(void *arg)
{
	int ret;
...
static int get_remote_software_version(sem_tlv_t *tlv_head, char *version, char *buildno,int *space_flag)
{
	...
	if (tlv_head->type == SEM_SOFTWARE_VERSION_SYN_REQUEST)
	{
		...
		*space_flag = head->space_flag;
		sem_syslog_dbg("remote version is %s , buildno is %s ,CF space flag is %d\n", version, buildno,*space_flag);
		return 0;
	}
	else
	{
...
int ax8800_syn_software_version_response(int sd, sem_tlv_t *temp_tlv_head, board_fix_param_t *board, int slot_id)
{
...
int ax8800_product_dbm_file_create(product_info_syn_t *product)
{
	FILE *fd;
	int i;
	char temp_buf[100];
 ...
 *
 */
int update_product_ax8800_state(void)
{
	FILE *fd;
	int i;
 ...
 *
 */
int ax8800_syn_product_info(board_info_syn_t *temp)
{
	int i, j;
	char send_buf[SEM_TIPC_SEND_BUF_LEN];
	sem_pdu_head_t *head;
...
int ax8800_hardware_reset_slot(unsigned int bit_mask, unsigned int flag, unsigned int operater_flag)
{
	int ret;
	int reg_data;
...
product_fix_param_t dt_ax8800_fix_param =
{
    .product_type = XXX_YYY_AX8800,
    .product_id = PRODUCT_AX8800,
    .name = "AX8800",
    .slotcount = DISTRIBUTE_AX8800_SLOT_NUM,
    .master_slot_count = DISTRIBUTE_AX8800_MASTER_SLOT_NUM,
    .master_slot_id = {DISTRIBUTE_AX8800_FIRST_MASTER_SLOT, DISTRIBUTE_AX8800_SECOND_MASTER_SLOT},
    .default_master_slot_id = DISTRIBUTE_AX8800_FIRST_MASTER_SLOT,
	.more_than_one_master_board_on = 1,
	.active_master_slot_id = -1,
	.fan_num = 8,
	.is_distributed = 1,
	.support_board_arr = ax8800_support_board_arr,
	.board_on_mask = AX8800_DEFAULT_BOARD_ON_MASK,
	.get_slot_id = get_slot_id,
	.is_remote_master_on = ax8800_is_remote_master_on,
	.force_active_master = ax8800_force_active_master,
	.remote_master_state = ax8800_remote_master_state,
	.get_board_on_mask = ax8800_get_board_on_mask,
	.get_product_sys_info = ax_8800_get_product_sys_info,
	.board_inserted = ax8800_board_inserted,
	.board_removed = ax8800_board_removed,
	.tipc_init = ax8800_tipc_init,
	.master_active_or_stby_select = ax8800_master_active_or_stby_select,
	.active_stby_switch = ax8800_active_stby_switch,
	.synchronise_software_version = ax8800_synchronise_software_version,
	.syn_software_version_response = ax8800_syn_software_version_response,
	.product_dbm_file_create = ax8800_product_dbm_file_create,
	.update_product_state = update_product_ax8800_state,
	.syn_product_info = ax8800_syn_product_info,	
	.sync_send_file = ax8800_sync_send_file_s,
	.sync_recv_file = ax8800_sync_recv_file_s,
	.hardware_reset_slot = ax8800_hardware_reset_slot
};
