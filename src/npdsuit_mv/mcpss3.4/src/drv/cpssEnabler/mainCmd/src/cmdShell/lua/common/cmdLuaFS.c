...
static int l_cmdFSread(lua_State *L)
{
    ...
    while (len > 0)
    {
        ret = cmdFSread(fd, buf, len < sizeof(buf) ? len : sizeof(buf));
        if (ret < 0)
        {
            if (lua_objlen(L, -1) > 0)
            {
...
static int l_cmdFSwrite(lua_State *L)
{
    ...
    fd = lua_tointeger(L, 1);
    ret = cmdFSwrite(fd, lua_tostring(L, 2), lua_objlen(L, 2));
    if (ret < 0)
    {
        lua_pop(L, 1);
        lua_pushnil(L);
...
static int l_cmdFSclosedir(lua_State *L)
{
    if (lua_isnumber(L, 1))
    {
        cmdFSclosedir(lua_tointeger(L, 1));
    }
    return 0;
}
