...
static int nullStreamDestroy(cmdStreamPTR stream)
{
    return 0;
}
static int nullStreamRead(cmdStreamPTR stream, void* bufferPtr, int bufferLength)
{
    return 0;
}
static int nullStreamReadChar(cmdStreamPTR stream, char* charPtr, int timeOut)
{
    return -1;
}
static int nullStreamReadLine(cmdStreamPTR stream, char* bufferPtr, int bufferLength)
{
    return 0;
}
static int nullStreamWrite(cmdStreamPTR stream, const void* bufferPtr, int bufferLength)
{
    return bufferLength;
}
static int nullStreamWriteBuf(struct cmdStreamSTC* stream, const char* s, int len)
{
    return len;
}
static int nullStreamWriteLine(cmdStreamPTR stream, const char *s)
{
    return cmdOsStrlen(s);
}
static int nullStreamConnected(cmdStreamPTR stream)
{
    return 0;
}
static int nullStreamSetTtyMode(cmdStreamPTR stream, int mode)
{
    return 0;
}
...
int streamGenericWriteBuf(cmdStreamPTR streamP, const char *s, int len)
{
    ...
    for (p = s; p < e; p++)
    {
        if (*p == '\n')
        {
            if (p != s)
            {
                ret = streamP->write(streamP, (char*)s, p-s);
                if (ret < 0)
