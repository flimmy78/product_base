...
static GT_32 inputBuffer[CMD_MAX_ARGS];

/* commands fields input argument buffer */
static GT_32 fieldBuffer[CMD_MAX_FIELDS];
...
/* for private versions (for ex. automatic cpssInitSystem before shell) */
    ...
    while (GT_TRUE)
    {
        ...
        else
        {
            ...
            cmdOsPrintf("\nCalled function\n");
    #endif
        gRc = wrapperRef(inputBuffer, fieldBuffer, numFields, outputBuffer);
        }
        ...
        ((status = cmdGetCommand(commandName, &command)) == GT_OK))
    {
        ...
        for (i = 0; i < args; i++)
        {
            ...
            switch (token)
            {
                ...
                case tokDECIMAL:
                case tokHEXADECIMAL:
                    if(cmdParseGetNumeric((GT_U32*)(inputBuffer + i)) != GT_OK)
                    {
                        cmdOsSigSemSignal(gRunMtx);
        ...
        {
            if(cmdReadFields(inFieldBuffer,(GT_U32*)fieldBuffer,&numFields)
               != GT_OK)
            {
                cmdOsSigSemSignal(gRunMtx);
                return CMD_SYNTAX_ERROR;
...
static int cmdLuaValueToGT32(lua_State *L, int i, GT_32 *ret)
{
    switch (lua_type(L, i))
    {
        case LUA_TNIL:
