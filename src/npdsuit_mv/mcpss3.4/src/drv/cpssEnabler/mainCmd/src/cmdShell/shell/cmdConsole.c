...
cmdStreamPTR defaultStdio = NULL;

/*Holds the current stdio stream*/
static cmdStreamPTR currentStream;
...
/***** Externs *********************************************************/
#ifdef  CMD_LUA_CLI
GT_STATUS luaCLI_EventLoop(cmdStreamPTR IOStream);
#endif
...
*
*******************************************************************************/
static GT_STATUS cmdTakeGlobalStdio(cmdStreamPTR IOStream)
{
...
*
*******************************************************************************/
static GT_STATUS cmdReleaseGlobalStdio(GT_VOID)
{
    GT_STATUS rc; /*return code*/
...
*
* DESCRIPTION:
*       command line shell used for pipe console connection
*       uses prompt with end of line symbol "->\n" 
*
* INPUTS:
*       stream  - pointer to stream descriptor
*
...
#define CMD_PIPE_PROMPT  "->\n"

GT_STATUS cmdPipeEventLoop
(
    cmdStreamPTR IOStream
)
{
    GT_CHAR inputBuffer[4 * CMD_MAX_ARGS];
    GT_CHAR fieldBuffer[4 * CMD_MAX_FIELDS];
    ...
    if (IOStream->isConsole)
        IOStream->writeLine(IOStream, CMD_PIPE_PROMPT);
    ...
    while (IOStream->connected(IOStream) && (notExitFlag))
    {
        ...
        if (status == CMD_OK)
        {
            ...
            if ((outputBuffer != NULL) && (cmdOsStrlen(outputBuffer) != 0))
            {
                ...
                /* Append the prompt to the last message*/
                if (IOStream->isConsole)
                    cmdOsStrCat(outputBuffer, CMD_PIPE_PROMPT);
                /* send output buffer generated by api call */
                IOStream->writeLine(IOStream, outputBuffer);
                IOStream->writeLine(IOStream, "\n\n");
            }
        ...
        else
        {
            ...
            /* Append the prompt to the last messgae*/
            if (IOStream->isConsole)
                cmdOsStrCat(galtisError, CMD_PIPE_PROMPT);
            IOStream->writeLine(IOStream, galtisError);
        }
        cmdReleaseGlobalStdio();
    }
