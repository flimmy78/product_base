...
static GT_STATUS cmdDxChRxPktReceive
(
    IN GT_U8                                devNum,
    IN GT_U8                                queueIdx,
    IN GT_U32                               numOfBuff,
...
)
{
	...
	do
	{
		...
		rc = rxTxDxChNetRxPktHandle(devNum, queue);
...
*/
	...
	if((rxTxParams.doCapture == GT_TRUE && 
		rxTxParams.wasInitDone == GT_TRUE) != GT_TRUE)
	{
		return GT_OK;
	}

	tmpRxParamPtr = osMalloc(sizeof(CPSS_DXCH_NET_RX_PARAMS_STC));
	if(tmpRxParamPtr == NULL)
	{
		return GT_OUT_OF_CPU_MEM;
	}

	osMemCpy(tmpRxParamPtr,
			 rxParamsPtr,
			 sizeof(CPSS_DXCH_NET_RX_PARAMS_STC));


	rc = cmdCpssRxPktReceive(devNum,
							 queueIdx,
							 tmpRxParamPtr,
							 rxTxParams.freeRxInfoFunc,
							 rxTxParams.copyRxInfoFunc,
							 numOfBuff,
							 packetBuffs,
							 buffLen);
...
)
{
	   ...
	   || (rxTableMode == GALTIS_RX_CYCLIC_E))
	{
		...
		{
			rxInfoFree((void *)indexToInsertPacket);
		}
