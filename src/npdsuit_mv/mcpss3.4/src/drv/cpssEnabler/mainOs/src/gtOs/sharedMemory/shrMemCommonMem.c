...
)
{
	...
	printf("%s\n",p);
    printf("*********** shrMemMmapMemory() test OK *********\n");		
...
)
{
    ...
    if((unsigned long)pmemPtr == base) /* Want to place info struct into block? */
    {
    ...
    pmemPtr->base = (GT_U32)base;
    pmemPtr->name = namePtr;
    pmemPtr->curr = pmemPtr->base;
    if((unsigned long)pmemPtr == base)
