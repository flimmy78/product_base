   ...
   taskDelay( long interval )
{
    ...
    ticks2timespec(interval, &ts);
    return taskDelay_timespec(&ts);
}

#endif


/*luoxun --many different, need more attention.*/
...
*****************************************************************************/
pthread_t 
   taskId2ThreadId(long taskid)
{
    //v2pthread_cb_t *tcb = NULL;
	pthread_t *threadId = NULL;

    if (1)
        threadId = getpid();
    else
        threadId = (pthread_t *)NULL;
