...
static GT_MUTEX        poolMtx;

/* Array of created pools */
static GT_POOL_HEAD *poolArray;
static GT_U32        freePoolHeaderIdx; /* Index of the next free pool header.*/

/* Max number of pools supported by this module */
static GT_U32        poolMaxPools;
...
)
{
    ...
    pBytePtr = (GT_U8*)((GT_U32)(pBytePtr + alignment) & (~alignment));
...
)
{
    ...
    while (pBlockNode != NULL)
    {
        ...
        if ( ( (GT_U8*)pBuf >= pBlocStart ) &&           /* in the pool boundaries */
             ...
             ( ((GT_U32)pBuf & pHead->poolAlignment) == 0 )/* ...and aligned  */
           )
            break;
...
)
{
    ...
    while (pBlockNode != NULL)
    {
        ...
        osPrintf("\nPool's ID          : 0x%08X\n", poolId);
        osPrintf("Pool's Boundaries  : 0x%08X - 0x%08X (%d Bytes)\n",
                 (GT_U32)(pBlockNode->pPoolBlock),
                 (GT_U32)(pBlockNode->pPoolBlock) + pHead->memBlocSize - 1,
                 pHead->memBlocSize);

        osPrintf("Aligned Boundaries : 0x%08X - 0x%08X (%d Bytes)\n",
                 (GT_U32)(pBlockNode->pAlignedBlockStart),
                 (GT_U32)(pBlockNode->pAlignedBlockEnd),
