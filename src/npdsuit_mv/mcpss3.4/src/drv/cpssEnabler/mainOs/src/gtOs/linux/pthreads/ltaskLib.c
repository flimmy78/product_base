...
static OS_OBJECT_LIST_STC tasks = {
    NULL,                   /* list */
    0,                      /* allocated */
...
)
{
    ...
    t = osObjLibGetNewObject(&tasks, 1, name?name:"tUsrRoot", &h);
    tsk = (_V2L_taskSTC*)h;
...
)
{
    ...
    if ( pthread_create( &(tsk->pthrid), &attrs,
                         (GT_VOID*(*)(GT_VOID *))start_addr, arglist) != 0 )
    {
        ...
        return GT_FAIL;
            ...
            int ret = 0;
            ret = strcmp(name, "NpdDBUS");
            printf("%s %s %d name=%s\n", __FILE__, __FUNCTION__, __LINE__, name);
            printf("%s %s %d strcmp(name,NpdDBUS) ret=%d\n", __FILE__, __FUNCTION__, __LINE__, ret);
            if(ret){
                printf("%s %s %d pthread_detach()\n", __FILE__, __FUNCTION__, __LINE__);
    ...
    #ifndef __AX_PLATFORM__
        if (tid != NULL)
            *tid = (GT_TASK)tsk->pthrid;
        pthread_join(tsk->pthrid, NULL);
        printf("%s %s %d ret pthrid %#x\n", __FILE__, __FUNCTION__, __LINE__, tsk->pthrid);
        printf("%s %s %d ret tid %#x\n", __FILE__, __FUNCTION__, __LINE__, *tid);
    #endif
...
/*****************************************************************************
** taskId2ThreadId - get thread id by task id
*****************************************************************************/
pthread_t 
   taskId2ThreadId(int t)
{
    if (t<tasks.allocated && TASK(t) && TASK(t)->header.type)
        return (TASK(t)->pthrid);
    return 0;
}
...
*
*******************************************************************************/
GT_STATUS osTaskGetThreadId
(
    IN GT_U32 tid,
	OUT pthread_t *pthrid
)
{
    *pthrid = taskId2ThreadId(tid);
    return GT_OK;
}
#endif
