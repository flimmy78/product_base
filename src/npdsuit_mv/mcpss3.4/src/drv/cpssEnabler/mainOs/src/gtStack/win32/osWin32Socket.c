...
*
*******************************************************************************/
GT_STATUS osSocketCreateAddr( IN  GT_CHAR * ipAddr,
                            ...
                            IN  GT_SIZE *        sockAddrLen)
{
    ...
    if (sockAddr == NULL)
            newSocket = accept ((SOCKET) socketFd,0,0);
    else
...
*
*******************************************************************************/
GT_SIZE osSocketSend(IN  GT_SOCKET_FD   socketFd ,
                     IN  GT_VOID *      buff,
                     IN  GT_SIZE        buffLen)
{
    int ret;
    ...
    sizeSent = send ((SOCKET)socketFd, buff, buffLen, 0);
...
*
*******************************************************************************/
GT_SIZE osSocketSendTo( IN  GT_SOCKET_FD   socketFd ,
                        IN  GT_VOID *      buff,
                        IN  GT_SIZE        buffLen,
                        IN  GT_VOID *      sockAddr,
                        IN  GT_SIZE        sockAddrLen)
{
    ...
    sizeSent = sendto ((SOCKET)socketFd, buff, buffLen, 0, (struct sockaddr*)sockAddr,
                                           (int)sockAddrLen);
    if (sizeSent == SOCKET_ERROR)
        ret = WSAGetLastError ();
...
*
*******************************************************************************/
GT_SIZE osSocketRecv(IN  GT_SOCKET_FD   socketFd,
                     IN  GT_VOID *      buff,
                     IN  GT_SIZE        buffLen,
                     IN  GT_BOOL        removeFlag)
{
    ...
    recvSize = recv ((SOCKET)socketFd, buff, buffLen, flags);
...
*
*******************************************************************************/
GT_SIZE osSocketRecvFrom( IN  GT_SOCKET_FD   socketFd ,
                          IN  GT_VOID *      buff,
                          IN  GT_SIZE        buffLen,
                          IN  GT_BOOL        removeFlag,
                          ...
                          OUT GT_SIZE *      sockAddrLen)
{
    ...
    recvSize = recvfrom ((SOCKET)socketFd, buff, buffLen, flags,
...
)
{
    ...
    *ipAddr = ((struct sockaddr_in*)(sockAddr))->sin_addr.s_addr;
