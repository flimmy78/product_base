...
GT_SOCKET_FD osSocketTcpCreate (GT_32 sockSize)
{
    ...
    {   
        ...
        if (setsockopt (sock, SOL_SOCKET, SO_SNDBUF, (GT_CHAR*)&optval,
                        sizeof (optval)) != 0)
        {
			close((int)sock);    /* code optmize: Resource leak - not close sock */			
            return GT_ERROR;
        }
...
GT_SOCKET_FD osSocketUdpCreate (GT_32 sockSize)
{
    ...
    {   
        ...
        if (setsockopt (sock, SOL_SOCKET, SO_SNDBUF, (GT_CHAR*)&optval,
                        sizeof (optval)) != 0)
        {
			close((int)sock);    /* code optmize: Resource leak - not close sock. zhangdi@autelan.com 2013-01-17 */
            return GT_ERROR;
        }
        if (setsockopt (sock, SOL_SOCKET, SO_RCVBUF, (GT_CHAR*)&optval,
                        sizeof (optval)) != 0)
...
)
{
    ...
    if (sock < 0)  /* code optmize: Argument cannot be negative. zhangdi@autelan.com 2013-01-17 */
    {
        return GT_ERROR;
    }    
    if(setsockopt(sock, IPPROTO_TCP, TCP_NODELAY,  (GT_CHAR*) &optval, sizeof(optval))!=0)
	{
		return GT_ERROR;   /* code optmize: No check of the return value. zhangdi@autelan.com 2013-01-17 */
	}
    return sock ;
}
...
{
    return (GT_SIZE) send((int)socketFd,        /* Socket FD                 */
                          buff,                 /* Buffer to send            */
...
{
    return (GT_SIZE) sendto((int)socketFd,             /* Socket FD                */
...
)
{
    ...
    return (GT_SIZE) recv((int)socketFd,        /* Socket FD                */
                          buff,                 /* Buffer to receive        */
                          (size_t)buffLen,      /* Max length to received   */
                          flags);               /* Flags                    */
}
...
)
{
    ...
    return (GT_SIZE) recvfrom((int)socketFd,             /* Socket FD                */
                              buff,                      /* Buffer to receive        */
                              (size_t)buffLen,           /* Max length to received   */
                              flags,                     /* Flags                    */
                              (struct sockaddr*)sockAddr,/* Socket address recv from */
