...
# if defined(_syscall0)
    _syscall0(int, mv_noop)
    _syscall2(int, mv_ctl, unsigned int, cmd, unsigned long, arg)
#   define mv_ctrl(cmd,arg)  mv_ctl((cmd),(unsigned long)(arg))
    _syscall0(int, mv_tasklock)
    _syscall0(int, mv_taskunlock)
    _syscall0(int, mv_taskunlockforce)
    _syscall1(int, mv_sem_give, int, semid)
    _syscall1(int, mv_sem_take, int, semid)
    _syscall1(int, mv_sem_trytake, int, semid)
    _syscall2(int, mv_sem_take_tmo, int, semid, unsigned int, timeout)
# endif
#endif
