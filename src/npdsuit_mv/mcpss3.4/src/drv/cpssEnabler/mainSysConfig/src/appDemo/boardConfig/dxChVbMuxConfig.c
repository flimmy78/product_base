...
)
{
    ...
    portsTaggingBmp.ports[0] = 0;
    portsTaggingBmp.ports[1] = 0;
    ...
    for (ii = 0; (ii < networkPortsNum); ii++)
    {
        ...
        /* PVID */
        /* Set default vid = 0 */
        RC_CHECK(cpssDxChBrgVlanPortVidSet(devNum,port,0))
    ...
    for (ii = 0; (ii < cascadePortsNum); ii++)
    {
        ...
        if(appDemoPpConfigList[ppIndex].devFamily == CPSS_PP_FAMILY_DXCH_LION_E)
        {
            ...
            RC_CHECK(cpssDxChCscdPortTypeSet(devNum,port,CPSS_CSCD_PORT_DSA_MODE_EXTEND_E))
        }
        else
        {
            RC_CHECK(cpssDxChCscdPortTypeSet(devNum,port,CPSS_CSCD_PORT_DSA_MODE_REGULAR_E))
        }
    }
    ...
    RC_CHECK(cpssDxChCscdPortTypeSet(devNum,CPSS_CPU_PORT_NUM_CNS,CPSS_CSCD_PORT_DSA_MODE_REGULAR_E))
    ...
    for(ii = 0 ; ii < 31 ; ii++)
    {
        ...
        RC_CHECK(cpssDxChCscdDevMapTableSet(devNum,(GT_U8)ii, 0,
                        &nullPortCascadeLink,
                        CPSS_DXCH_CSCD_TRUNK_LINK_HASH_IS_INGR_PIPE_HASH_E))/*don't care for port*/
    }
