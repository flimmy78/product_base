...
/* Add for the define of BOARD_TYPE, zhangdi 2011-04-07,modify 2011-05-13 */
    ...
    if (boardRevId == 8)  /* native MII*/
    {
        ...
        for(i = 0; i < PRV_CPSS_MAX_PP_DEVICES_CNS; i++)
        {
            if(appDemoPpConfigList[i].valid == GT_TRUE)
            {
                ...
                {
                    appDemoPpConfigList[i].cpuPortMode = PRV_CPSS_GEN_NETIF_CPU_PORT_MODE_NONE_E;
                }
                else
                {
                    appDemoPpConfigList[i].cpuPortMode = PRV_CPSS_GEN_NETIF_CPU_PORT_MODE_MII_E;
                }
            }
        }
    }
...
)
{
    ...
    {
        /* make the fist paramet to devnum for get good pci_bus_no, zhangdi@autelan.com 2012-01-08 */
        #if 0
        extDrvGetPciIntVec(GT_PCI_INT_D,&intVecNum);
        localPpPh1Config.intVecNum = (GT_U32)intVecNum;
        extDrvGetIntMask(GT_PCI_INT_D,&(localPpPh1Config.intMask));
        #else
        extDrvGetPciIntVec(appDemoPpConfigList[devIdx].deviceId,&intVecNum);
        localPpPh1Config.intVecNum = (GT_U32)intVecNum;
        extDrvGetIntMask(appDemoPpConfigList[devIdx].deviceId,&(localPpPh1Config.intMask));
		#endif
    ...
    if(boardRevId == 4)
    {
        ...
        if (localPpPh1Config.deviceId == 0xE5A711AB)
        {
            rc = appDemoDebugDeviceIdSet(devIdx, CPSS_98DX1035_CNS);
        }
...
)
{
    ...
    for(ii = SYSTEM_DEV_NUM_MAC(0); ii < SYSTEM_DEV_NUM_MAC(ppCounter); ++ii)
    {
        ...
        if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_XCAT2_E)
        {
            ...
            rc = prvCpssDrvHwPpGetRegField(devNum,0x0000002C,14,1,&cpuDisabled);
            if (rc != GT_OK)
            {
                return rc;
            }
    ...
    for(ii = SYSTEM_DEV_NUM_MAC(0); ii < SYSTEM_DEV_NUM_MAC(ppCounter); ++ii)
    {
        ...
        {
            /* XCAT2 not need the WA , regardless to boardRevId */
            ignoreTrunkWa = 1;
...
)
{
    ...
    CPSS_ENABLER_DBG_TRACE_RC_MAC("before waInit", 0);
    printf("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);/*xcat debug*/

    rc = waInit(boardRevId);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("waInit", rc);

    /* For xCat2 B2B change options for ports 24-27 on both devices */
    if((2 == ppCounter) && (PRV_CPSS_PP_MAC(0)->devFamily == CPSS_PP_FAMILY_DXCH_XCAT2_E))
    {
        for (port=24; port<=27;port++) 
        {
            PRV_CPSS_PP_MAC(0)->phyPortInfoArray[port].portTypeOptions = PRV_CPSS_GE_PORT_GE_ONLY_E;
            PRV_CPSS_PP_MAC(1)->phyPortInfoArray[port].portTypeOptions = PRV_CPSS_GE_PORT_GE_ONLY_E;
        }    
    }
...
)
{
    ...
    if (appDemoPpConfigList[oldDevNum].cpuPortMode ==  PRV_CPSS_GEN_NETIF_CPU_PORT_MODE_SDMA_E)
    {
        printf("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);/*xcat debug*/
    ...
    {
        printf("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);/*xcat debug*/
        /* do not allocate rx/tx descriptors and rx buffers*/
        retVal = appDemoAllocateDmaMem(localPpPh2Config.deviceId,0,0,RX_BUFF_ALLIGN_DEF,0,
                                       AU_DESC_NUM_DEF,
                                       &localPpPh2Config);
    }
    ...
    printf("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);/*xcat debug*/	
    #if 0
	retVal = GT_OK;/*xcat debug*/
    #endif
    printf("\t\t in getPpPhase2Config() !!! zhangdi\n");
...
)
{
    ...
    else
    {
        ...
        rc = cpssDxChPortSpeedSet(devNum,portNum,CPSS_PORT_SPEED_12000_E);
        CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChPortSpeedSet", rc);
        if (rc != GT_OK)
        {
            return rc;
...
)
{
    ...
    for(i = SYSTEM_DEV_NUM_MAC(0); i < SYSTEM_DEV_NUM_MAC(ppCounter); ++i)
    {
		...
		/* 2X12G12S PHY addr start from 4, need't modify. but AC12C is form 8 */
        if((BOARD_TYPE == BOARD_TYPE_AX81_AC12C)||(BOARD_TYPE == BOARD_TYPE_AX81_AC8C))
        {
			printf("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__); /*xcat debug*/
			printf(" Here we change the port-phy address !!\n"); /*xcat debug*/
			
            /* port[0-3] === PHY address[8-11], zhangdi 2011-04-07 */
            /* port 0-23*/
            for(portNum = 0; portNum < 24; portNum++)
            {
                if( !PRV_CPSS_PHY_PORT_IS_EXIST_MAC(devNum, portNum) )
                    continue;
                /* Here the PHY address is start from 0x8(port 0) in AC12c */
    ...
    {/* ppCounter == 2 */
        ...
        for(i = SYSTEM_DEV_NUM_MAC(0); i < SYSTEM_DEV_NUM_MAC(ppCounter); ++i)
        {
            ...
            {
                /* retrieve CPU enabled from Sampled at Reset - CPU_INT_EXT */
                retVal = prvCpssDrvHwPpGetRegField(devNum,0x0000002C,14,1,&cpuDisabled);
                if (retVal != GT_OK)
                {
                    return retVal;
                }
    ...
    else
    {
        ...
        /* Board Type classification is needed! */
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].internal10GPortConfigFuncPtr  = internalXGPortConfig;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].internalCscdPortConfigFuncPtr = cascadePortConfig;
        ...
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].numberOfCscdPorts = 2;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdPortsArr[0].portNum = 26;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdPortsArr[0].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdPortsArr[0].trunkId = 0xFFFF;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdPortsArr[1].portNum = 27;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdPortsArr[1].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdPortsArr[1].trunkId = 0xFFFF;

        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].numberOfCscdTargetDevs = 4;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[0].targetDevNum = 0;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[0].linkToTargetDev.linkNum  = 26;/* port 26 link to 98Dx275(slot 0) */
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[0].linkToTargetDev.linkType = CPSS_CSCD_LINK_TYPE_PORT_E;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[1].targetDevNum = 1;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[1].linkToTargetDev.linkNum  = 26;/* port 26 link to 98Dx804(slot 0) */
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[1].linkToTargetDev.linkType = CPSS_CSCD_LINK_TYPE_PORT_E;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[2].targetDevNum = 2;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[2].linkToTargetDev.linkNum  = 27;/* port 26 link to 98Dx275(slot 1) */
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[2].linkToTargetDev.linkType = CPSS_CSCD_LINK_TYPE_PORT_E;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[3].targetDevNum = 3;
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[3].linkToTargetDev.linkNum  = 27;/* port 26 link to 98Dx804(slot 1) */
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].cscdTargetDevsArr[3].linkToTargetDev.linkType = CPSS_CSCD_LINK_TYPE_PORT_E;
        #else
        appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].internal10GPortConfigFuncPtr = internalXGPortConfig;
    ...
    for(i = SYSTEM_DEV_NUM_MAC(0); i < SYSTEM_DEV_NUM_MAC(ppCounter); ++i)
    {
        ...
        if(PRV_CPSS_PP_MAC(devNum)->revision > 0)
        {
            if (boardRevId == 5)
            {
                ...
                for (port = 24; port < 26;port++)
                {
                    ...
                    rc = prvCpssDrvHwPpSetRegField(devNum,0x8c,port+4,1,1);
                    CPSS_ENABLER_DBG_TRACE_RC_MAC("prvCpssDrvHwPpSetRegField", rc);
                    if( rc != GT_OK )
                    {
                        return rc;
                    }
        ...
        switch(appDemoPpConfigList[i].deviceId)
        {
            case PRV_CPSS_DXCH_XCAT2_FE_DEVICES_CASES_MAC:
                ...
                rc = prvCpssDrvHwPpSetRegField(devNum,regAddr,30,1,1);
                if (rc != GT_OK)
                    return rc;
            break;
        }
