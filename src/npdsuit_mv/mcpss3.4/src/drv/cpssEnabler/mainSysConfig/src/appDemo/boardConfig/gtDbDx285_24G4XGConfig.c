...
)
{
    ...
    if (appDemoPpConfigList[devIdx].channel == CPSS_CHANNEL_PCI_E)
    {
	   ...
	   osPrintf("getPpPhase1Config::devIdx %d devId %#0x pciBase %#0x deviceBase %#0x \n", \
	   			devIdx,localPpPh1Config.deviceId,localPpPh1Config.baseAddr,localPpPh1Config.internalPciBase);
       ...
       localPpPh1Config.intMask   = (GT_U32)intVecNum;
#else
	if(boardRevId == 2)
	{/* RD-DX285-48G-4XG */
		#if 0
		...
		#else
		extDrvGetPciIntVec(devIdx ,&intVecNum);
		localPpPh1Config.intVecNum = (GT_U32)intVecNum;
		extDrvGetIntMask(devIdx,&(localPpPh1Config.intMask));
		#endif
	}
	else
	{
	...
	osPrintf("getPpPhase1Config::devIdx %d intVecNum %d mask %#0x\n", \
   			devIdx,localPpPh1Config.intVecNum,localPpPh1Config.intMask);
...
)
{
    ...
    retVal = appDemoAllocateDmaMem(localPpPh2Config.deviceId,RX_DESC_NUM_DEF,
                                   ...
                                   &localPpPh2Config);
    #endif
    /*luoxun -- The arguments of appDemoAllocateDmaMem() are different from cpss1.3*/
    #if 0
    #ifdef __AX_PLATFORM__
    retVal = appDemoAllocateDmaMem(localPpPh2Config.deviceId,RX_DESC_NUM_DEF,
                                   RX_BUFF_SIZE_DEF,RX_BUFF_ALLIGN_DEF,
                                   ...
                                   &localPpPh2Config);
    #else   
    retVal = appDemoAllocateDmaMem(localPpPh2Config.deviceId,localPpPh2Config.devNum,
    							   RX_DESC_NUM_DEF,RX_BUFF_SIZE_DEF,RX_BUFF_ALLIGN_DEF,
                                   TX_DESC_NUM_DEF,AU_DESC_NUM_DEF,
                                   &localPpPh2Config);
    #endif
    #endif
...
*/
    ...
    if(PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[port_num].portType !=
       PRV_CPSS_PORT_XG_E)
    {
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->smiRegs.smi10GePhyConfig0[port_num];

    /* read Port<n> XAUI PHY External Configuration Register0 */
    ...
    /*  Set XAUI PHY External Configuration register0
        1. Set PHY to Reset state - clear bit 0
        2. Set PHY power control - set bit 1 */
    value |= 0x3;

    /* write register to unreset XAUI PHY and PowerUP */
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->smiRegs.smi10GePhyConfig1[port_num];

    /* read Port<n> XAUI PHY External Configuration Register1 */
    rc = prvCpssDrvHwPpReadRegister(devNum, regAddr, &value);
    if (rc != GT_OK)
    {
        return rc;
    ...
    /*  Set Port<n> XAUI PHY External Configuration Register1
        1. Set PHYDevAddr to 5 - DTE device
        2. Set AnyDevAddrEn to 0 - responds to SMI transactions with device address=<PHYDevAddr>  */
    value &= 0xFFFFFFC5;
...
)
{
	...
	/* for product AU5612 GE port(0,0) as CASCADE port */
	if (( PRODUCT_ID_AX5K != auteware_product_id ) && ( PRODUCT_ID_AX5K_I != auteware_product_id )) {
...
*
*******************************************************************************/
/*luoxun --In this function, combination may be wrong.*/
...
)
{
    ...
    GT_U8   ppCounter;
    #ifndef __AX_PLATFORM__
    GT_U8 numberOf10GPortsToConfigure = 0;
	GT_U8 ports10GToConfigureArr[2] = {0};
	GT_U8 numberOfCscdTrunks = 0;
	GT_U8 numberOfCscdPorts = 0;
	GT_U8 numberOfCscdTargetDevs = 0;
	CSCD_INIT_PORT_DESC_STC cscdPortsArr[2] = {0};
	CSCD_INIT_UC_TBL_STC cscdTargetDevsArr[2] = {0};
    #endif
    ...
    {/* ppCounter == 2 */
        ...
        for(i = SYSTEM_DEV_NUM_MAC(0); i < SYSTEM_DEV_NUM_MAC(ppCounter); ++i)
        {
            #ifndef __AX_PLATFORM__
            /* 98DX275 device */
			if(CPSS_98DX275_CNS == appDemoPpConfigList[i].deviceId)
			{
				/* XG ports */
				numberOf10GPortsToConfigure = 1;
				ports10GToConfigureArr[0] = 24;
				/* ports10GToConfigureArr[1] = 25; */

				/* Cascading */
				numberOfCscdTrunks = 0;
				numberOfCscdPorts = 2;
				cscdPortsArr[0].portNum = 26;
				cscdPortsArr[0].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
				cscdPortsArr[0].trunkId = 0xFFFF;
				cscdPortsArr[1].portNum = 25;
				cscdPortsArr[1].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
				cscdPortsArr[1].trunkId = 0xFFFF;

				/* Cascading Target */
				numberOfCscdTargetDevs = 2;
				
				cscdTargetDevsArr[0].targetDevNum = (0 == i) ? 1 : 0;
				cscdTargetDevsArr[0].linkToTargetDev.linkNum = 26;/* port 26 link to 98Dx804 */
				cscdTargetDevsArr[0].linkToTargetDev.linkType = CPSS_CSCD_LINK_TYPE_PORT_E;
				cscdTargetDevsArr[1].targetDevNum = 4;
				cscdTargetDevsArr[1].linkToTargetDev.linkNum = 25;/* port 25 link to 98Dx4122 */
				cscdTargetDevsArr[1].linkToTargetDev.linkType = CPSS_CSCD_LINK_TYPE_PORT_E;
			}
			/* 98DX804 device */
        	else if(CPSS_98DX804_CNS == appDemoPpConfigList[i].deviceId)
        	{
				/* XG ports */
				numberOf10GPortsToConfigure = 3;
				ports10GToConfigureArr[0] = 24;
				ports10GToConfigureArr[1] = 25;
				#ifndef __AX_PLATFORM__
				ports10GToConfigureArr[2] = 26;
				#endif
				
				/* Cascading */
				numberOfCscdTrunks = 0;
				numberOfCscdPorts = 1;
				cscdPortsArr[0].portNum = 27;
				cscdPortsArr[0].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
				cscdPortsArr[0].trunkId = 0xFFFF;
				#ifdef __AX_PLATFORM__
				cscdPortsArr[1].portNum = 26;
				cscdPortsArr[1].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
				cscdPortsArr[1].trunkId = 0xFFFF;
				#endif

				/* Cascading Target */
				numberOfCscdTargetDevs = 1;
				
				cscdTargetDevsArr[0].targetDevNum = (0 == i) ? 1 : 0;
				cscdTargetDevsArr[0].linkToTargetDev.linkNum = 27;/* port 27 link to 98Dx275 */
				cscdTargetDevsArr[0].linkToTargetDev.linkType = CPSS_CSCD_LINK_TYPE_PORT_E;

			}			
            #endif
            /* XG ports */
            appDemoPpConfigList[i].numberOf10GPortsToConfigure = numberOf10GPortsToConfigure;
            appDemoPpConfigList[i].ports10GToConfigureArr[0] = ports10GToConfigureArr[0];
            appDemoPpConfigList[i].ports10GToConfigureArr[1] = ports10GToConfigureArr[1];
			#ifndef __AX_PLATFORM__
			appDemoPpConfigList[i].ports10GToConfigureArr[2] = ports10GToConfigureArr[2];
			#endif
            
            /* Cascading */
            appDemoPpConfigList[i].numberOfCscdTrunks = numberOfCscdTrunks;
            appDemoPpConfigList[i].numberOfCscdPorts = numberOfCscdPorts;
            appDemoPpConfigList[i].cscdPortsArr[0].portNum = cscdPortsArr[0].portNum;
            appDemoPpConfigList[i].cscdPortsArr[0].cscdPortType = cscdPortsArr[0].cscdPortType;
            appDemoPpConfigList[i].cscdPortsArr[0].trunkId = cscdPortsArr[0].trunkId;
            appDemoPpConfigList[i].cscdPortsArr[1].portNum = cscdPortsArr[1].portNum;
            appDemoPpConfigList[i].cscdPortsArr[1].cscdPortType = cscdPortsArr[1].cscdPortType;
            appDemoPpConfigList[i].cscdPortsArr[1].trunkId = cscdPortsArr[1].trunkId;
			
			/* Cascading Target */
            appDemoPpConfigList[i].numberOfCscdTargetDevs = numberOfCscdTargetDevs;

            appDemoPpConfigList[i].cscdTargetDevsArr[0].targetDevNum = cscdTargetDevsArr[0].targetDevNum;
            appDemoPpConfigList[i].cscdTargetDevsArr[0].linkToTargetDev.linkNum = cscdTargetDevsArr[0].linkToTargetDev.linkNum;
            appDemoPpConfigList[i].cscdTargetDevsArr[0].linkToTargetDev.linkType = cscdTargetDevsArr[0].linkToTargetDev.linkType;
			appDemoPpConfigList[i].cscdTargetDevsArr[1].targetDevNum = cscdTargetDevsArr[1].targetDevNum;
			appDemoPpConfigList[i].cscdTargetDevsArr[1].linkToTargetDev.linkNum = cscdTargetDevsArr[1].linkToTargetDev.linkNum;
			appDemoPpConfigList[i].cscdTargetDevsArr[1].linkToTargetDev.linkType = cscdTargetDevsArr[1].linkToTargetDev.linkType;
    ...
    else
    {
        ...
        #endif
        /*
              *luoxun -- need modification.
              *          appDemoPpConfigList[0]
              *          appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)]
              */
        if(CPSS_98DX265_CNS == appDemoPpConfigList[0].deviceId) {
			appDemoPpConfigList[0].numberOf10GPortsToConfigure = 2;
			appDemoPpConfigList[0].ports10GToConfigureArr[0] = 24;
			appDemoPpConfigList[0].ports10GToConfigureArr[1] = 25;

			/* for product AU5612 port(0,0) as CASCADE port */
			if(PRODUCT_ID_AX5K == auteware_product_id) {
				appDemoPpConfigList[0].numberOfCscdPorts = 1;
				appDemoPpConfigList[0].cscdPortsArr[0].portNum = 0;
				appDemoPpConfigList[0].cscdPortsArr[0].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
				appDemoPpConfigList[0].cscdPortsArr[0].trunkId = 0xFFFF;
			}
			else if(PRODUCT_ID_AX5K_I == auteware_product_id) {
	 			/* for product AU5612 port(0,24) as CASCADE port */
				appDemoPpConfigList[0].numberOfCscdPorts = 1;
				appDemoPpConfigList[0].cscdPortsArr[0].portNum = 24;
				appDemoPpConfigList[0].cscdPortsArr[0].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
				appDemoPpConfigList[0].cscdPortsArr[0].trunkId = 0xFFFF;
			}						
		}
		/* by qinhs@autelan.com on 2009-7-26 
		  * 	for AX5612i demo board verification
		  */
		else if(CPSS_98DX275_CNS == appDemoPpConfigList[0].deviceId) { 
			if(PRODUCT_ID_AX5K_I == auteware_product_id) {
				/* for product AU5612 port(0,24) as CASCADE port */
				appDemoPpConfigList[0].numberOfCscdPorts = 1;
				appDemoPpConfigList[0].cscdPortsArr[0].portNum = 24;
				appDemoPpConfigList[0].cscdPortsArr[0].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_EXTEND_E;
		...
		else if(CPSS_98DX255_CNS == appDemoPpConfigList[0].deviceId) {
			appDemoPpConfigList[0].numberOf10GPortsToConfigure = 0;
		}
		else {
			appDemoPpConfigList[0].numberOf10GPortsToConfigure = 0;
		}
   ...
   for SOHO only builds. */
    ...
    if(boardRevId == 4)
    {
        ...
        for(vlanEgressPortNum = 24; vlanEgressPortNum < 28; vlanEgressPortNum++)
        {
            ...
            retVal = cpssDxChBrgVlanPortVidSet(devNum, (GT_U8)ingressPortNum,
            ...
            retVal = cpssDxChBrgVlanPortVidSet(devNum, (GT_U8)vlanEgressPortNum, 
                                              (GT_U16)vlanEgressPortNum); 
            if(retVal != GT_OK)
            {
                return retVal;
