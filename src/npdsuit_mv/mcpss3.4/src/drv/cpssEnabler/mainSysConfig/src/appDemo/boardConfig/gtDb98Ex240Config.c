...
)
{
    ...
    for (devId = 0; devId < PRV_CPSS_MAX_PP_DEVICES_CNS ; devId++)
    {
        if ((GT_FALSE == appDemoPpConfigList[devId].valid) ||
            (appDemoPpConfigList[devId].devFamily != CPSS_PP_FAMILY_PUMA_E))
        {
            continue;
        }
...
)
{
    ...
    if( hsuInfo.hsuState == CPSS_HSU_STATE_COMPLETED_E)
    {
...
)
{
    ...
    CPSS_PORT_TX_Q_TAIL_DROP_PROF_TC_PARAMS tailDropProfile;
    GT_U32                                  profile;
    GT_U32                                  tc;
    ...
    for (i = 0; (i < PRV_CPSS_MAX_PP_DEVICES_CNS) ; i++)
    {
        ...
        if(appDemoPpConfigList[i].devFamily != CPSS_PP_FAMILY_PUMA_E)
        {
...
)
{
        ...
        PRV_CPSS_PP_MAC(devNum)->devType == CPSS_98EX8502_CNS  )
    {
        ...
        rc = prvCpssDrvHwPpWriteRegister(devNum,regAddr,0x40);
        if( rc != GT_OK )
        {
            return rc;
        }
        regAddr = PRV_CPSS_EXMXPM_DEV_REGS_MAC(devNum)->
                                LMS1.LMS1Group0.PHYAutoNegConfigReg2;
        rc = prvCpssDrvHwPpWriteRegister(devNum,regAddr,0x40);
        if( rc != GT_OK )
        {
            return rc;
    ...
    for (fport = 0; fport < 4; fport++)
    {
        ...
        for (i = 0; i < CPSS_EXMXPM_FABRIC_SERDES_LANES_NUM_CNS; i++)
        {
            regAddr = PRV_CPSS_EXMXPM_PP_MAC(devNum)->regsAddr.
                HG_XPCS.laneRegs.laneCfg.laneCfg1[fport][i];
            if(rldBoard == GT_TRUE)
            {
                rc = prvCpssDrvHwPpWriteRegister(devNum, regAddr, rldPcsTxLaneArr[i]);
            }
            else
            {
                rc = prvCpssDrvHwPpWriteRegister(devNum, regAddr, qdrPcsTxLaneArr[i]);
            }
            CPSS_ENABLER_DBG_TRACE_RC_MAC("prvCpssDrvHwPpWriteRegister", rc);
            if (rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    for (ii = 0; (ii < PRV_CPSS_MAX_PP_DEVICES_CNS) && (GT_OK == rc); ii++)
    {
        ...
        if(appDemoPpConfigList[ii].devFamily == CPSS_PP_FAMILY_PUMA_E)
        {
            /*******************************************************************************/
            ...
            prvCpssDrvHwPpSetRegField(dev, 0x100000, 12, 2, 0);
            prvCpssDrvHwPpSetRegField(dev, 0x340004, 31, 1, 1);

            /* Set cascade mode per port to be value_port. */
            prvCpssDrvHwPpSetRegField(dev, 0x00D00084, 0, 32, 0x00FFFFFF);
