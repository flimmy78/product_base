...
*
* COMMENTS:
*       [SergiyL]: not all devices are handled;  I'm not sure all devices assigned to correct families;
*       Better rewrite this func (or replace call of this func) in same way as in appDemoSysConfigFuncsGet()
*
*******************************************************************************/
...
)
{
    ...
    isFirst = GT_TRUE;
    printf("appDemoSysGetPciInfo()\n");
    printf("devIdx = %d, isFirst = %d, &pciInfo = %x\n", devIdx, isFirst, &pciInfo);
    ...
    while (GT_OK == status)
    {
        ...
        /* get family - started by Natan, finished by SergiyL */
        ...
        isFirst = GT_FALSE;
        printf("devIdx = %d, isFirst = %d, pciInfo.pciBusNum = %d\n", devIdx, isFirst, pciInfo.pciBusNum );
