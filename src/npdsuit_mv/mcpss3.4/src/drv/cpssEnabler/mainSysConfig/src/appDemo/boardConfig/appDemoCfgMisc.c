...
)
{
    ...
    if(((GT_U32)ppPhase2Params->auqCfg.auDescBlock) % tmpData)
    {
    ...
    /* Au block size calc & malloc  */
    coreGetAuDescSize(devType,&tmpData);
    ppPhase2Params->fuqCfg.auDescBlockSize = tmpData * auDescNum;
    ...
    ppPhase2Params->netIfCfg.rxBufInfo.allocMethod = CPSS_RX_BUFF_STATIC_ALLOC_E;
    ...
    if((((GT_U32)tmpPtr) % rxBufAllign) != 0)
    {
        tmpPtr = (GT_U32*)(((GT_U32)tmpPtr) +
                           (rxBufAllign - (((GT_U32)tmpPtr) % rxBufAllign)));
    }
