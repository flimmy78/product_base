        ...
        for(i = 0; i < txDxChTblCapacity; i++) /*For each descriptor*/
        {
            ...
            {/*For packet sent by this descriptor*/
                ...
                switch(cpssDxChTxPacketDescTbl[i].dsaParam.dsaType)
                {
                    case DXCH_NET_DSA_CMD_FROM_CPU_E:
                        ...
                        cpssPcktParams.dsaParam.dsaInfo.fromCpu.srcDev = cpssDxChTxPacketDescTbl[i].dsaParam.dsaInfo.fromCpu.srcDev;
                        cpssPcktParams.dsaParam.dsaInfo.fromCpu.dstInterface.devPort.devNum = cpssDxChTxPacketDescTbl[i].dsaParam.dsaInfo.fromCpu.dstInterface.devPort.devNum;
    
                        devNum = cpssPcktParams.dsaParam.dsaInfo.fromCpu.dstInterface.devPort.devNum;
    
                        /* the value of cpssPcktParams.dsaParam.dsaInfo.fromCpu.dstInterface.devPort.devNum
                           was converted to 'hwDevNum' .
    
                           so to apply as 'sw devNum' we need to 'Convert it back'
                        */
                        CONVERT_BACK_DEV_PORT_DATA_MAC(devNum,dummyPort);
                        ...
                        if(cpssDxChTxPacketDescTbl[i].cmdType == DXCH_TX_BY_VIDX)
                        {
                            ...
                            {
                                cpssPcktParams.dsaParam.dsaInfo.fromCpu.extDestInfo.multiDest.excludedInterface.devPort.devNum = cpssDxChTxPacketDescTbl[i].dsaParam.dsaInfo.fromCpu.extDestInfo.multiDest.excludedInterface.devPort.devNum;
                    ...
                    case DXCH_NET_DSA_CMD_FORWARD_E:
                        ...
                        cpssPcktParams.dsaParam.dsaInfo.forward.srcDev = cpssDxChTxPacketDescTbl[i].dsaParam.dsaInfo.forward.srcDev;
                        ...
                        cpssPcktParams.dsaParam.dsaInfo.forward.dstInterface.devPort.devNum = cpssDxChTxPacketDescTbl[i].dsaParam.dsaInfo.forward.dstInterface.devPort.devNum;

                        devNum = cpssPcktParams.dsaParam.dsaInfo.forward.dstInterface.devPort.devNum;
                        /* the value of cpssPcktParams.dsaParam.dsaInfo.fromCpu.dstInterface.devPort.devNum
                           was converted to 'hwDevNum' .
                ...
                /* send ok,signal User Process */
				#ifndef __AX_PLATFORM__
				prvCpssDrvEvReqNotify((PRV_CPSS_DRV_EVENT_HNDL_STC*)evReqHndl);
				#endif
                    ...
                    (status == GT_NO_RESOURCE)))
                {
                    ...
                    retVal = cmdCpssEventSelect(evReqHndl,NULL,NULL,0);
                    ...
                    if(cmdIsCpuEtherPortUsed() == GT_TRUE)
                    {
                        while(1)
                        {
                            retVal = cpssDxChNetIfMiiTxBufferQueueGet(evReqHndl,&_devNum,
                    ...
                    else
                    {
                        while(1)
                        {
                            retVal = cpssDxChNetIfTxBufferQueueGet(evReqHndl,&_devNum,
                                                            &cookie,&queueIdx,&retVal);
                            if(retVal == GT_NO_MORE)
                            {
                                break;
          ...
          if(status == GT_NO_RESOURCE)
          {
            if(cpssDxChTxPacketDescTbl[i].txSyncMode==GT_TRUE)
            {
              if(cmdIsCpuEtherPortUsed() == GT_TRUE) /* native mii ? */
              {
                cmdOsPrintf("startPacketTransmission 1 native mii tbd\n"); 
              }
              else
              {
                status = 
            ...
            else
            {
              if(cmdIsCpuEtherPortUsed() == GT_TRUE) /* native mii ? */
              {
                cmdOsPrintf("startPacketTransmission 2 native mii tbd\n"); 
              }
              else
              {
                status = 
...
)
{
    ...
    for(entryIndex = 0; entryIndex < txDxChTblCapacity; entryIndex++)
    {
    ...
    if(entryIndex == txDxChTblCapacity)
    {
        ...
        {
            cpssDxChTxPacketDescTbl = cmdOsMalloc(txDxChTblCapacity * sizeof(DXCH_PKT_DESC_STC));
        }
        else
        {
            cpssDxChTxPacketDescTbl = cmdOsRealloc(cpssDxChTxPacketDescTbl,
                                        txDxChTblCapacity * sizeof(DXCH_PKT_DESC_STC));
        }
        if(cpssDxChTxPacketDescTbl == NULL)
        {
            txDxChTblCapacity = 0;
...
)
{
    ...
    for (; *entryIndex < txDxChTblCapacity; (*entryIndex)++)
    {
...
)
{
    ...
    if(cpssDxChTxPacketDescTbl != NULL)
    {
         ...
         if((numberOfPacketsDeleted + numberOfPacketsNotValid) == txDxChTblCapacity)
         {
...
)
{
    ...
    if(multiProcessAppDemo != 0)
    {
        ...
        remoteCmd.command = CPSS_MP_CMD_RXTX_START_TRANSMIT_E;
                ...
                remoteCmd.data.rxTxStartTransmit.txDxChTblCapacity = txDxChTblCapacity;
