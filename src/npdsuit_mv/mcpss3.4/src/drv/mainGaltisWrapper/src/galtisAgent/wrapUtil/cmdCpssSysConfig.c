...
static GT_U32           sysConfLlIterator;
...
GT_STATUS mngGetSysNextElement(GT_VOID)
{
    ...
    switch(sysConfInfo->deviceId)
    {
        ...
        case PRV_CPSS_DXCH_XCAT_DEVICES_CASES_MAC:/* xCat devices */
          /* to avoid need in conversion script for Galtis tests return faked
...
static GT_STATUS cmdBuildSysConf(GT_VOID)
{
    ...
    GT_U8               i,j,k = PRV_CPSS_MAX_PP_DEVICES_CNS;

    GT_U8   fakePortsNum;
    CMD_SYSCONF_INFO    devSysConfData;
    CMD_SYSCONF_INFO    portSysConfData;
