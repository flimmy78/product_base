...
static GT_STATUS mmuGroup(GT_U32  poolId,STRUCT_MEM_DATA *first,
...
*
*******************************************************************************/
GT_U32 prvCpssMemDevPoolCreate()
{
    GT_VOID *poolId;
...
)
{
    ...
    if (nodePtrId < allocatedBase)
    {
        ...
        tmpMemData->size = (allocatedBase - nodePtrId);
...
)
{
    ...
    if(MEM_NODE_PTR(first) == (GT_U32)NULL)
    {
        return GT_FAIL;
    }
...
)
{
    ...
    GT_U32           freeIterator ;   /* iterator for nodes */
    ...
    if (memData->ptr != ptr)
    {
        memFreeLeft = mmuNewData(memData->ptr, (ptr - memData->ptr), FREEPTR) ;
        if (memFreeLeft == NULL)
        {
            return GT_FAIL ;
        }
    ...
    if ((memData->ptr + memData->size) > (ptr + size))
    {
        memFreeRight = mmuNewData((ptr + size),
            ((memData->ptr + memData->size) - (ptr + size)),
            FREEPTR) ;
...
{
    GT_U32           freeIterator ;  /* iterator for free memory nodes */
    GT_U32           usedIterator ;  /* iterator for allocated memory nodes */
    ...
    *memoBlocksNumPtr = 0 ;
    ...
    while (1)
    {
        ...
        if ((endFree == GT_FALSE) && (endUsed == GT_FALSE))
        {
            if ((GT_U32)(freeMemData->ptr) <= (GT_U32)(usedMemData->ptr))
            {
                memData = freeMemData ;
                nextFree = GT_TRUE  ;
                nextUsed = GT_FALSE ;
