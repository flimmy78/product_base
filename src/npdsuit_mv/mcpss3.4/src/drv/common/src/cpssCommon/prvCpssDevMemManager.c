...
)
{
    ...
    if (OFFSET_IN_WORDS(foundBlock) == givenHwOffsetInWords)
    {
        ...
        if (SIZE_IN_WORDS(foundBlock) == blockSize)
        {
            ...
            allocatedBlock->nextBySizeOrPartitionPtr.partitionPtr = partition;
            *hwOffsetHandlePtr = (GT_32)allocatedBlock;
            return GT_OK;
        }
        else
        {
...
)
{
    ...
    if( (handler == 0)  || (partition == 0) ||
        (partition == (GT_DMM_PARTITION*)0xFFFFFFFE) ||
...
{
    GT_U32 pId;
    GT_U32 allocatedTable[1024], i, j, stam, mm;
    GT_U32 loopId;
    ...
    for(loopId = 0; loopId < 20; loopId++)
    {
        for ( i = 0; i < 1024; i++ )
        {
            if( 0 == (allocatedTable[i] =
                      prvCpssDmmAllocate(pId,((osRand()%900)+3)*4 , 2)))
            {
            ...
            if( (loopId == 0) && ( i == 550 ) )
                ...
                if( 0 == (allocatedTable[j] = prvCpssDmmAllocate(pId,((osRand()%900)+3)*4,2)))
                {
                    cpssOsPrintf("Alloc Fail");
                    return 26;
                }
...
{
    GT_U32 pId;
    GT_U32 *allocatedTable, i, j;
    GT_U32 loopId;
    GT_U32       *memoryMap;

    allocatedTable = cpssOsIpMalloc(4*1000*1024);
...
{
    GT_U32 pId;
    GT_U32 *allocatedTable, i, j, stam;
    GT_U32 loopId;
    GT_U32 *memoryMap;
    ...
    for(loopId = 0; loopId < 1; loopId++)
    {
        ...
        {
            if( 0 == (allocatedTable[i] =
                      prvCpssDmmAllocate(pId,((osRand()%13)+3)*4 , 2)))
            {
                cpssOsPrintf("Alloc Fail");
                return 26;
            }
