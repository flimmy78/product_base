...
CPSS_EXTDRV_HSU_WARM_RESTART_FUNC              cpssExtDrvHsuWarmRestart
  = (CPSS_EXTDRV_HSU_WARM_RESTART_FUNC) extServiceFuncNotImplementedCalled;
...
)
{
    ...
    {
        /* ext drv functions */
        currPtr = (GT_U32*)extDrvFuncBindInfoPtr;
        currSize = sizeof(CPSS_EXT_DRV_FUNC_BIND_STC) / sizeof(GT_U32);
        currGlobPtr = (GT_U32*)&prvCpssBindFuncs.extDrv;

        for(ii = 0 ; ii < currSize ; ii++,currPtr++,currGlobPtr++)
        {
            if(currPtr)
            {
    ...
    {
        /* OS functions */
        currPtr = (GT_U32*)osFuncBindPtr;
        currSize = sizeof(CPSS_OS_FUNC_BIND_STC) / sizeof(GT_U32);
        currGlobPtr = (GT_U32*)&prvCpssBindFuncs.os;
