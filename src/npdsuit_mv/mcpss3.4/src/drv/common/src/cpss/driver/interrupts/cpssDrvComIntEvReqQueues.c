...
)
{
	...
	/* Add lock before modify prvCpssDrvComIntEvReqQueuesDb
     * the cpssOsTaskLock() in this macro is the true thread lock, we need it.
     * zhangdi@autelan.com   2012-06-21
     */
    INT_LOCK(intKey);
...
)
{
    if(hndlEvPtr->hndlBindType == PRV_CPSS_DRV_HANDLE_BIND_USE_SIGNAL_TYPE_E)
    {
        ...
        if(GT_FALSE == PRV_CPSS_DRV_HANDEL_SEM_SIGNAL_MAC(hndlEvPtr))
        {
            ...
            /* and signal the user process */
            cpssOsSigSemSignal((GT_VOID*)PRV_CPSS_DRV_HANDEL_SEM_MAC(hndlEvPtr));
        }
    }
    else
    {
