...
)
{
    ...
    if(handleBindType == PRV_CPSS_DRV_HANDLE_BIND_USE_SIGNAL_TYPE_E)
    {
        ...
        PRV_CPSS_DRV_HANDEL_SEM_MAC(evHndlPtr) = (GT_U32)semId;
...
)
{
    ...
    {
        cpssOsSigSemSignal((GT_VOID*)PRV_CPSS_DRV_HANDEL_SEM_MAC(evHndlPtr));
        /* allow application to finish 'last moment' issues with the handler */
        cpssOsTimerWkAfter(10);
        cpssOsSigSemDelete((GT_VOID*)PRV_CPSS_DRV_HANDEL_SEM_MAC(evHndlPtr));
    }
...
{
    return prvCpssDrvInterruptConnect(intVecNum,
                            intMask,intRoutine,
                            cookie,connectionId);
}
