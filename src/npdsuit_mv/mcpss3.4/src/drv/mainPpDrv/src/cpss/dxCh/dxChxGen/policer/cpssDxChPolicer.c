...
#if 1
/* this is for nam_*.* to check the asic family , by hanhui@autelan.com 2011-05-12*/
int cpssDxChFamilyCheck
(
	IN  GT_U8       devNum,
	IN  GT_U32      family
){
#ifndef __AX_PLATFORM__
	if(PRV_CPSS_PP_MAC(devNum)->appDevFamily & family){
		return GT_TRUE;
	}
#endif
	return GT_FALSE;
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 2, hwData);
}
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 2, 1, hwData);
}
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 7, 2, hwData);
}
...
)
{
    ...
    return prvCpssDrvHwPpSetRegField(devNum, regAddr, 4, 3, hwData);
}
