...
)
{
    ...
    {
        rc = prvCpssDrvHwPpPortGroupWriteRam(devNum, portGroupId, addr, length,
                                             dataPtr);
    }
    return rc;
}
...
)
{
    ...
    if (ifChannel > CPSS_CHANNEL_PEX_E || regType > CPSS_DIAG_PP_REG_PCI_CFG_E)
    {
        return GT_BAD_PARAM;
    }

    if (ifChannel == CPSS_CHANNEL_PCI_E ||
        ifChannel == CPSS_CHANNEL_PEX_E)
    {
        return regPciWrite (baseAddr, regType, offset, data, doByteSwap);
    }
    else if (ifChannel == CPSS_CHANNEL_SMI_E)
    {
        if (regType == CPSS_DIAG_PP_REG_PCI_CFG_E)
        {
            return GT_FAIL;
        }
        return regSmiWrite (baseAddr, offset, data, doByteSwap);
    }
    else /* ifChannel == CPSS_CHANNEL_TWSI_E */
    {
        if (regType == CPSS_DIAG_PP_REG_PCI_CFG_E)
        {
            return GT_FAIL;
        }
        return regTwsiWrite (baseAddr, offset, data, doByteSwap);
    }
}
...
)
{
    ...
    else if (ifChannel == CPSS_CHANNEL_SMI_E)
    {
        ...
        return regSmiRead (baseAddr, offset, dataPtr, doByteSwap);
    }
    else /* ifChannel == CPSS_CHANNEL_TWSI_E */
    {
        if (regType == CPSS_DIAG_PP_REG_PCI_CFG_E)
        ...
        return regTwsiRead (baseAddr, offset, dataPtr, doByteSwap);
    }
}
...
)
{
    ...
    if (ifChannel > CPSS_CHANNEL_PEX_E || phyAddr >= BIT_5 ||
...
)
{
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(devNum,portGroupId)
    {
        ...
        status = prvCpssDrvHwPpPortGroupReadRegister (devNum, portGroupId,
                                                      regAddr, &originalVal);
        if (status != GT_OK)
        {
            return status;
        ...
        if (status != GT_OK)
        {
            status1 = prvCpssDrvHwPpPortGroupWriteRegister (devNum, portGroupId, regAddr,
                                                            originalVal);
            if(status1 != GT_OK)
                return status1;
            else
        ...
        status = prvCpssDrvHwPpPortGroupWriteRegister (devNum, portGroupId, regAddr,
                                                       originalVal);
        if (status != GT_OK)
        {
            return status;
...
)
{
    ...
    {
        str1_Ptr  = (GT_U32)(str1Ptr) + loop ;
        str2_Ptr  = (GT_U32)(str2Ptr) + loop ;
...
)
{
    ...
    status = prvCpssDrvHwPpPortGroupWriteRegBitMask (devNum, portGroupId,
    ...
    status = prvCpssDrvHwPpPortGroupReadRegBitMask (devNum, portGroupId,
...
)
{
    ...
    if (regType == CPSS_DIAG_PP_REG_PCI_CFG_E)
    {
        ...
        *((volatile GT_U32*)address) = DIAG_BYTE_SWAP_MAC(doByteSwap, regData);
    }
    else /* CPSS_DIAG_PP_REG_INTERNAL_E */
    {
        ...
        *(volatile GT_U32 *)(address) = DIAG_BYTE_SWAP_MAC(doByteSwap, data);
        data = ((GT_U8)(offset >> 24)) << 16;
        *(volatile GT_U32 *)(address) = DIAG_BYTE_SWAP_MAC(doByteSwap, data);
        data = ((GT_U8)(offset >> 24)) << 16;
        *(volatile GT_U32 *)(address) = DIAG_BYTE_SWAP_MAC(doByteSwap, data);
        ...
        address = baseAddr | (2 << 24) | (offset & 0x00ffffff);
        *(volatile GT_U32 *)(address) = DIAG_BYTE_SWAP_MAC(doByteSwap, regData);
        address = baseAddr | (2 << 24) | (offset & 0x00ffffff);
        *(volatile GT_U32 *)(address) = DIAG_BYTE_SWAP_MAC(doByteSwap, regData);
        address = baseAddr | (2 << 24) | (offset & 0x00ffffff);
...
)
{
    ...
    if ((searchType != PRV_CPSS_DIAG_REGS_NUM_ONLY_SEARCH_E) &&
        (searchType != PRV_CPSS_DIAG_REGS_ADDR_SEARCH_E) &&
        (searchType != PRV_CPSS_DIAG_REGS_DATA_SEARCH_E))
    {
        return GT_BAD_PARAM;
    }
    ...
    for (i = 0; i < regsListSize; i++)
    {
        ...
        if (regsListPtr[i] == PRV_CPSS_SW_PTR_BOOKMARK_CNS)
        {
            ...
            if (regsListPtr[i+1] & PRV_CPSS_SW_TYPE_WRONLY_CNS)
            {
                /* if the pointer field data is write only skip the pointer,
                   else skip the whole range*/
                i += 1 + ((regsListPtr[i+1] & PRV_CPSS_SW_TYPE_PTR_FIELD_CNS)? 2 :
                           regsListPtr[i+2]);
            }
            else
            {
                ...
                regsInfoSearch (devNum,
                                portGroupId,
                                searchType,
                               (GT_U32*)regsListPtr[i + 3],
                                regsListPtr[i + 2] / sizeof(GT_U32),
                                offset,
                                unitIdAddrPattern,
                                unitIdAddrMask,
                                checkRegExistFuncPtr,
