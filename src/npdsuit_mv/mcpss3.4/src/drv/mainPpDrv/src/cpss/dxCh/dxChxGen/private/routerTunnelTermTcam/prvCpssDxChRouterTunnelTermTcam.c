...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_CH1_E | CPSS_CH1_DIAMOND_E | CPSS_CH3_E | CPSS_XCAT_E | CPSS_LION_E | CPSS_XCAT2_E);
...
)
{
    ...
    rc = prvCpssDxChPortGroupBusyWait(devNum,CPSS_PORT_GROUP_UNAWARE_MODE_CNS,hwAddr,0,GT_FALSE);
    if(rc != GT_OK)
    {
        return rc;
    }
...
* INPUTS:
*       devNum              - the device number
*       lineIndex           - line index in TCAM to write to (0..1023)
*       columnIndex         - column index in TCAM line to write to (0..4)
*
* OUTPUTS:
*       none.
*
...
* INPUTS:
*       devNum              - the device number
*       lineIndex           - line index in TCAM to write to (0..3K-1)
*       columnIndex         - column index in TCAM line to write to (0..3)
*
* OUTPUTS:
*       none.
*
...
)
{
    ...
    GT_U32      tempKeyArr[6];          /* TCAM key in hw format            */
    GT_U32      tempMaskArr[6];         /* TCAM mask in hw format           */
    GT_U32      tempActionArr[6];       /* TCAM action (rule) in hw format  */
...
)
{
    ...
    if (PRV_CPSS_IS_MULTI_PORT_GROUPS_DEVICE_MAC(devNum))
    {
            ...
            ((dsaSrcDevice.pattern & DEV_NUM_MASK_CNS) == devNum))
        {
            ...
            if ((dsaSrcIsTrunk.mask == 0) && ((dsaSrcPortTrunk.mask & PORT_GROUP_ID_MASK_CNS) != 0))
            {
        ...
        if ((srcIsTrunk.mask == 0) && ((srcPortTrunk.mask & PORT_GROUP_ID_MASK_CNS) != 0))
        {
            /* CPSS can't support this option */
            return GT_BAD_PARAM;
        }
        ...
        /* bits 4,5 in mask of <srcPortTrunk> = "don't care" (trunk or port) */
        if ((srcPortTrunk.mask & PORT_GROUP_ID_MASK_CNS) == 0)
        {
    ...
    portGroupIdData.mask        = PORT_GROUP_ID_FROM_GLOBAL_PORT_MAC(srcPortTrunk.mask);
    portGroupIdData.pattern     = PORT_GROUP_ID_FROM_GLOBAL_PORT_MAC(srcPortTrunk.pattern);
    dsaPortGroupIdData.mask     = PORT_GROUP_ID_FROM_GLOBAL_PORT_MAC(dsaSrcPortTrunk.mask);
    dsaPortGroupIdData.pattern  = PORT_GROUP_ID_FROM_GLOBAL_PORT_MAC(dsaSrcPortTrunk.pattern);
...
)
{
    ...
    dsaSrcIsTrunk.mask       = ((hwTtiMaskPerPortGroupArr[5] >>  7) & 0x1);
    dsaSrcIsTrunk.pattern    = ((hwTtiKeyPerPortGroupArr[5]  >>  7) & 0x1);
