...
)
{
    ...
    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC *ipTcamFuncs;
    ...
    if(cpssDxChIpPatTrieSearch(pRootTrie,ipDestPtr,ipPrefix,
                               &oldNumOfRules,&ipNextHopEntry) != NULL)
    {
       ...
       /* if we don't have the same number of routes we need to delete the old
        ...
        {
            retVal = cpssDxChIpTcamDeleteIpUcEntry(vrId,ipDestPtr,ipPrefix,
                                               protocolStack);
            if (retVal != GT_OK)
            {
                return (GT_NOT_FOUND);
            ...
            if (cpssDxChIpTcamMgmCheckAvailbleMem(pIpTcamPool,numOfActualRules)
                != GT_OK)
            {
    ...
    else
    {
        ...
        retVal = prvCpssDxChIpTcamFreeSpaceCountersCheck(prefixNumber,ipMode,mcEntryType,
        ...
        retVal =  prvCpssDxChIpTcamDispatchCheckAvailbleMem(ipTcamShadowPtr,
                                                           entryType,
                                                           1,
                                                           &entryTypeAmount,
                                                            1);
    ...
    {
        typeSize = sizeof(CPSS_DXCH_IP_LTT_ENTRY_STC);
    }
    ...
    {
        switch(nextHopInfoPtr->ipLttEntry.routeType)
        {
        ...
        for(i=0; i <  ipTcamShadowPtr->shadowDevList.ipShareDevNum; i++)
        {
            ...
            {
                if (nextHopInfoPtr->ipLttEntry.numOfPaths > CPSS_DXCH_IP_MAX_ECMP_QOS_GROUP)
                    return GT_BAD_PARAM;
            }
            else
            {
                if(nextHopInfoPtr->ipLttEntry.routeType == CPSS_DXCH_IP_QOS_ROUTE_ENTRY_GROUP_E)
                {
                    if (nextHopInfoPtr->ipLttEntry.numOfPaths > CPSS_DXCH_IP_MAX_LION_QOS_GROUP)
                        return GT_BAD_PARAM;
                }
                else
                {
                    if (nextHopInfoPtr->ipLttEntry.numOfPaths > CPSS_DXCH_IP_MAX_LION_ECMP_GROUP)
                        return GT_BAD_PARAM;
                }
        ...
        switch(nextHopInfoPtr->ipLttEntry.ipv6MCGroupScopeLevel)
        {
        ...
        cpssOsMemCpy(ipNextHopEntry,&nextHopInfoPtr->ipLttEntry,
    ...
    for (i = 0; i < numOfActualRules ; i++)
    {
        ...
        retVal = cpssDxChIpPatTrieInsert(vrId,
                                         pRootTrie,
                                         ipTcamShadowPtr,
                                         entryType,
                                         ipDestPtr,
...
)
{
    ...
    for (i = numOfActualRules -1; i >= 0; i--)
    {
        ...
        retVal = cpssDxChIpPatTrieDel(vrId,
                                      pRootTrie,
                                      ipTcamShadowPtr,
                                      entryType,
                                      ipDestPtr,
...
)
{
    ...
    if (pRootTrie != NULL)
    {
        ...
        {
            retVal = cpssDxChIpPatTrieGetNextPostOrderTrieNode(&nodePtr,&isLastNode);
            if (retVal != GT_OK)
                return retVal;
    ...
    retVal = cpssDxChIpPatTrieFlush(pRootTrie,
                                    ipTcamShadowPtr,
                                    entryType,
                                    ipTcamFuncs,
                                    cpssOsFree,
...
)
{
    ...
    /*IP Tcam Routing Pool Manager header.*/
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *resultNode;
    ...
    resultNode = cpssDxChIpPatTrieSearch(pRootTrie,ipAddrPtr,prefixLength,
                                         NULL,&ipNextHopEntry);
    if(resultNode == NULL)
    {
        return GT_NOT_FOUND;
    ...
    rc = prvCpssDxChIpTcamDispatcherRuleSet(resultNode,
                                            ipTcamShadowPtr,
                                            &tcamRulePtr);
    if (rc != GT_OK)
    {
...
)
{
    ...
    resultNode = cpssDxChIpPatTrieSearch(pRootTrie,ipAddrPtr,prefix,
                                         NULL,&ipNextHopEntry);
    if(resultNode == NULL)
    {
        return GT_NOT_FOUND;
    ...
    {
        cpssOsMemCpy(&nextHopInfoPtr->ipLttEntry,ipNextHopEntry,
                     sizeof(CPSS_DXCH_IP_LTT_ENTRY_STC));
    }
...
{
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *pRootTrie,*resultNode;         /* Pointer the 1st level bucket to which*/
    ...
    resultNode = cpssDxChIpPatTrieGetNext(pRootTrie,ipAddrPtr,&prefixlen,
                                          &ipNextHopEntry);
    ...
    *prefixPtr = prefixlen;
    if((resultNode != NULL) && (nextHopInfoPtr != NULL))
    {
        ...
        {
            /* get hsu infor for the node */
            retVal = prvCpssDxChIpTcamDispatchGetEntryHsuInfo(ipTcamShadowPtr,
                                                              entryType,
                                                              resultNode,
                                                              &ruleIdx);
            if(retVal != GT_OK)
