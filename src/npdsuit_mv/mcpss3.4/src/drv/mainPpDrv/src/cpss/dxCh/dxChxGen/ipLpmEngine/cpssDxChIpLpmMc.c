...
)
{
    ...
    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC    *ipMcTcamFuncs;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC              *pRootTrie;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC              *pGroupNode,*pSrcNode;
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC     *mcGroupNode,*mcSrcNode;
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC     *mcDefGroupNode;
    CPSS_DXCH_IP_LTT_ENTRY_STC                      *lclIpLttEntryPtr = NULL;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC              *pSrcTrie = NULL;
    ...
    PRV_CPSS_DXCH_IP_TCAM_RULE_NODE_STC             *zeroRule = NULL;

    /* roll back params */
    GT_BOOL                                         isSrcTrieBeenCreated = GT_FALSE;
    GT_VOID                                         *rollBackFreeArray[4];
    GT_U32                                          rollBackIndex = 0;
    PRV_CPSS_DXCH_VR_TCAM_TBL_BLOCK_STC             *vrEntryPtr,vrEntryTmp;
    PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_ENTRY_TYPE_ENT grEntryType;
    PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_ENTRY_TYPE_ENT srcEntryType;
    ...
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC     *mcGroupNodePtr;
    ...
    for(i=0; i <  ipTcamShadowPtr->shadowDevList.ipShareDevNum; i++)
    {
        ...
        {
            if (ipLttEntryPtr->numOfPaths > CPSS_DXCH_IP_MAX_ECMP_QOS_GROUP)
                return GT_BAD_PARAM;
        }
        else
        {
            if(ipLttEntryPtr->routeType == CPSS_DXCH_IP_QOS_ROUTE_ENTRY_GROUP_E)
            {
                if (ipLttEntryPtr->numOfPaths > CPSS_DXCH_IP_MAX_LION_QOS_GROUP)
                    return GT_BAD_PARAM;
            }
            else
            {
                if (ipLttEntryPtr->numOfPaths > CPSS_DXCH_IP_MAX_LION_ECMP_GROUP)
                    return GT_BAD_PARAM;
            }

        }
        if (ipLttEntryPtr->routeEntryBaseIndex >= PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.routerNextHop)
            return GT_BAD_PARAM;
    }
    ...
    {
        grEntryType =PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_IPV4_MC_GROUP_ENTRY_E;
        srcEntryType =PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_IPV4_MC_SOURCE_ENTRY_E;
        entryTypeAmount.entryType=CPSS_DXCH_TCAM_MANAGER_SINGLE_ENTRY_E;
    }
    else
    {
        grEntryType = PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_IPV6_MC_GROUP_ENTRY_E;
        srcEntryType = PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_IPV6_MC_SOURCE_ENTRY_E;

        if (ipTcamShadowPtr->shadowType == CPSS_DXCH_IP_TCAM_XCAT_POLICY_BASED_ROUTING_SHADOW_E)
        {
            entryTypeAmount.entryType=CPSS_DXCH_TCAM_MANAGER_SINGLE_ENTRY_E;
        }
        else
    ...
    pGroupNode = cpssDxChIpPatTrieSearch(pRootTrie,lclIpGroup,
    ...
    if ((pGroupNode != NULL) && (ipSrcPrefix != 0))
    {
        ...
        if(pSrcTrie != NULL)
        {
            pSrcNode = cpssDxChIpPatTrieSearch(pSrcTrie,ipSrcPtr,
    ...
    ret = prvCpssDxChIpTcamFreeSpaceCountersCheck(prefixNumber,ipMode,mcEntryType,
    ...
    if(pGroupNode == NULL)
    {
        ...
        {
            ret = prvCpssDxChTcamEntriesHandlerSetRealIdxAsAlloc(vrEntryPtr->tcamEntriesHandler,
                                                                 grEntryType,
                                                                 0,
                                                                 &zeroRule);
            if (ret != GT_OK)
        ...
        entryTypeAmount.amount = 1;
        ret =  prvCpssDxChIpTcamDispatchCheckAvailbleMem(ipTcamShadowPtr,
                                                        grEntryType,
                                                        1,
                                                        &entryTypeAmount,
                                                        1);
        ...
        if (ipTcamShadowPtr->shadowType == CPSS_DXCH_IP_TCAM_CHEETAH2_SHADOW_E)
        {
            ...
            if (zeroRule != NULL)
            {
                ret2 = prvCpssDxChTcamEntriesHandlerFree(vrEntryPtr->tcamEntriesHandler,
                                                         grEntryType,
                                                         zeroRule);
                if (ret2 != GT_OK)
                    return ret2;
    ...
    {
        /* create src trie */
        if(cpssDxChIpPatTrieInit(ipTcamShadowPtr,&pSrcTrie) != GT_OK)
        {
            /*osSemSignal(ipTcamShadowPtr->ipUnitSem);*/
            return GT_FAIL;
        }
        /* Make root node of source trie as dummy node */
        cpssOsMemSet(pSrcTrie->ipAddr,0,sizeof(GT_U8) * IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
        pSrcTrie->size = 1;
        isSrcTrieBeenCreated = GT_TRUE;

        /* allocate */
        mcGroupNode = (PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC*)cpssOsMalloc(
        ...
        if (ipSrcPrefix != 0)
        {
            ...
            if (cpssDxChIpPatTrieSearch(pRootTrie,
    ...
    {
        lclIpLttEntryPtr = cpssOsMalloc(sizeof (CPSS_DXCH_IP_LTT_ENTRY_STC));
        if (lclIpLttEntryPtr == NULL)
            return GT_OUT_OF_CPU_MEM;
    }
    ...
    if (ipSrcPrefix != 0)
    {
        ...
        {
            /* allocate */
            mcSrcNode = (PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC*)cpssOsMalloc(
                sizeof(PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC));
            if (mcSrcNode == NULL)
            {
                if (isSrcTrieBeenCreated)
                    cpssDxChIpPatTrieDestroy(ipTcamShadowPtr,pSrcTrie);
         ...
         ((groupFoundHaveAPointer2Def == GT_TRUE) && (ipSrcPrefix == 0))))
    {
        ...
        if (ruleIdxArr != NULL)
        {
            ret = prvCpssDxChIpTcamDispatchSetEntryHsuInfo(ipTcamShadowPtr,
                                                           grEntryType,
                                                           ruleIdxArr[0],
                                                           &tcamRulePtr);
            if (ret != GT_OK)
                return ret;
            ruleNodePtr = (PRV_CPSS_DXCH_IP_TCAM_RULE_NODE_STC *)tcamRulePtr;
        }
        else
        {
            /* for Ch2 device, a group cannot sit in index 0, so if index 0 is available,
        ...
        if (ret == GT_OK)
            ret = cpssDxChIpPatTrieInsert(vrId,
                                          pRootTrie,
                                          ipTcamShadowPtr,
                                          grEntryType,
                                          lclIpGroup,
        ...
        if (ipTcamShadowPtr->shadowType == CPSS_DXCH_IP_TCAM_CHEETAH2_SHADOW_E)
        {
            ...
            {
                ret = prvCpssDxChTcamEntriesHandlerFree(vrEntryPtr->tcamEntriesHandler,
                                                        grEntryType,
                                                        zeroRule);
                if(ret != GT_OK)
                    return ret;
    ...
    if ((overwrite == GT_TRUE) && (ipSrcPrefix == 0))
    {
        ...
        if (cpssOsMemCmp(mcGroupNode,mcDefGroupNode,
                         sizeof(PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC)) == 0)
        {
            pRootTrie = vrEntryPtr->ipMcTrie[protocolStack];
            ...
            while ((cpssDxChIpPatTrieGetNext(pRootTrie,traverseIpGroup,
...
)
{
    GT_STATUS           ret = GT_OK;
    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC *ipMcTcamFuncs;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC     *pRootTrie,*pSrcTrie;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC     *pGroupNode,*pSrcNode;
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC *mcGroupNode,*mcSrcNode;
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC *mcGroupSrcNode,*mcDefGroupNode;
    ...
    mcGroupSrcNode = mcSrcNode = mcGroupNode = mcDefGroupNode = NULL;
    pSrcNode = NULL;
    ...
    if ((ipSrcPrefix == 0) &&
         (mcGroupNode->isAPointer2Def == GT_TRUE) &&
         (cpssDxChIpPatTrieIsEmpty(mcGroupNode->ipMcSrcTrie) == GT_FALSE))
    {
        /*osSemSignal(ipTcamShadowPtr->ipUnitSem);*/
        return (GT_BAD_STATE);
    }
    ...
    else
    {
        ...
        if ((mcGroupNode->isAPointer2Def == GT_TRUE) &&
            (cpssDxChIpPatTrieIsEmpty(mcGroupNode->ipMcSrcTrie) == GT_TRUE))
        {
            deleteEmptyGroup = GT_TRUE;
        }
    }
    ...
    if (deleteEmptyGroup == GT_TRUE)
    {
        ...
        ret = cpssDxChIpPatTrieDestroy(ipTcamShadowPtr, mcGroupNode->ipMcSrcTrie);
        if (ret != GT_OK)
        {
            /*osSemSignal(ipTcamShadowPtr->ipUnitSem);*/
            return(ret);
...
{
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC *mcGroupSrcNode;
...
)
{
    ...
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *pRootTrie;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *pSrcNode;
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC *mcGroupNode,*mcSrcNode;
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC *mcGroupSrcNode,*mcDefGroupNode;
    ...
    PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_ENTRY_TYPE_ENT  grEntryType;
    PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_ENTRY_TYPE_ENT  srcEntryType;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC          *nodePtr = NULL;
    ...
    while ((cpssDxChIpPatTrieGetNext(pRootTrie,ipGroup,
                                     &ipGroupPrefix,
                                     (GT_PTR)&mcGroupNode))
           != NULL)
    {
        ...
        if(mcGroupNode->ipMcSrcTrie != NULL)
        {
            ...
            {
                ret = cpssDxChIpPatTrieGetNextPostOrderTrieNode(&nodePtr,&isLastNode);
                if (ret != GT_OK)
                    return ret;
            ...
            ret = cpssDxChIpPatTrieFlush(mcGroupNode->ipMcSrcTrie,
                                         ipTcamShadowPtr,
                                         srcEntryType,
                                         ipMcTcamFuncs,
    ...
    ret = cpssDxChIpPatTrieFlush(pRootTrie,ipTcamShadowPtr,grEntryType,ipMcTcamFuncs,
                                 prvCpssDxchIpTcamMcGroupSrcNodeStcFreeFunc,
                                 flushDefault);
    if (ret == GT_OK)
    {
...
{
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC               *pRootTrie;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC               *pSrcTrie;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC               *pSrcGroupNode;
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC      *mcNode;
    ...
    pSrcGroupNode = cpssDxChIpPatTrieSearch(pRootTrie,
    ...
    if (pSrcGroupNode == NULL)
        return GT_NOT_FOUND;
    rc = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_CHECK_RULE_VALIDITY_E,
    ...
    rc = prvCpssDxChIpTcamDispatcherRuleSet(pSrcGroupNode,
                                            ipTcamShadowPtr,
                                            &tcamRulePtr);
    if (rc != GT_OK)
    {
    ...
    rc =  prvCpssDxChGetIpTcamRowColumn(ipTcamShadowPtr,
                                        tcamRulePtr,
    ...
    if (ipSrcPrefixLen != 0)
    {
        ...
        if(pSrcTrie != NULL)
        {
            pSrcGroupNode = cpssDxChIpPatTrieSearch(pSrcTrie,
...
)
{
    ...
    if (internalRuleIdxPtr != NULL)
    {
        ...
        rc = prvCpssDxChIpTcamDispatchGetEntryHsuInfo(ipTcamShadowPtr,
                                                      grEntryType,
                                                      pSrcGroupNode,
                                                      &ruleIdx);
        if(rc != GT_OK)
...
)
{
    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC      *ipMcTcamFuncs;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC            *pRootTrie;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC            *pGroupNode;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC            *pSrcNode = NULL;
    PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC   *mcNode;
    ...
    {
        /* search the defualt group */
        pGroupNode = cpssDxChIpPatTrieGetNext(pRootTrie,ipGroupArr,groupPrefixPtr,
                                              (GT_PTR)&mcNode);
    ...
    {
        pGroupNode = cpssDxChIpPatTrieSearch(pRootTrie,ipGroupArr,
                                             *groupPrefixPtr,NULL,
                                             (GT_PTR)&mcNode);
    ...
    while(1)
    {
            ...
            (mcNode->isAPointer2Def == GT_TRUE))
        {
            ...
            {
                /* search next source */
                pSrcNode = cpssDxChIpPatTrieGetNext(mcNode->ipMcSrcTrie,ipSrcArr,
...
)
{
    ...
    mcGroupSrcNode = (PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC*)dataEntry;
...
)
{
    ...
    mcGroupSrcNode = (PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC*)pData;
...
)
{
    ...
    GT_BOOL     isLastNode;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC          *srcTrieRootPtr = NULL;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC          *srcTrieArray = NULL;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC          *nodePtr = NULL;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC          *boundingSonPtr = NULL;
    GT_U32      i,j;
    ...
    GT_PTR                                              tcamRulePtr;
    PRV_CPSS_DXCH_IP_TCAM_MGM_ALLOC_METHOD_ENT          tcamHandlerAllocMethod;
    PRV_CPSS_DXCH_IP_TCAM_SHADOW_STC                    *ipTcamShadowPtr;
    ...
    srcTrieRootPtr = ((PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC*)(trieNodePtr->pData))->ipMcSrcTrie;
    ...
    if ((srcTrieRootPtr != NULL) && (numOfSources != 0))
    {
        ...
        if (protocolStack == CPSS_IP_PROTOCOL_IPV4_E)
        {
            ...
            entryType = PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_IPV4_MC_SOURCE_ENTRY_E;
        }
        else
        {
            requestedEntries.entryType = CPSS_DXCH_TCAM_MANAGER_QUAD_ENTRY_E;
            requestedEntries.amount = numOfSources;
            entryType = PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_IPV6_MC_SOURCE_ENTRY_E;
        }

        ret = prvCpssDxChIpTcamDispatchCheckAvailbleMem(ipTcamShadowPtr,entryType,numOfSources,
                                                        &requestedEntries,1);
        if (ret != GT_OK)
            return ret;
        ...
        srcTrieArray = cpssOsMalloc(numOfSources * sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC));
        if (srcTrieArray == NULL)
            return GT_OUT_OF_CPU_MEM;

        tcamHandlerAllocMethod = PRV_CPSS_DXCH_IP_TCAM_MGM_ALLOC_MIN_E;
        allocatedTcamRulePtr = NULL;
        lowerBound = NULL;
        upperBound = NULL;

        /* get the group index for the new group location */
        ret = prvCpssDxChGetIpTcamOffset(ipTcamShadowPtr,moveToTcamRulePtr,GT_TRUE,&newGroupIndex);
        if (ret != GT_OK)
        {
        ...
        while (isLastNode == GT_FALSE)
        {
            ...
            {
                /* copy current trie */
                cpssOsMemCpy(&srcTrieArray[i],nodePtr,sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC));

                /* allocate token info */
                PRV_CPSS_DXCH_ALLOCATE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,&srcTrieArray[i]);
                ...
                ret = prvCpssDxChIpTcamDispatchAllocate(ipTcamShadowPtr,entryType,
                ...
                if (ret != GT_OK)
                {
                    ...
                    for (j = 0 ; j < i ; j++)
                    {
                        ret2 = prvCpssDxChIpTcamDispatchFree(ipTcamShadowPtr,entryType,&srcTrieArray[j]);
                        if (ret2 != GT_OK)
                        {
                            /* failure during recovery; unstable state */
                            return ret2;
                        }
                        PRV_CPSS_DXCH_FREE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,&srcTrieArray[j]);
                    }
                    /* for last trie node, just free toekn info as TCAM location allocation failed */
                    PRV_CPSS_DXCH_FREE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,&srcTrieArray[i]);
                    cpssOsFree(srcTrieArray);
                    return ret;
                }
                ...
                ((PRV_CPSS_DXCH_IP_TCAM_MC_GROUP_SRC_NODE_STC*)(srcTrieArray[i].pData))->gIndex = (GT_U16)newGroupIndex;
                i++;
            }
        }
        ...
        for (i = 0 ; i < numOfSources ; i++)
        {
            ...
            ret = cpssDxChIpPatTrieGetAddrFromNode(&srcTrieArray[i],ipAddr,&prefixLen);
            if (ret != GT_OK)
            {
                /* this function can't fail */
                return ret;
            ...
            ret = prvCpssDxChIpTcamDispatcherRuleSet(&srcTrieArray[i],ipTcamShadowPtr,&tcamRulePtr);
            if (ret != GT_OK)
            {
                /* this function can't fail */
                return ret;
            ...
            if (ret != GT_OK)
            {
                for (j = 0 ; j < numOfSources ; j++)
                {
                    ...
                    if (j < i)
                    {
                        ret2 = prvCpssDxChIpTcamDispatcherRuleSet(&srcTrieArray[i],ipTcamShadowPtr,&tcamRulePtr);
                        if (ret2 != GT_OK)
                        {
                            /* failure during recovery; unstable state */
                            return ret2;
                        }
                        ret2 = cpssDxChIpTcamTrieDelPrefixCheetah2(vrId,
                                                                  tcamRulePtr,
                                                                  ipTcamShadowPtr,
                                                                  srcTrieArray[i].pData,
                                                                  protocolStack,
    ...
    if (ret != GT_OK)
    {
        ...
        if ((srcTrieRootPtr != NULL) && (numOfSources != 0))
        {
            for (j = 0 ; j < numOfSources ; j++)
            {
                ...
                ret2 = prvCpssDxChIpTcamDispatcherRuleSet(&srcTrieArray[j],ipTcamShadowPtr,&tcamRulePtr);
                if (ret2 != GT_OK)
                {
                    /* failure during recovery; unstable state */
                    return ret2;
    ...
    if ((srcTrieRootPtr != NULL) && (numOfSources != 0))
    {
        ...
        while (isLastNode == GT_FALSE)
        {
            ...
            {
                /* delete the old rule */
                ret = prvCpssDxChIpTcamDispatcherRuleSet(nodePtr,ipTcamShadowPtr,&tcamRulePtr);
                if (ret != GT_OK)
                {
                    /* this function can't fail */
                    return ret;
                ...
                ret = prvCpssDxChIpTcamDispatchFree(ipTcamShadowPtr,entryType,nodePtr);
                if (ret != GT_OK)
                {
                    /* failure during clean up of old entries; TCAM has multiple
                       entries for same prefix; unstable state */
                ...
                PRV_CPSS_DXCH_FREE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,nodePtr);
                /* note that new TCAM location was allocated with nodePtr as the
                   cookie, so there is no need to update cookie */
                cpssOsMemCpy(nodePtr,&srcTrieArray[i],sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC));
                i++;
            }
            ...
            {
                /* update bounding son */
                ret = cpssDxChIpPatTrieGetBoundingSon(nodePtr,ipTcamShadowPtr,&boundingSonPtr);
                if (ret != GT_OK)
                {
                    /* failure during clean up of old entries; TCAM has multiple
                       entries for same prefix; unstable state */
