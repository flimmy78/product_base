...
********************************************************************************
* prvCpssDxChHwMultiPortGroups.c
*
* DESCRIPTION:
...
)
{
    ...
    if(regValue == 1)
    {
        /* the case of (onlyCheck == GT_FALSE) would have already returned GT_TIMEOUT
            from the macro of PRV_CPSS_DXCH_MAX_NUM_ITERATIONS_CHECK_CNS */
        return GT_BAD_STATE;
    }
...
)
{
    ...
    if(GT_TRUE  == prvCpssDxChDuplicatedMultiPortGroupsGet(devNum, regAddr,
                                                    &portGroupsBmpValue))
    {
...
)
{
    ...
    return prvCpssDxChPortGroupsBmpCounterSummary(devNum,
...
{
    return prvCpssDxChPortGroupsBmpMultiCounterSummary(
                                        devNum,
                                        CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
                                        regAddr,
                                        fieldOffset1,
...
{
    return prvCpssDxChPortGroupsBmpCounterSet(devNum,
                                              CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
                                              regAddr, fieldOffset,
...
{
    return prvCpssDxChPortGroupsBmpCounterArraySet(devNum,
                                                   CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
                                                   startAddr,
                                                   numberOfWords,
                                                   counterValueArr);
...
#ifdef _WIN32
    #define MULTI_PORT_GROUPS_DEBUG_INFO
#endif /*_WIN32*/

#ifdef  MULTI_PORT_GROUPS_DEBUG_INFO
/*******************************************************************************
* prvCpssDxChPortGroupsDebugInfoPrint
*
* DESCRIPTION:
*       print the port groups debug info , in PRV_CPSS_DXCH_PP_PORT_GROUPS_DEBUG_INFO_STC
*
* APPLICABLE DEVICES:
*        DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; Lion; xCat2.
*
...
*
*******************************************************************************/
GT_STATUS prvCpssDxChPortGroupsDebugInfoPrint
(
    IN GT_U8 devNum
)
{
    GT_U32  ii,jj;
    PRV_CPSS_DXCH_PP_PORT_GROUPS_DEBUG_INFO_STC *debugInfoPtr;
    PRV_CPSS_DXCH_PP_PORT_GROUPS_DEBUG_UPLINK_INFO_STC *uplinkInfoPtr;

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);

    debugInfoPtr = PRV_CPSS_DXCH_PP_PORT_GROUPS_DEBUG_INFO_PTR_MAC(devNum);

    if(debugInfoPtr->debugInfoValid == GT_FALSE)
    {
        cpssOsPrintf("debugInfoValid = GT_FALSE \n");
        return GT_OK;
    }

    cpssOsPrintf("numOfUplinkPorts [%d] \n",
                debugInfoPtr->numOfUplinkPorts);

    for(ii = 0;ii < debugInfoPtr->numOfUplinkPorts ;ii++)
    {
        uplinkInfoPtr = &debugInfoPtr->uplinkInfoArr[ii];

        cpssOsPrintf("uplink [%d] info \n",ii);

        cpssOsPrintf("startPortGroupId [%d] \n",
                    uplinkInfoPtr->startPortGroupId);

        cpssOsPrintf("uplinkPortGroupPortInfo.portGroupId [%d] \n",
                    uplinkInfoPtr->uplinkPortGroupPortInfo.portGroupId);

        cpssOsPrintf("uplinkPortGroupPortInfo.localPort [%d] \n",
                    uplinkInfoPtr->uplinkPortGroupPortInfo.localPort);

        cpssOsPrintf("numberOfInternalConnections [%d] \n",
                    uplinkInfoPtr->numberOfInternalConnections);

        for(jj = 0 ; jj < uplinkInfoPtr->numberOfInternalConnections ; jj++)
        {
            cpssOsPrintf("\t internalConnectionsArr[%d].portGroupId [%d] \n",jj,
                        uplinkInfoPtr->internalConnectionsArr[jj].portGroupId);

            cpssOsPrintf("\t internalConnectionsArr[%d].localPort [%d] \n",jj,
                        uplinkInfoPtr->internalConnectionsArr[jj].localPort);
        }
    }

    for(ii = 0;ii < (CPSS_MAX_PORT_GROUPS_CNS - 1);ii++)
    {
        cpssOsPrintf("unknownUnregDesignatedDeviceArray[%d] = [%d] \n",ii,
                    debugInfoPtr->unknownUnregDesignatedDeviceArray[ii]);
    }

    cpssOsPrintf("unknownUnregDesignatedDeviceIndex [%d] \n",
                debugInfoPtr->unknownUnregDesignatedDeviceIndex);

    if(debugInfoPtr->internalTrunksPtr)
    {
        for(ii = 0;ii < CPSS_MAX_PORT_GROUPS_CNS ;ii++)
        {
            cpssOsPrintf("internalTrunksPtr[%d] = [%2.2x] \n",ii,
                        debugInfoPtr->internalTrunksPtr[ii]);
        }
    }
    else
    {
        cpssOsPrintf("no internal trunks used \n");
    }

    cpssOsPrintf("numOfCpuCodes [%d] \n",
                debugInfoPtr->numOfCpuCodes);


    for(ii = 0;ii < debugInfoPtr->numOfCpuCodes ;ii++)
    {
        cpssOsPrintf("unknownUnregCpuCodesPtr[%d] = [%d] \n",ii,
                    debugInfoPtr->unknownUnregCpuCodesPtr[ii]);
    }

    cpssOsPrintf("lastPortGroupBmp [%d] \n",
                debugInfoPtr->lastPortGroupBmp);

    cpssOsPrintf("srcIdFilterFloodToUplinks [%d] \n",
                debugInfoPtr->srcIdFilterFloodToUplinks);


    return GT_OK;
}
#endif /*MULTI_PORT_GROUPS_DEBUG_INFO*/
