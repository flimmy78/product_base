...
)
{
    ...
    {
        poolArrPtr = (GT_ETH_PORT_POOL_HEAD*)
            osMalloc(sizeof (*poolArrPtr) * GT_ETH_PORT_NUM_POOLS_CNS);
    ...
    if ((ALIGN_4B_MASK_CNS & (GT_U32)poolPtr) != 0)
    {
        /* align the memory pool block */
        tmp_PTR = poolPtr;
        poolPtr = (GT_U8*)((GT_U32)(poolPtr + ALIGN_4B_MASK_CNS) &
                            (~ALIGN_4B_MASK_CNS));
        neededPoolSize -= poolPtr - tmp_PTR;
    }
