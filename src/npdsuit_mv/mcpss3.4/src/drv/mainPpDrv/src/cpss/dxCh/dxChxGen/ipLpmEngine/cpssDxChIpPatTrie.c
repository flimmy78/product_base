...
{
    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_DEL_PREFIX_FUN           delPrefixFunc;
...
)
{
    ...
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *boundTrieNodePtr = NULL;

    if ((trieNodePtr->leftSon == NULL) && (trieNodePtr->rightSon != NULL))
    {
        PRV_CPSS_DXCH_SET_BOUNDING_SON_MAC(ipTcamShadowPtr->tcamManagementMode,
    ...
    {
        ret = prvCpssDxChIpTcamDispatcherRuleSet(trieNodePtr->leftSon,
                                                 ipTcamShadowPtr,
                                                 &leftSonRulePtr);
        if (ret != GT_OK)
        {
        ...
        ret = prvCpssDxChIpTcamDispatcherRuleSet(trieNodePtr->rightSon,
                                                 ipTcamShadowPtr,
                                                 &rightSonRulePtr);
        if (ret != GT_OK)
        {
        ...
        ret = prvCpssDxChIpTcamDispatcherCompRulesPos(ipTcamShadowPtr,
                                                      leftSonRulePtr,
                                                      rightSonRulePtr,
                                                      &result);
        if (ret != GT_OK)
...
*       indicateUpdateChildren - in the case of bulk insert, weather to
...
{
    GT_STATUS ret = GT_OK;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *boundingSon  = NULL;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *currTrieNode = trieNodePtr;

    /* look out for the highest position rule of both sons*/
    while ((currTrieNode != NULL) && (currTrieNode->pData == NULL))
    {
        /* retrive bounding son */
        ret= cpssDxChIpPatTrieGetBoundingSon(currTrieNode,ipTcamShadowPtr,&boundingSon);
        if (ret != GT_OK)
        {
            return ret;
        }
        ...
        ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_RULE_COPY_E,
        ...
        if (indicateUpdateChildren == GT_TRUE)
            currTrieNode->updateStatus |= PRV_PAT_TRIE_NODE_UPDATE_CHILDREN_E;
...
*
*******************************************************************************/
static PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *getValidFather
(
    IN PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *trieNodePtr
)
{
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *currTrieNode;
...
(
    IN  PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *trieNodePtr,
    IN  PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *oldTrieNodePtr
)
{
    PRV_CPSS_DXCH_IP_PAT_TRIE_OLD_NODE_REC_STC *oldNodeStc;
    GT_BOOL                             nodeAlreadyOverwriting;

    nodeAlreadyOverwriting =
        ((trieNodePtr->updateStatus & PRV_PAT_TRIE_NODE_INVALIDATE_FIRST_E) > 0)?
        GT_TRUE: GT_FALSE;

    /* since this node will take place of an old existing node we need to
       invalidate the space first */
    trieNodePtr->updateStatus |= PRV_PAT_TRIE_NODE_INVALIDATE_FIRST_E;

    /* since this node will take place of an old existing node
       record which one is it */
    if (nodeAlreadyOverwriting == GT_FALSE)
    {
        oldNodeStc = cpssOsMalloc(sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_OLD_NODE_REC_STC));
        if (oldNodeStc == NULL)
            return GT_FAIL;
    ...
    {
        oldNodeStc = (PRV_CPSS_DXCH_IP_PAT_TRIE_OLD_NODE_REC_STC*)trieNodePtr->pData;
        oldNodeStc->oldNode = oldTrieNodePtr;
    }
...
*       isInvalidateNeededPtr - to indicate the higher level weather the place it
...
(
    ...
    IN  PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_ENTRY_TYPE_ENT entryType,
    IN  PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                *trieNodePtr,
    IN  PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                *originalTrieNodePtr,
    ...
    OUT PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                **oldNodePtrPtr
)
{
    GT_STATUS ret  = GT_OK;
    ...
    GT_32 i;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC **sonsStack;/* stack to record the passed sons*/
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *currValidFather,*currTrieNode = NULL;
    GT_BOOL invalidateFirst = GT_FALSE;
    GT_BOOL hadInvlidateFirst = GT_FALSE;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC  *oldNode = NULL, *preOldNode = NULL;
    GT_PTR moveFromTcamRulePtr = NULL;
    GT_PTR moveToTcamRulePtr = NULL;
    GT_PTR alocatedTcamRulePtr = NULL;
    PRV_CPSS_DXCH_IP_TCAM_RULE_NODE_STC *allocTempPtr = NULL;
    GT_PTR freeRule = NULL;

    sonsStack = cpssOsMalloc(sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *)*128);/* at most 128 sons*/
    if (sonsStack == NULL)
    {
        return GT_FAIL;
    }
    ...
    {
        currValidFather = getValidFather(currTrieNode);
        if (currValidFather)
        {
            ret = prvCpssDxChIpTcamDispatcherRuleSet(currValidFather,
                                                     ipTcamShadowPtr,
                                                     &upperRulePtr);
            if (ret != GT_OK)
            {
        ...
        ret = prvCpssDxChIpTcamDispatcherRuleSet(currTrieNode,
                                                 ipTcamShadowPtr,
                                                 &lowerRulePtr);
        if (ret != GT_OK)
        {
        ...
        ret =  prvCpssDxChIpTcamDispatchAllocate(ipTcamShadowPtr,
                                                entryType,
                                                lowerRulePtr,
                                                upperRulePtr,
    ...
    if (ret == GT_OK)
    {
        ...
        {
            ret = prvCpssDxChIpTcamDispatcherRuleSet(sonsStack[i],
                                                     ipTcamShadowPtr,
                                                     &freeRule);
            if (ret != GT_OK)
            {
            ...
            {
                /* record weather this node used to have an invalidate first
                   flag */

                if ((sonsStack[i]->updateStatus & PRV_PAT_TRIE_NODE_INVALIDATE_FIRST_E) == 0 )
                {
                ...
                else
                {
                    ...
                    preOldNode = ((PRV_CPSS_DXCH_IP_PAT_TRIE_OLD_NODE_REC_STC*)(sonsStack[i]->pData))->oldNode;
                }
                ...
                {
                    sonsStack[i]->updateStatus &= ~PRV_PAT_TRIE_NODE_INVALIDATE_FIRST_E;
                ...
                if ((sonsStack[i]->updateStatus & PRV_PAT_TRIE_NODE_UPDATE_HW_E) == 0)
                {
                ...
                sonsStack[i]->updateStatus |= PRV_PAT_TRIE_NODE_UPDATE_HW_E;
            }
            if (ret != GT_OK)
            {
                cpssOsFree(sonsStack);
            ...
            {
                ret = prvCpssDxChTcamEntriesHandlerFree(ipTcamShadowPtr->tcamEntriesHandler,
                                                        entryType,
            ...
            ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_ALLOCATED_RULE_AND_TRIE_UPDATE_E,
                                                            ipTcamShadowPtr->tcamManagementMode,
                                                            NULL,
                                                            sonsStack[i],
                                                            alocatedTcamRulePtr);
                    ...
                    PRV_CPSS_DXCH_IP_TCAM_MANAGEMENT_TCAM_ENTRIES_HANDLER_MODE_E)
            {
                ...
                ret = prvCpssDxChTcamEntriesHandlerAllocByGivenRule(ipTcamShadowPtr->tcamEntriesHandler,
                                                                    entryType,
                                                                    (PRV_CPSS_DXCH_IP_TCAM_RULE_NODE_STC*)freeRule,
                                                                    &allocTempPtr);
            ...
            else
            {
                ...
                ret = cpssDxChTcamManagerEntryClientCookieUpdate(ipTcamShadowPtr->tcamManagerHandlerPtr,
                                                                 ...
                                                                 sonsStack[i]->tcamAllocInfo.tokenNode->tcamManagerEntryToken,
                                                                 sonsStack[i]);
            }
            if (ret != GT_OK)
            {
                        ...
                        PRV_CPSS_DXCH_IP_TCAM_MANAGEMENT_TCAM_MANAGER_MODE_E)
    {
        if( (trieNodePtr->pData == NULL)                 &&
            ( (trieNodePtr->leftSon != NULL &&
               trieNodePtr->leftSon->tcamAllocInfo.tokenNode->tcamManagerEntryToken == NON_INITIALIZED_TOKEN_CNS)   ||
              (trieNodePtr->rightSon != NULL &&
               trieNodePtr->rightSon->tcamAllocInfo.tokenNode->tcamManagerEntryToken == NON_INITIALIZED_TOKEN_CNS) )  )
        {
...
(
    ...
    IN  PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_ENTRY_TYPE_ENT    entryType,
    IN  PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                  *trieNodePtr,
    IN  PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                  *trieDestNodePtr,
    IN  PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC        *ipTcamFuncsPtr,
    IN  GT_BOOL                                             isBulkInsert,
    IN  GT_BOOL                                             defragEnable,
    OUT GT_PTR                                              *allocNodePtrPtr,
...
)
{
    ...
    while (currTrieNode != NULL)
    {
        ...
        {
            ret = prvCpssDxChIpTcamDispatcherRuleSet(boundingSon,
                                                     ipTcamShadowPtr,
                                                     &lowerIdx);
            if (ret != GT_OK)
            {
    ...
    if (ret == GT_OK)
    {
        ...
        while (currTrieNode != NULL)
        {
            ...
            else
            {
                ...
                if ((currTrieNode->updateStatus & PRV_PAT_TRIE_NODE_INVALIDATE_FIRST_E) == 0 )
                {
                    hadInvlidateFirst = GT_FALSE;
                }
                else
                {
                    hadInvlidateFirst = GT_TRUE;
                    preOldNode = ((PRV_CPSS_DXCH_IP_PAT_TRIE_OLD_NODE_REC_STC*)(currTrieNode->pData))->oldNode;
                }
                ...
                {
                    currTrieNode->updateStatus &= ~PRV_PAT_TRIE_NODE_INVALIDATE_FIRST_E;
                ...
                if ((currTrieNode->updateStatus & PRV_PAT_TRIE_NODE_UPDATE_HW_E) == 0)
                {
                    invalidateFirst = GT_TRUE;
                ...
                currTrieNode->updateStatus |= PRV_PAT_TRIE_NODE_UPDATE_HW_E;
                ret = GT_OK;
            }
            if (ret != GT_OK)
                break;
            ...
            else
            {
                ...
                ret = cpssDxChTcamManagerEntryClientCookieUpdate(ipTcamShadowPtr->tcamManagerHandlerPtr,
                                                                 ipTcamShadowPtr->clientId,
                                                                 currTrieNode->tcamAllocInfo.tokenNode->tcamManagerEntryToken,
                                                                 (GT_PTR)currTrieNode );
            }
            ...
            currTrieNode = getValidFather(currTrieNode);
        }
...
(
    ...
    IN    PRV_CPSS_DXCH_IP_TCAM_SHADOW_STC                  *ipTcamShadowPtr,
    IN    PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_ENTRY_TYPE_ENT entryType,
    IN    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                *boundingSonPtr,
    IN    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                *validFatherPtr,
    IN    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC      *ipTcamFuncsPtr,
    IN    GT_BOOL                                           isBulkInsert,
    IN    GT_BOOL                                           isMoveOnly,
    IN    GT_BOOL                                           defragEnable,
    OUT   PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                *trieNodePtr,
    OUT   GT_BOOL                                           *isInvalidateNeededPtr,
    OUT   PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC                **oldNodePtrPtr
)
{
...
(
    ...
    IN PRV_CPSS_DXCH_IP_TCAM_SHADOW_STC                 *ipTcamShadowPtr,
    IN PRV_CPSS_DXCH_TCAM_ENTRIES_HANDLER_ENTRY_TYPE_ENT entryType,
    INOUT PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC            *trieNodePtr,
    IN    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC            *boundingSonPtr,
...
{
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *validFather;
    GT_PTR upperIdx = NULL;
    GT_PTR lowerIdx = NULL;
    GT_PTR alocatedTcamRulePtr = NULL;
    ...
    validFather = getValidFather(trieNodePtr);
    if (validFather != NULL)
    {
        ret = prvCpssDxChIpTcamDispatcherRuleSet(validFather,
                                                 ipTcamShadowPtr,
                                                 &upperIdx);
        if (ret != GT_OK)
        {
    ...
    if (boundingSonPtr != NULL)
    {
        ret = prvCpssDxChIpTcamDispatcherRuleSet(boundingSonPtr,
                                                 ipTcamShadowPtr,
                                                 &lowerIdx);
        if (ret != GT_OK)
        {
    ...
    if(isMoveOnly == GT_FALSE)
    {
        ...
        if (ruleNodePtr == NULL)
        {
            ...
            if (ret == GT_OK)
            {
                ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_ALLOCATED_RULE_UPDATE_E,
                                                                ipTcamShadowPtr->tcamManagementMode,
                                                                NULL,
                                                                trieNodePtr,
                                                                alocatedTcamRulePtr);
        ...
        else
        {
            ...
            {
                trieNodePtr->tcamAllocInfo.tokenNode->tcamManagerEntryToken = (GT_U32)ruleNodePtr;
                ret = cpssDxChTcamManagerEntryClientCookieUpdate(ipTcamShadowPtr->tcamManagerHandlerPtr,
                                                                 ipTcamShadowPtr->clientId,
                                                                 trieNodePtr->tcamAllocInfo.tokenNode->tcamManagerEntryToken,
                                                                 (GT_PTR)trieNodePtr);
                    if (ret != GT_OK)
                    {
                        /* fatal error ! */
...
)
{
    ...
    GT_U8  keySuffix[IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES];
    GT_U8  currPrefix[IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES];
    GT_U32 keySuffixLen =0,currPrefixLen;
    GT_U8  rootSuffix[IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES];
    GT_U32 rootSuffixLen;
    GT_BOOL isEcmpPrefixSearch = GT_FALSE;
    GT_BOOL invalidatePrefix = GT_FALSE;
    OUT PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC  *oldNodePtr;

    GT_STATUS ret = GT_OK;

    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *newRootTrie,*newKeyTrie;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *validFather,*boundingSon;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC **contTrie;
    PRV_CPSS_DXCH_IP_TCAM_MGM_ALLOC_METHOD_ENT allocMethod =
        PRV_CPSS_DXCH_IP_TCAM_MGM_ALLOC_MID_E;
    ...
    if (rootPtr->size == 0)
    {
        ...
        rootPtr->pData = entry;
        PRV_CPSS_DXCH_ALLOCATE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,
                                              rootPtr);
        rootPtr->prefixFlagType = PRV_PAT_TRIE_PREFIX_FLAG_ADDR;
        rootPtr->vrId = vrId;
        ...
        ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_RULE_UPDATE_TRIE_E,
                                                           ipTcamShadowPtr->tcamManagementMode,
                                                           rootPtr,
                                                           rootPtr,
                                                           NULL);
        ...
        else
        {
            rootPtr->updateStatus |=
                PRV_PAT_TRIE_NODE_UPDATE_HW_E | PRV_PAT_TRIE_NODE_NEW_INSERT_E;
        }
        return ret;
    }
    ...
    cpssOsMemCpy(currPrefix,ipAddrPtr,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
    ...
    while (currTrieNode!= NULL)
    {
        ...
        cpssOsMemSet(rootSuffix,0,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
        cpssOsMemSet(keySuffix,0,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);

        i =0;
        if ((isEcmpPrefixSearch == GT_FALSE) &&
            (currTrieNode->prefixFlagType == PRV_PAT_TRIE_PREFIX_FLAG_ECMP))
        {
                 ...
                 (currTrieNode->prefixFlagType == PRV_PAT_TRIE_PREFIX_FLAG_ECMP))
        {
            ...
            copyIpAddr(keySuffix,currPrefix,i,currPrefixLen,0);
        ...
        if (i < currTrieNode->prefixLen) /* Split node*/
        {
            ...
            copyIpAddr(rootSuffix,currTrieNode->ipAddr,i,currTrieNode->prefixLen,0);
            rootSuffixLen = currTrieNode->prefixLen - i;

            /* create a new rootPtr node */
            newRootTrie = (PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *)cpssOsMalloc(
                sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC));
            if (newRootTrie == NULL)
            {
                return GT_OUT_OF_CPU_MEM;
            }
            ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_INVALID_RULE_E,
                                                            ipTcamShadowPtr->tcamManagementMode,
                                                            NULL,
                                                            newRootTrie,
                                                            NULL);
            ...
            cpssOsMemCpy(newRootTrie->ipAddr,currTrieNode->ipAddr,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
            newRootTrie->prefixLen = i;
            newRootTrie->prefixFlagType = PRV_PAT_TRIE_PREFIX_FLAG_ADDR;
            newRootTrie->size = currTrieNode->size;
            newRootTrie->father = currTrieNode->father;
            newRootTrie->updateStatus = PRV_PAT_TRIE_NODE_UPDATE_STATUS_OK_E;
            ...
            if (i < currPrefixLen)
            {
                ...
                newKeyTrie = (PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *)cpssOsMalloc(
                    sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC));
                if (newKeyTrie == NULL)
                {
                    return GT_OUT_OF_CPU_MEM;
                ...
                newKeyTrie->size = 1;
                cpssOsMemCpy(newKeyTrie->ipAddr,keySuffix,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
                ...
                PRV_CPSS_DXCH_ALLOCATE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,
                                                      newKeyTrie);
                ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_INIT_TOKEN_RULE_INFO_E,
                                                                ipTcamShadowPtr->tcamManagementMode,
                                                                NULL,
                                                                newKeyTrie,
                                                                NULL);
                ...
                newKeyTrie->prefixFlagType = (GT_U8)PRV_PAT_TRIE_PREFIX_FLAG_ADDR;
                ...
                if (ret == GT_OK)
                {
                    ...
                    {
                        dbgTcamWrites++;
                        DBG_PRINT(("Adding prefix rule = %d\n", newKeyTrie->tcamAllocInfo.ruleNode->offset));
                        ret = prvCpssDxChIpTcamDispatcherRuleSet( newKeyTrie,
                                                                  ipTcamShadowPtr,
                                                                  &tcamRulePtr);
                        if (ret != GT_OK)
                        {
                        ...
                        ret = ipTcamFuncsPtr->ipTcamSetPrefix(vrId,
                                                              ...
                                                              entry,
                                                              *ipTcamFuncsPtr->workDevListPtr);
                        newKeyTrie->updateStatus = PRV_PAT_TRIE_NODE_UPDATE_STATUS_OK_E;
                    }
                    else
                    {
                ...
                if (ret != GT_OK)
                {
                    ...
                    PRV_CPSS_DXCH_FREE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,
                                                       newKeyTrie);
                ...
                if (isBulkInsert == GT_TRUE)
                    ...
                    newRootTrie->updateStatus = PRV_PAT_TRIE_NODE_UPDATE_CHILDREN_E;
            }
            else
            {
                ...
                if ((isBulkInsert == GT_TRUE) &&
                    (currTrieNode->updateStatus != PRV_PAT_TRIE_NODE_UPDATE_STATUS_OK_E))
                ...
                if (ret == GT_OK)
                {
                    ...
                    {
                        dbgTcamWrites++;
                        DBG_PRINT(("Adding rule = %d\n", newRootTrie->tcamAllocInfo.ruleNode->offset));
                        ret = prvCpssDxChIpTcamDispatcherRuleSet( newRootTrie,
                                                                  ipTcamShadowPtr,
                                                                  &tcamRulePtr);
                        if (ret != GT_OK)
                        {
            ...
            cpssOsMemCpy(currTrieNode->ipAddr,rootSuffix,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
        ...
        else
        {
                ...
                ((isEcmpPrefixSearch == GT_TRUE) && (i == currPrefixLen)))
            {
                if (currTrieNode->pData != NULL)
                {
                    if (isBulkInsert == GT_FALSE)
                    {
                        ...
                        DBG_PRINT(("Overriding rule to root = %d\n", rootPtr->tcamAllocInfo.ruleNode->offset));
                        ret = prvCpssDxChIpTcamDispatcherRuleSet( currTrieNode,
                                                                  ipTcamShadowPtr,
                                                                  &tcamRulePtr);
                        if (ret != GT_OK)
            ...
            if (isEcmpPrefixSearch == GT_FALSE)
            {
                ...
                while ((currTrieNode->rightSon != NULL) &&
                       (currTrieNode->rightSon->prefixFlagType == PRV_PAT_TRIE_PREFIX_FLAG_ECMP))
                {
            ...
            if ((*contTrie == NULL) ||
                ...
                ((*contTrie)->prefixFlagType != PRV_PAT_TRIE_PREFIX_FLAG_ECMP)))
            {
                ...
                cpssOsMemCpy(newKeyTrie->ipAddr,keySuffix,
                             IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
                newKeyTrie->prefixLen = keySuffixLen;
                newKeyTrie->vrId = vrId;
                newKeyTrie->entryType = entryType;
                newKeyTrie->ipTcamFuncsPtr = ipTcamFuncsPtr;
                ...
                newKeyTrie->prefixFlagType = (GT_U8)((isEcmpPrefixSearch == GT_TRUE) ?
                    PRV_PAT_TRIE_PREFIX_FLAG_ECMP : PRV_PAT_TRIE_PREFIX_FLAG_ADDR);
        ...
        cpssOsMemCpy(currPrefix,keySuffix,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
...
)
{
    ...
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *currTrieNode,*resultTrieNode;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC **contTrie;
...
)
{
    ...
    while (currTrieNode != NULL)
    {
        ...
        copyIpAddr(currPrefix,currTrieNode->ipAddr,0,currTrieNode->prefixLen,
                   128-currPrefixLen);
        currTrieNode = currTrieNode->father;
    ...
    copyIpAddr(ipAddrPtr,currPrefix,128-currPrefixLen,128,0);
...
)
{
    ...
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *currTrieNode,*otherFather= NULL;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *exactMatchNode = NULL;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC **contTrie;
    ...
    if (currTrieNode != NULL)
    {
        ...
        rc = cpssDxChIpPatTrieGetAddrFromNode(currTrieNode,ipAddrPtr,prefixLenPtr);
        if (rc != GT_OK)
        {
            return NULL;
        }
...
*       vrId                - the virtual router id.
*       rootPtr             - Root of the trie to search for the entry.
*       activateOnValidOnly - weather to preform the given functions on valid
...
*                             on.
*       preformOnRoot       - weather to preform the actions on the root node.
...
(
    ...
    IN PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_TRAVERSE_FUN      downTraverseFunc,
    IN PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_TRAVERSE_FUN      upTraverseFunc,
    IN PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_TRAVERSE_FUN      invalidateTraverseFunc,
    IN GT_PTR                                           extraData,
...
)
{
    ...
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *currTrieNode, *currParentTrieNode;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *nextTrieNode;
    ...
    cpssOsMemSet(currPrefix,0,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
    currPrefixLen = 0;
    ...
    while (currTrieNode != NULL)
    {
        ...
        if (isUpdateOnly == GT_TRUE)
        {
            ...
            {
                invalidateFirst =
                    ((currTrieNode->updateStatus & PRV_PAT_TRIE_NODE_INVALIDATE_FIRST_E) > 0)?
                    GT_TRUE:GT_FALSE;
                if (invalidateFirst == GT_TRUE)
                {
                    oldNodeStc = (PRV_CPSS_DXCH_IP_PAT_TRIE_OLD_NODE_REC_STC*)currTrieNode->pData;
                    ret = prvCpssDxChIpTcamDispatcherRuleSet(currTrieNode,
                                                             ipTcamShadowPtr,
                                                             &currentRulePtr);
                    if (ret != GT_OK)
                    ...
                    ret = prvCpssDxChIpTcamDispatcherRuleSet(oldNodeStc->oldNode,
                                                             ipTcamShadowPtr,
                                                             &oldRulePtr);
                    if (ret != GT_OK)
                    {
            ...
            if ((currTrieNode->updateStatus & PRV_PAT_TRIE_NODE_NEW_INSERT_E) > 0)
        ...
        if ((isUpdateOnly == GT_FALSE) ||
            ((currTrieNode->updateStatus & PRV_PAT_TRIE_NODE_UPDATE_CHILDREN_E) > 0))
        {
...
{
    GT_STATUS ret = GT_OK;
    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_SET_PREFIX_FUN setPrefixFunc;
    GT_PTR tcamLocationRulePtr = NULL;

    setPrefixFunc =
        ((PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC*)extraData)->ipTcamSetPrefix;

    ret = prvCpssDxChIpTcamDispatcherRuleSet(trieNodePtr,
                                             ipTcamShadowPtr,
                                             &tcamLocationRulePtr);
    if (ret != GT_OK)
    {
...
)
{
    ...
    delPrefixFunc =
        ((PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC*)extraData)->ipTcamDelPrefix;
    ret = prvCpssDxChIpTcamDispatcherRuleSet(trieNodePtr,
                                             ipTcamShadowPtr,
                                             &tcamRulePtr);
    if (ret != GT_OK)
...
{
    GT_STATUS ret;
    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_TRAVERSE_FUN upTraverseFunc;
...
)
{
    ...
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *newRootTrie= NULL,*currTrieNode;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *currSonNode;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC **contTrie,**mergeTrie = NULL;
    GT_PTR tcamRulePtr = NULL;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *boundingSon =NULL;
    cpssOsMemSet(keySuffix,0,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
    ...
    while (currTrieNode != NULL)
    {
            ...
            ((isEcmpPrefixSearch == GT_TRUE) && (i == currPrefixLen)))
        {
            ...
            currTrieNode->pData = NULL;
            ret =  prvCpssDxChIpTcamDispatchFree(ipTcamShadowPtr,entryType,currTrieNode);
            if (ret != GT_OK)
            {
                return ret;
            }
            ...
            /* free token node*/
             PRV_CPSS_DXCH_FREE_TOKEN_NODE_FOR_VALID_TRIE_MAC(ipTcamShadowPtr->tcamManagementMode,
                                                              currTrieNode);
             ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_INVALID_RULE_E,
                                                             ipTcamShadowPtr->tcamManagementMode,
                                                             NULL,
    ...
    if (ret == GT_OK)
    {
        ...
        else while (currTrieNode != NULL)
        {
                ...
                (currTrieNode->father != NULL))
            {
                ...
                if (currTrieNode->pData != NULL)
                {
                    if(ipTcamShadowPtr->tcamManagementMode == PRV_CPSS_DXCH_IP_TCAM_MANAGEMENT_TCAM_MANAGER_MODE_E)
                    {
                        ret = cpssDxChTcamManagerEntryClientCookieUpdate(ipTcamShadowPtr->tcamManagerHandlerPtr,
                                                                         ipTcamShadowPtr->clientId,
                                                                         currTrieNode->tcamAllocInfo.tokenNode->tcamManagerEntryToken,
                ...
                copyIpAddr(currTrieNode->ipAddr,newRootTrie->ipAddr,0,
...
{
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *newTrie;
    GT_STATUS ret = GT_OK;
    if (rootPtrPtr == NULL)
    {
        return GT_BAD_PTR;
    ...
    newTrie = cpssOsMalloc(sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC));
    if (newTrie == NULL)
    {
        return GT_FAIL;
    }

    cpssOsMemSet(newTrie,0,sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC));
    ...
    newTrie->prefixLen = 0;
    newTrie->father = NULL;
    newTrie->updateStatus = PRV_PAT_TRIE_NODE_UPDATE_STATUS_OK_E;
    ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_INVALID_RULE_E,
                                                    ipTcamShadowPtr->tcamManagementMode,
                                                    NULL,
                                                    newTrie,
...
)
{
    ...
    PRV_CPSS_DXCH_FREE_ROOT_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,
                                           rootPtr);
    cpssOsFree(rootPtr);
    return GT_OK;
}
...
)
{
    ...
    ret =  prvCpssDxChIpTcamDispatchFree(ipTcamShadowPtr,entryType,trieNodePtr);
    if (ret != GT_OK)
    {
        return ret;
    }
    ...
    ret = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_INVALID_OLD_RULE_E,
                                                    ipTcamShadowPtr->tcamManagementMode,
                                                    NULL,
                                                    trieNodePtr,
                                                    NULL);
...
*
* OUTPUTS:
*       nodePtr           - points to the next trie node to process
...
)
{
    GT_STATUS                                       rc = GT_OK;
    GT_STATUS                                       rc2 = GT_OK;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC              *boundingSon;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC              *validFather;
    PRV_CPSS_DXCH_IP_PAT_TRIE_TCAM_FUNCS_OBJ_STC    *ipTcamFuncsPtr;
    GT_BOOL                                         isInvalidateNeeded;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC              *oldNodePtr;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC              prefixBeforeRelocation;
    PRV_CPSS_DXCH_IP_TCAM_MGM_ALLOC_METHOD_ENT      allocMethod;
    ...
    cpssOsMemCpy(&prefixBeforeRelocation,prefixPtr,sizeof(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC));
    PRV_CPSS_DXCH_ALLOCATE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,&prefixBeforeRelocation);
    rc = prvCpssDxChIpTcamDispatcherRuleSet(prefixPtr,ipTcamShadowPtr,&oldTcamLocationRulePtr);
    ...
    if (rc != GT_OK)
    {
        PRV_CPSS_DXCH_FREE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,&prefixBeforeRelocation);
        return rc;
    }


    /* get bounding son, valid father and TCAM functions */
    rc = cpssDxChIpPatTrieGetBoundingSon(prefixPtr,ipTcamShadowPtr,&boundingSon);
    if (rc != GT_OK)
    {
        PRV_CPSS_DXCH_FREE_TOKEN_NODE_MAC(ipTcamShadowPtr->tcamManagementMode,&prefixBeforeRelocation);
        return rc;
    }
    validFather    = getValidFather(prefixPtr);
    ipTcamFuncsPtr = prefixPtr->ipTcamFuncsPtr;
    /* choose allocation methood */
    allocMethod = PRV_CPSS_DXCH_IP_TCAM_MGM_ALLOC_MID_E;
    if (validFather == NULL)
    {
        allocMethod = PRV_CPSS_DXCH_IP_TCAM_MGM_ALLOC_MAX_E;
    }
    ...
    if (validFather != NULL)
    {
        rc = prvCpssDxChIpTcamDispatcherRuleSet(validFather,
                                                ipTcamShadowPtr,
                                                &upperIdx);
        if (rc != GT_OK)
        {
    ...
    if(boundingSon!= NULL)
    {
        rc = prvCpssDxChIpTcamDispatcherRuleSet(boundingSon,
                                                ipTcamShadowPtr,
                                                &lowerIdx);
        if (rc != GT_OK)
        {
    ...
    rc =  prvCpssDxChIpTcamDispatchAllocate(ipTcamShadowPtr,
                                           prefixPtr->entryType,
                                           lowerIdx,
                                           upperIdx,
                                           allocMethod,
    ...
    rc = prvCpssDxChIpTcamDispatcherRuleSet( prefixPtr,
                                             ipTcamShadowPtr,
    ...
    rc = prvCpssDxChIpTcamDispatcherRuleSet(&prefixBeforeRelocation,
                                            ipTcamShadowPtr,
                                            &oldTcamLocationRulePtr);
    if (rc != GT_OK)
    {
    ...
    if (rc != GT_OK)
    {
        rc2 = prvCpssDxChIpTcamDispatchFree(ipTcamShadowPtr,
                                            prefixPtr->entryType,
                                            prefixPtr);
        if (rc2 != GT_OK)
            return GT_FAIL;
        rc2 = prvCpssDxChIpTcamDispatcherRuleInfoUpdate(PRV_CPSS_DXCH_DISPATCHER_TCAM_ALLOCATED_RULE_AND_TRIE_UPDATE_E,
                                                        ipTcamShadowPtr->tcamManagementMode,
                                                        NULL,
                                                        prefixPtr,
                                                        oldTcamLocationRulePtr);
...
#ifdef DEBUG_IP_PAT_TRIE
void printPatTrieRec(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *rootPtr,GT_U8* ipAddr, GT_U32 prefixLen,GT_U32 spaces,
                     GT_U32 numOfDash)
{
    GT_U8 uIp[IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES],copyIp[IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES];
    GT_U32 i =0,addi = 0, size = 0;
    PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *validFather;
    cpssOsMemSet(&uIp,0,16);
    copyIpAddr(ipAddr,rootPtr->ipAddr,0,rootPtr->prefixLen,prefixLen);
    copyIpAddr(uIp,ipAddr,0,(prefixLen+rootPtr->prefixLen),0);
    if (rootPtr->pData != NULL)
    {
        ...
        addi = 1;
        validFather = getValidFather(rootPtr);
        osPrintf("%.2d ) ",rootPtr->tcamAllocInfo.ruleNode->offset);

        for (i = 0; i <= (rootPtr->prefixLen+prefixLen)/8; i++)
        {
            /*bot = (i == ((rootPtr->prefixLen+prefixLen)/8 -1))?
                (7 - ((rootPtr->prefixLen+prefixLen)%8)):0; */

            osPrintf("%.2X",uIp[i]);
            /*r (j = 7;j >= 0; j--)
            {
                osPrintf("%d",(uIp[i] >> j) & 0x1);
            }*/
        }
        ...
        if (validFather != NULL)
        {
            if (validFather->tcamAllocInfo.ruleNode->offset <= rootPtr->tcamAllocInfo.ruleNode->offset)
            {
                osPrintf("!!!");
            }
            /*osPrintf(" - F %d\n",validFather->ruleNode->offset);*/
        }
        else
        {
            /*osPrintf("\n");*/
    ...
    cpssOsMemCpy(copyIp,ipAddr,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
    ...
    if (rootPtr->rightSon != NULL)
    {
        ...
        {
            osPrintf("rightSize Error!\n");
        }
        printPatTrieRec(rootPtr->rightSon,copyIp,prefixLen+rootPtr->prefixLen,spaces+addi,
                        numOfDash+addi);
    ...
    if (rootPtr->leftSon != NULL)
    {
        ...
        {
            osPrintf("leftSize Error!\n");
        }
        printPatTrieRec(rootPtr->leftSon,ipAddr,prefixLen+rootPtr->prefixLen,spaces+addi,
                        numOfDash);
    ...
    {
        osPrintf("sizeMismatch!!!\n");
    }

}

void printPatTrie(PRV_CPSS_DXCH_IP_PAT_TRIE_NODE_STC *rootPtr)
{
    GT_U8 ipAddr1[IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES];
    cpssOsMemSet(ipAddr1,0,IP_PAT_TRIE_MAX_PREFIX_LEN_IN_BYTES);
    printPatTrieRec(rootPtr,ipAddr1,0,0,0);
