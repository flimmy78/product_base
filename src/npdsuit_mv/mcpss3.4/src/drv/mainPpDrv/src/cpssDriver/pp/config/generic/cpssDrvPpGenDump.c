...
*
*******************************************************************************/
static GT_STATUS prvPrintPpInfo
(
    IN GT_U8                    devNum
)
{
    ...
    DUMP_PRINT((" devNum = [%d] \n",devInfoPtr->devNum));
    DUMP_PRINT((" devType = [0x%8.8x] \n",devInfoPtr->devType));
    DUMP_PRINT((" devFamily = [%s] \n",
...
)
{
    ...
    for (i = 0; i < regCount; i++)
    {
        ...
        if (regArray[i] == PRV_CPSS_SW_PTR_BOOKMARK_CNS)
        {
            ...
            if (regArray[i+1] & PRV_CPSS_SW_TYPE_WRONLY_CNS)
            {
                /* if the pointer field data is write only skip the pointer,
                   else skip the whole range*/
                i += 1 + ((regArray[i+1] & PRV_CPSS_SW_TYPE_PTR_FIELD_CNS)? 2 :
                           regArray[i+2]);
            }
            else
            {
                if((i+3) < regCount)
                {
                    printPpDumpRegisters (devNum, portGroupId, (GT_U32*)regArray[i+3], regArray[i+2]/4 );
                    i += 3;
                }
            }
        }
