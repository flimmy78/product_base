...
/*******************************************************************************
* prvCpssDrvInterruptsAfterSysUp
*
* DESCRIPTION:
*       Interrupts initialization after system startup process was performed.
*
* INPUTS:
...
* COMMENTS:
*       This initialization includes:
*           1. Updating the mask registers of the interrupt mask registers that
*              cannot be written before start-init.
*
*******************************************************************************/
GT_STATUS prvCpssDrvInterruptsAfterSysUp
(
    IN GT_U8   devNum
)
{
	...
	/* Here we add the portGroupId for drvHwPpInitInMasks()      */
	/* driverDxExMxInitIntMasks() zhangdi@autelan.com 2011-03-35 */
    GT_U32   portGroupId; 
    portGroupId = PRV_CPSS_NETIF_SDMA_PORT_GROUP_ID_MAC(devNum);

    /* Initialize the interrupt mask register's that can    */
    /* be modified only after phase2.                       */
    retVal = prvCpssDrvPpObjConfig[devNum]->genPtr->drvHwPpInitInMasks(devNum,portGroupId,GT_FALSE,GT_TRUE);
	if(retVal != GT_OK)
    {
        return retVal;
    }
