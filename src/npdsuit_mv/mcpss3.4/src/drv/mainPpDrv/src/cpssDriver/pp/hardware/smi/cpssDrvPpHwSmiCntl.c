...
)
{
    ...
    return cpssExtDrvHwIfSmiTskRegRamWrite(baseAddress, addr, data, length);
}
...
)
{
    ...
    return cpssExtDrvHwIfSmiTskRegVecWrite(baseAddress, addrArr, dataArr, arrLen);
}
...
)
{
    ...
    return cpssExtDrvHwIfSmiTaskReadReg(devNum, regAddr, data);
}
...
)
{
    portGroupId = portGroupId;
    /* here we go directly to the "BUS" */
    return cpssExtDrvHwIfSmiTaskWriteReg(devNum, regAddr, data);
}
