...
*                     Init.
*       afterSysUp - Is the call to this function is done before / after system is started-up
*
* OUTPUTS:
*      None.
*
...
*
* COMMENTS:
*      add afterSysUp, because we can only enable interrupt after system UP !
*      zhangdi  2011-03-25
*
*******************************************************************************/
...
)
{
    ...
    for(i = 0; i < (numOfIntBits >> 5); i++)
    {
        if((prvCpssDrvPpConfig[devNum]->intCtrl.accessB4StartInit[i] == b4StartInit) &&
			(prvCpssDrvPpConfig[devNum]->intCtrl.accessAfterSysUp[i] == afterSysUp))		
        {
                ...
                (0 == PRV_CPSS_DRV_HW_IF_PCI_COMPATIBLE_MAC(devNum)))
            {
			   	if(afterSysUp)
			   	{
            		osPrintfEvt("system startup set device %d interrupt reg %#x shadow %#0x\r\n",	\
            				devNum,intReqNodeList[i << 5].intMaskReg,intMaskShadow[i]);
			   	}
