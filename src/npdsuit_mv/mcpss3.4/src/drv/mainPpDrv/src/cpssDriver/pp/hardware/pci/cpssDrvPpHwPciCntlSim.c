...
#define HW_BYTE_SWAP(devNum,_portGroupId,data) \
    ((prvCpssDrvPpConfig[devNum]->hwCtrl[_portGroupId].doByteSwap == GT_TRUE) ? \
        ((((data) << 24))               | \
         (((data) & 0xff00) << 8)       | \
         (((data) & 0xff0000) >> 8)     | \
         (((data) >> 24))):               \
...
)
{
    ...
    if (SIM_ADDR_COMPL_EN_GET(devNum,portGroupId) == GT_FALSE)
    {
        ...
        for(j = 0; j < length; j++)
        {
            data = HW_BYTE_SWAP(devNum, portGroupId,dataPtr[j]);
...
)
{
    ...
    else
    {
        ...
        dataCompletion = HW_BYTE_SWAP(devNum, portGroupId,dataCompletion);
...
)
{
    ...
    else  /* use Region #1 */
    {
        ...
        tmpValue = HW_BYTE_SWAP(devNum, portGroupId,data);
...
)
{
    ...
    temp = HW_BYTE_SWAP(devNum, portGroupId,data);
