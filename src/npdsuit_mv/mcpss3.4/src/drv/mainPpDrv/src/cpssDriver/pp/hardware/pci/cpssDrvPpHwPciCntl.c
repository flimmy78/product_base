...
/* macro to byte swap when needed */
#define HW_BYTE_SWAP(devNum,portGroupId,data)                           \
    ((prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].doByteSwap == GT_TRUE) ?  \
        BYTESWAP_MAC(data):                                     \
        (data))
...
#define DRAIN_WRITE_BUFFERS(devNum,portGroupId) {                           \
    GT_U32 dummyBaseAddr, dummyData;                            \
...
)
{
    ...
    doByteSwap = doByteSwap; /* fix VC warning C4100: 'doByteSwap' : unreferenced formal parameter */
    ...
    *(volatile GT_U32 *)(address) = 0;
    ...
    if ((isDiag == GT_FALSE) && (prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].hwComplSem == NULL))
    {
...
)
{
    GT_U32  lastWord;
    GT_U32  index;
    if (hsuInfo.hsuState == CPSS_HSU_STATE_IN_PROGRESS_E)
    {
        /* we are in HSU process -- hardware should not be affected */
        return GT_OK;
    }
    ...
    if (address & 0x7)
    {
        *(volatile GT_U32*)(address) = HW_BYTE_SWAP(devNum, portGroupId,dataPtr[0]);
        address +=4;
        length -=1;
    }
    ...
    {
        cpssExtDrvPciDoubleWrite(address, HW_BYTE_SWAP(devNum, portGroupId,dataPtr[index]), HW_BYTE_SWAP(devNum, portGroupId,dataPtr[index+1]));
    }

    /* write 32-bit postfix, if exists  - check if length is odd */
    if (length & 0x1)
    {
        *((volatile GT_U32*)(address)) = HW_BYTE_SWAP(devNum, portGroupId,lastWord);
    }
...
)
{
    ...
    GT_U32  nextRegionAddr; /* address of the next region after the one
                                currently used */
    GT_U32  loopLength; /* when length exceeds region addr, Set to end of
                            region range */
    ...
    if((regAddr & 0xFF000000) != 0)
    {
        ...
        if(i == 1)
        {
            ...
            data = HW_BYTE_SWAP(devNum,portGroupId, data);
        ...
        if((GT_U32)(regAddr + length * 4 - 1) < nextRegionAddr)
        {
            ...
            for(j = 0; j < length; j++)
            {
                *((volatile GT_U32*)address) = HW_BYTE_SWAP(devNum, portGroupId,dataPtr[j]);
    ...
    else
    {
        ...
        if((GT_U32)(regAddr+length*4-1) < 0x1000000)
        {
            ...
            for(i = 0;i < length;i++)
            {
                *((volatile GT_U32*) address) = HW_BYTE_SWAP(devNum, portGroupId,dataPtr[i]);
                /* need to put GT_SYNC to avoid from having the CPU doing
                * write combine. Some CPUs like advanced new PPC can do write combining
                * of two writes to continuous addresses. So, to avoid that we use the eioio
                * in PPC architecture;*/
...
)
{
    ...
    #if 0
    printf("hwRead( devNum=%d, portGroupId=%d, regAddr=0x%8x, length=%d )\n", devNum, portGroupId, regAddr, length);/*xcat debug*/
    printf("       baseAddr=0x%8x\n", prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].baseAddr);/*xcat debug*/
    printf("       doByteSwap=%d\n", prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].doByteSwap);/*xcat debug*/
    #endif
    ...
    {
        #if 0
        if((regAddr & 0xFFF00000)==0x09800000)
        {
            printf("       read regAddr %8x\n", regAddr);/*xcat debug*/
            //return GT_BAD_PARAM;
        }
        #endif
        cpssOsMutexLock(prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].hwComplSem);
        ...
        #if 0   /*xcat debug*/
        printf("       addrCompletShadow[0]=0x%2x\n", (prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].addrCompletShadow[0]));/*xcat debug*/
        printf("       addrCompletShadow[1]=0x%2x\n", (prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].addrCompletShadow[1]));/*xcat debug*/
        printf("       addrCompletShadow[2]=0x%2x\n", (prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].addrCompletShadow[2]));/*xcat debug*/
        printf("       addrCompletShadow[3]=0x%2x\n", (prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].addrCompletShadow[3]));/*xcat debug*/
        ...
        if ((GT_U32)(regAddr+length*4-1) < nextRegionAddr)
        {
            for(j = 0;j < length;j++)
            {
                ...
                dataPtr[j] = HW_BYTE_SWAP(devNum, portGroupId,temp);
                address += 4;
            }
            cpssOsMutexUnlock ( prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].hwComplSem);
        }
    ...
    else
    {
        ...
        if ((GT_U32)(regAddr+length*4-1) < 0x1000000)
        {
            for(i = 0;i < length;i++)
            {
                temp = *( (volatile GT_U32 *) address);
                GT_SYNC;
                dataPtr[i] = HW_BYTE_SWAP(devNum, portGroupId,temp);
                address += 4;
            }
        }
        else
...
)
{
    ...
    else
    {
        ...
        dataComletion = HW_BYTE_SWAP(devNum, portGroupId,dataComletion);
...
)
{
    ...
    else  /* use Region #1 */
    {
        ...
        *(volatile GT_U32 *)(addrCompletion) = HW_BYTE_SWAP(devNum, portGroupId,data);
    ...
    tmpValue = HW_BYTE_SWAP(devNum, portGroupId,value);
    /* write data 3 times */
...
{
    GT_U32 address;     /* physical access address for PCI transaction. */
    GT_U32  temp;
...
)
{
    ...
    address = (prvCpssDrvPpConfig[devNum]->hwCtrl[portGroupId].internalPciBase + regAddr);
    *((volatile GT_U32*)address) = HW_BYTE_SWAP(devNum, portGroupId,data);
...
)
{
    ...
    if(allignSize != length)
    {
        ...
        {
            tempSrcAddr =
                (GT_U32*)(((GT_U32)tempData) + (i * sizeOfBurst *
                                                sizeof(GT_U32)));
    ...
    for(i = 0; i < length; i++)
    {
        tempData[i + words2Allign] = HW_BYTE_SWAP(devNum, portGroupId,data[i]);
    }
    if(memoryAccessTraceOn == GT_TRUE)
    {
    ...
    {
        tempSrcAddr =
            (GT_U32*)(((GT_U32)tempData) + (i * ramBurstInfo->burstSize *
                                            sizeof(GT_U32)));
        tempDestAddr= (allignDestAddr +
...
)
{
    ...
    for(i = 0; i < numOfBursts; i++)
    {
        tempDstAddr = (GT_U32*)(((GT_U32)tempData) + (i * sizeOfBurst *
                                                      sizeof(GT_U32)));
    ...
    for(i = 0; i < length; i++)
    {
        data[i] = HW_BYTE_SWAP(devNum, portGroupId,tempData[i + words2Allign]);
    }
    if(memoryAccessTraceOn == GT_TRUE)
    {
        for(i = 0; i < allignSize; i++)
...
)
{
    ...
    {
        data[i] = val + i;
        data[i] = HW_BYTE_SWAP(0,portGroupId,data[i]);
    }
