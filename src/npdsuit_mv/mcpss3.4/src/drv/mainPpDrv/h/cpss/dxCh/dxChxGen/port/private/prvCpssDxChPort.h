...
#if 1   /* cpss1.3 */
/* get the register address - port LED interface Ctrl Register */
#define PRV_CPSS_DXCH_PORT_LED_INTF_CTRL_REG(devNum,portNum,intfIndex,regAddrPtr)\
    if( (portNum) != CPSS_CPU_PORT_NUM_CNS )                \
        *(regAddrPtr) = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->ledRegs.ledControl[intfIndex];


/* get the register address - xsmi interface ac timing*/
#define PRV_CPSS_DXCH_XSMI_AC_TIMING_REG(devNum,regAddrPtr)\
	*(regAddrPtr) = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->smiRegs.smi10GeAcTiming;
#endif
