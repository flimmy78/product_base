...
#if 1   /* cpss1.3 */
/* check that the device is DxCh2 return GT_BAD_PARAM on error */
#define PRV_CPSS_DXCH2_DEV_CHECK_MAC(devNum)            \
    if((0 == PRV_CPSS_IS_DEV_EXISTS_MAC(devNum)) ||     \
       (0 == PRV_CPSS_DXCH2_FAMILY_CHECK_MAC(devNum)))  \
    {                                                   \
        return GT_BAD_PARAM;                            \
    }

/* get the port type */
#define PRV_CPSS_DXCH_PORT_TYPE(devNum,portNum) \
        PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portNum].portType

/* use of this macro should be removed !!!  instead use PRV_CPSS_DXCH_PHY_PORT_CHECK_MAC */
#define PRV_CPSS_DXCH_VIRTUAL_PORT_CHECK_MAC \
        PRV_CPSS_DXCH_PHY_PORT_CHECK_MAC
