...
/*******************************************************************************
* prvTgfTailDropTxCountersGet
*
* DESCRIPTION:
*       Get all Tx buffers and descriptors
*
* INPUTS:
*       poolNum - shared pool number, range 0..7
*
* OUTPUTS:
*       getCntrPtr - (pointer to) port tx counters set
*
* RETURNS:
*       None
*
...
)
{
    ...
    /* 6.1 Set Dp limits for tail drop profile 8 */
    rc = prvTgfPortTxTailDropProfileSet(
            prvTgfDevNum,
            CPSS_PORT_TX_DROP_PROFILE_8_E,
            GT_FALSE, prvTgfPortMaxBuffLimit, prvTgfPortMaxBuffLimit);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPortTxTailDropProfileSet");

    /* check correct setting */
    rc = prvTgfPortTxTailDropProfileGet(
            prvTgfDevNum,
            CPSS_PORT_TX_DROP_PROFILE_8_E,
            &enableSharing, &dpMaxBuff, &dpMaxDesc);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPortTxTailDropProfileGet");

    /* verifying values */
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_FALSE, enableSharing,
            "got other enableSharing than expexted");
    UTF_VERIFY_EQUAL0_STRING_MAC(prvTgfPortMaxBuffLimit, dpMaxBuff,
            "got other dpMaxBuff than expexted");
    UTF_VERIFY_EQUAL0_STRING_MAC(prvTgfPortMaxBuffLimit, dpMaxDesc,
            "got other dpMaxDesc than expexted");
    ...
    while (prfIdx--)
    {
        if (isVariousPorts || (PRV_TGF_RECEIVE_PORT_IDX_CNS == prvTgfPortIdxArr[prfIdx]))
        {
            ...
            /* verifying values */
            UTF_VERIFY_EQUAL0_STRING_MAC(CPSS_PORT_TX_DROP_PROFILE_8_E, profileSetArr[prfIdx],
                    "got other PORT_TX_DROP_PROFILE than expexted");
        }
    }
}
...
)
{
    ...
    UTF_VERIFY_EQUAL0_STRING_MAC(enableMode, enableModeGet,
            "got other enableMode than expexted");
    ...
    if (PRV_TGF_PORT_TX_SHARED_DP_MODE_DP0_E == enableMode)
    {
        ...
        UTF_VERIFY_EQUAL0_STRING_MAC(prvTgfSharedLimit, sharedBufLimit,
                "got other sharedBufLimit than expexted");
        UTF_VERIFY_EQUAL0_STRING_MAC(prvTgfSharedLimit, sharedDescLimit,
                "got other sharedDescLimit than expexted");

        /* 7.3 bind shared pool 2 to profile 8, Tc 7 */
        ...
        UTF_VERIFY_EQUAL0_STRING_MAC(prvTgfPoolNum, poolNum,
                "got other poolNum than expexted");
    }
}
...
)
{
    ...
    PRV_TGF_PORT_TX_TAIL_DROP_WRTD_ENABLERS_STC wrtdEnablers = {0};
    PRV_TGF_PORT_TX_WRTD_MASK_LSB maskLsb = {0};
    CPSS_PORT_MAC_COUNTER_SET_STC portCntrs;
    CPSS_PORT_EGRESS_CNTR_STC     egrCntr = {0};
    GT_U32                        dp;
    GT_U8                         tc =0;
...
)
{
    GT_STATUS rc = GT_OK;
    GT_BOOL   enableSharing = GT_FALSE;
    GT_U16    dpMaxBuff = 0, dpMaxDesc = 0;
    ...
    rc = prvTgfPortTxTailDropProfileSet(prvTgfDevNum,
            CPSS_PORT_TX_DROP_PROFILE_3_E,
            GT_FALSE, prvTgfPortMaxBuffLimit, prvTgfPortMaxBuffLimit);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPortTxTailDropProfileSet");

    /* check correct setting */
    rc = prvTgfPortTxTailDropProfileGet(prvTgfDevNum,
            CPSS_PORT_TX_DROP_PROFILE_3_E,
            &enableSharing, &dpMaxBuff, &dpMaxDesc);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPortTxTailDropProfileGet");
...
)
{
    ...
    while (prfIdx--)
    {
        ...
        rc = prvTgfPclRuleValidStatusSet(
                CPSS_PCL_RULE_SIZE_STD_E, prfIdx, GT_FALSE);
