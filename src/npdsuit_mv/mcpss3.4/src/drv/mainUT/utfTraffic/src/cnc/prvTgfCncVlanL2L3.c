...
)
{
    ...
    if (GT_FALSE != prvUtfDeviceTestNotSupport(prvTgfDevNum, UTF_CH1_E | UTF_CH1_DIAMOND_E | UTF_CH2_E | UTF_CH3_E))
    {
...
)
{
    ...
    {
        PRV_UTF_LOG1_MAC(
            "[TGF]: prvTgfCncCounterSet FAILED, rc = [%d]", rc);
        return;
    }
    ...
    UTF_VERIFY_EQUAL2_STRING_MAC(
        burstCount, counter.packetCount.l[0],
        "packet counter expected: %d received %d",
        burstCount, counter.packetCount.l[0]);
...
)
{
    ...
    prvTgfCncTestSendPacketAndCheck(
        ...
        ((PRV_TGF_IP_PACKET_LEN_CNS + 4) * burstCount));
    ...
    prvTgfCncTestSendPacketAndCheck(
        ...
        (PRV_TGF_IP_PACKET_L3_LEN_CNS * burstCount));
}
...
)
{
    ...
    if(PRV_CPSS_IS_MULTI_PORT_GROUPS_DEVICE_MAC(prvTgfDevNum))
    {
        numOfShifts = 2;
    }
...
)
{
    ...
    {
        /* single port group device */
        portGroupLog = 0;
        portGroupNum = 1;
    }
    else
    {
        /* multiple port group device - 4 port groups assumed */
        portGroupLog = 2;
        portGroupNum = 4;
    }
