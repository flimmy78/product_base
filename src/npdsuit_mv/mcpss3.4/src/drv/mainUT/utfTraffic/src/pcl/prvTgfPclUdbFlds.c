...
)
{
    ...
    GT_U8   portNum = prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS];
    GT_STATUS rc, rc1 = GT_OK;
    ...
    /* restore default of MAC to ME entry 0 */
    rc = prvTgfTtiMacToMeSet(0, &macToMeValue, &macToMeMask);
...
)
{
    ...
    action.tag1VlanCmd         = PRV_TGF_TTI_VLAN_MODIFY_UNTAGGED_E;
    action.tunnelStart         = GT_FALSE;

    action.interfaceInfo.type            = CPSS_INTERFACE_PORT_E;
    action.interfaceInfo.devPort.devNum  = prvTgfDevNum;
...
/*******************************************************************************
* prvTgfTrafficPclOtherFeaturesConfig
*
* DESCRIPTION:
*       Configure other feature like MAC-to-ME
*
* INPUTS:
*       None
*
...
)
{
    ...
    switch (fieldIndex)
    {
        ...
        case PRV_TGF_PCL_RULE_FORMAT_SOURCE_PORT_E:
            ...
            maskStdPtr->sourcePort = 0x3F;
            pattStdPtr->sourcePort = prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS];
            prvTgfPacketsCountRx = 0;
            break;
        ...
        case PRV_TGF_PCL_RULE_FORMAT_IS_L2_VALID_E:
            ...
            prvTgfPacketInfoArray[0] = &prvTgfPacketMimInfo;
