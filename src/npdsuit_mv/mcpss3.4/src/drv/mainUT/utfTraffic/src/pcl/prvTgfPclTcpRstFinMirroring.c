...
)
{
    ...
    *packetsNumPtr = 0;
    ...
    while (1)
    {
        ...
        /* check the DSA tag */
        if ((GT_FALSE == rxParam.dsaCmdIsToCpu)
            || (GT_FALSE != rxParam.srcIsTrunk)
            || (rxParam.devNum != srcDev)
            || (rxParam.portNum != srcPort))
        {
