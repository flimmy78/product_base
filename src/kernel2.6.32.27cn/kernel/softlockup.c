...
#define CPLD_WATCHDOG_FILLUP_REG 0x0F
#define CPLD_WATCHDOG_ENABLE_REG 0x0D
#define CPLD_BASE_ADDR			 0x800000001D010000UL
...
#if 0
extern void octeon_write64_byte(uint64_t csr_addr, uint8_t val);
extern uint8_t octeon_read64_byte(uint64_t csr_addr);
#else
#define DO_COMPAT_ADDR_FIX
static void octeon_write64_byte(uint64_t csr_addr, uint8_t val)
{
...
void softlockup_tick(void)
{
	...
	/* Move fillup in the start of the function, to avoid (touch_timestamp == 0). 2012-10-22 */
	if (counter > 2)
	{
		octeon_read64_byte(CPLD_BASE_ADDR + CPLD_WATCHDOG_FILLUP_REG);
		counter = 0;
	}
	counter++;
	...
	if (kes_mem_print_handle && print_current_time_handle) {
		...
		kes_mem_print_handle("BUG: soft lockup - CPU#%d stuck for %lus! [%s:%d]\n",
...
cpu_callback(struct notifier_block *nfb, unsigned long action, void *hcpu)
{
	...
	case CPU_UP_PREPARE_FROZEN:
		...
		octeon_write64_byte(CPLD_BASE_ADDR + CPLD_WATCHDOG_ENABLE_REG, 0xff);
		if (IS_ERR(p)) {
	...
	case CPU_DEAD_FROZEN:
		...
		kthread_stop(p);
		octeon_write64_byte(CPLD_BASE_ADDR + CPLD_WATCHDOG_ENABLE_REG, 0xfe);
		break;
