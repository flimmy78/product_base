...
/**************************hanhui upgrade*************/
unsigned int br_user_mstp;/*1 is enabled 0 is not enable*/
/********************************end*****************/
...
/**********************hanhui upgrade****************/
#if 1
	case BRCTL_USER_MSTP_EN:
	{
		if ((1 != args[1]) && (0 != args[1])) {
			return ERANGE;
		}
		br_user_mstp = args[1];/*1 is mstp enabled 0 is not enable*/		
		printk(KERN_DEBUG "br ioctl set user mstp %s\n", br_user_mstp ? "enable" : "disable");	
