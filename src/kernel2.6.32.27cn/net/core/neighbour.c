...
/******************hanhui upgrade************/
	...
	unsigned char hw_addr[MAX_ADDR_LEN] = {0};
#endif
/**********************end******************/
...
#ifndef __AX_PLATFORM__
#ifdef CONFIG_ARPD
	NEIGH_PRINTK2("%s%d :: neigh %p(%d.%d.%d.%d)(%02X-%02X-%02X-%02X-%02X-%02X) %s notify arpd.\n", 	\
		__func__,__LINE__, neigh, 	\
		neigh->primary_key[0],neigh->primary_key[1],neigh->primary_key[2],neigh->primary_key[3], \
		neigh->ha[0],neigh->ha[1],neigh->ha[2],neigh->ha[3],neigh->ha[4],neigh->ha[5], \
		(NUD_STALE == neigh->nud_state)? "STALE": \
		(NUD_DELAY == neigh->nud_state)? "DELAY": \
		(NUD_PROBE == neigh->nud_state)? "PROBE": \
		(NUD_INCOMPLETE == neigh->nud_state)? "INCOMP": \
		(NUD_REACHABLE == neigh->nud_state)? "REACHA":"FAILED");
	
	/* by qinhs@autelan.com 10/16/2008 : ignore neigh entry with zero hardare address */
	if(0 != memcmp(neigh->ha, hw_addr,(neigh->dev ? neigh->dev->addr_len: MAX_ADDR_LEN))) {
