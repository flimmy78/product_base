...
static u32 nf_nat_ipv6_secure_port(const struct nf_conntrack_tuple *t,
				   __be16 dport)
{
	return secure_ipv6_port_ephemeral_new(t->src.u3.ip6, t->dst.u3.ip6, dport);
}
	  
bool nf_nat_ipv6_manip_pkt(struct sk_buff *skb,
				  ...
				  enum nf_nat_manip_type maniptype)
{
	...
	__be16 frag_off_=0;/*huangjing##change frag_off to frag_off_*/
	int hdroff;
	u8 nexthdr;
	int i=0;
	__be16 _frag_off, *fp;
	...
	/*huangjing##add  */
	if(ipv6_ext_hdr(nexthdr) && (nexthdr == NEXTHDR_FRAGMENT))
	{
        fp = skb_header_pointer(skb,
        			iphdroff + sizeof(*ipv6h)+offsetof(struct frag_hdr,
        				       frag_off),
        			sizeof(_frag_off),
        			&_frag_off);
        if (fp == NULL)
