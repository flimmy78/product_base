...
/* for debug CF card error, 2012-10-10 zhangdi@autelan.com */
int cf_debug = 0;   
module_param(cf_debug,int,0644);
...
static void octeon_cf_set_piomode(struct ata_port *ap, struct ata_device *dev)
{
	...
	/*wangchao change the "pio_mode" to improve the speed of writing for CF card*/
	if (ata_timing_compute(dev, XFER_PIO_1/*dev->pio_mode*/, &timing, T, T))
		BUG();
					  ...
					  int rw)
{
	...
	if(cf_debug)
	{
    	printk(KERN_DEBUG "######## dev->devno:%d, buffer 0x%llx,buflen %d, r/w: %d\n",dev->devno,buffer,buflen,rw);
    	printk(KERN_DEBUG "######## ap->ioaddr.data_addr:0x%llx \n",ap->ioaddr.data_addr);
    	printk(KERN_DEBUG "######## data_addr:0x%llx   \n",data_addr);
	}
	...
	if (rw) {
    	if(cf_debug)
    	{		
       	    printk(KERN_DEBUG "########## [write] to buffer:0x%llx words:%d to  data_addr:0x%llx \n\n",buffer,words,data_addr);
    	}
		count = 16;
		while (words--) {
    	...
    	if(cf_debug)
    	{	
        	printk(KERN_DEBUG "########## [read] to buffer:0x%llx words:%d  from  data_addr:0x%llx \n\n",buffer,words,data_addr);	
		}
		while (words--) {
				...
				const struct ata_taskfile *tf)
{
	...
	if(cf_debug)
	{		
   	    printk(KERN_DEBUG "######## ata%u: cmd 0x%X\n", ap->print_id, tf->command);
	}

/*	printk(KERN_ERR "ata%u: cmd 0x%X\n", ap->print_id, tf->command);*/
...
{
	printk(KERN_DEBUG "######## octeon_cf_init   \n");	
