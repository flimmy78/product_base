 ...
 */
static inline int ad_rtnetlink_fill_info
(
	struct sk_buff 		*skb,
	struct net_device 	*dev,
	int					type,
	struct ad_event  	*event
)
{
	struct ad_event *r = NULL;
	struct nlmsghdr  *nlh = NULL;
	unsigned char	 *b = skb_tail_pointer(skb);

	if (!skb || !dev || !event) {
		printk(KERN_ERR DRV_NAME "ad_rtnetlink_fill_info: skb %s dev %s event %s\n", skb ? "exist":"nil", dev ? "exist":"nil", event ? "exist":"nil");
		return -1;
	}
	
	nlh = NLMSG_PUT(skb, 0, 0, type, sizeof(*r));
	r = NLMSG_DATA(nlh);
	r->bond_ifindex = event->bond_ifindex;	
	r->slave_ifindex = event->slave_ifindex;
	r->state = event->state;
	r->flag = TRUNK_3AD_CHANE_IF_MSG;
...
)
{
	...
	if (!dev || !event) {
		printk(KERN_ERR DRV_NAME "ad_rtmsg_info: dev %s event %s\n", dev ? "exist":"nil", event ? "exist":"nil");
		return;
	}
	...
	if (ad_rtnetlink_fill_info(skb, dev, RTM_NEWLINK,
				  		event) < 0) {
		kfree_skb(skb);
		return;
	}
	NETLINK_CB(skb).dst_group = RTNLGRP_LINK;
	netlink_broadcast(init_net.rtnl, skb, 0, RTNLGRP_LINK, GFP_ATOMIC);
}
...
{
	struct ad_event  *event = NULL;		/* malloc whole event */
	int event_len = 0;				/* Its size */

	if (!bond || !slave) {
		printk(KERN_ERR DRV_NAME "ad_send_event: bond %s slave %s\n", bond ? "exist":"nil", slave ? "exist":"nil");
		return;
	}
	
	event_len = sizeof(struct ad_event);
	...
	/* Fill event */
	event->bond_ifindex= bond->ifindex;
	event->slave_ifindex= slave->ifindex;
	event->state = state;
	/* rtnetlink event channel */
	ad_rtmsg_info(bond, event);
...
static void ad_mux_machine(struct port *port)
{
...
#ifndef __AX_PLATFORM__
	int state = 1; /* 0 for add,1 for delete  */
	...
	if (port->sm_mux_state != last_state) {
		...
		#ifndef __AX_PLATFORM__
		if (__port_is_enabled(port) && (AD_MUX_COLLECTING_DISTRIBUTING == port->sm_mux_state)) {
			state = 0;
		}
		bond = __get_bond_by_port(port);
		if(bond && bond->dev) {
			if (port->slave && port->slave->dev) {
				pr_debug("Mux Machine: Port %d state %d,  Port link %d\n",  \
							port->actor_port_number, port->slave->state, port->slave->link);
				ad_send_event(bond->dev, port->slave->dev, state);

			}
			else {
				printk(KERN_ERR DRV_NAME "Mux Machine: port %d slave %s dev %s\n", port->actor_port_number, \
							port->slave ? "exist":"nil", (port->slave && port->slave->dev) ? "exist":"nil");
			}
		}
		else {
			printk(KERN_ERR DRV_NAME "Mux Machine: port %d bond %s dev %s\n", port->actor_port_number, \
							bond ? "exist":"nil", (bond && bond->dev) ? "exist":"nil");
		}
		#endif	
	}
}
