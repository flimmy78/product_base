...
module_param(bondname, charp, 0);
MODULE_PARM_DESC(bondname, "bonding interface name");
...
static int __init bonding_init(void)
{
	...
	if(NULL == bondname)
	{
		printk(KERN_ERR "Parameter 'bondname' must be set, for example: modprobe bonding -o bondx bondname=bondx\n");
		goto err;
	}
	res = bond_create(bondname);
	if(res)
		goto err;
