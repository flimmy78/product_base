...
/*<kes module add by libing@autelan.com begin 2011-09-13*/
int (*kes_mem_print_handle)(const char * fmt, ...) = NULL;
int (*kes_mem_dump_handle)(char *buff, int size) = NULL;
void (*print_current_time_handle)(void) = NULL;
/*kes module add by libing@autelan.com end 2011-09-13>*/
...
static void show_raw_backtrace(unsigned long reg29)
{
	...
	if (kes_mem_print_handle && print_current_time_handle) {
		...
		kes_mem_print_handle("Call Trace:");
	}
#ifdef CONFIG_KALLSYMS
	printk("\n");
	if (kes_mem_print_handle) {
		kes_mem_print_handle("\n");
	}
#endif
	while (!kstack_end(sp)) {
		...
		if (__get_user(addr, p)) {
			...
			if (kes_mem_print_handle) {
				kes_mem_print_handle(" (Bad stack address)");
			}
			break;
		}
	...
	const struct pt_regs *regs)
{
	...
	while ((unsigned long) sp & (PAGE_SIZE - 1)) {
		...
		if (kes_mem_print_handle) {
			kes_mem_print_handle(" %0*lx", field, stackdata);
		}
		i++;
	}
	printk("\n");
...
static void show_code(unsigned int __user *pc)
{
	...
	for(i = -3 ; i < 6 ; i++) {
		...
		if (pc16 ? __get_user(insn, pc16 + i) : __get_user(insn, pc + i)) {
			...
			if (kes_mem_print_handle) {
				kes_mem_print_handle(" (Bad address in epc)\n");
			}
			break;
		}
		printk("%c%0*x%c", (i?' ':'<'), pc16 ? 4 : 8, insn, (i?' ':'>'));
		if (kes_mem_print_handle) {
			kes_mem_print_handle("%c%0*x%c", (i?' ':'<'), pc16 ? 4 : 8, insn, (i?' ':'>'));
		}
	}
}
...
static void __show_regs(const struct pt_regs *regs)
{
	...
	for (i = 0; i < 32; ) {
		if ((i % 4) == 0) {
			printk("$%2d   :", i);
			if (kes_mem_print_handle) {
				kes_mem_print_handle("$%2d   :", i);
			}
		}
		if (i == 0) {
			printk(" %0*lx", field, 0UL);
			if (kes_mem_print_handle) {
				kes_mem_print_handle(" %0*lx", field, 0UL);
			}
		}
		else if (i == 26 || i == 27) {
			printk(" %*s", field, "");
			if (kes_mem_print_handle) {
				kes_mem_print_handle(" %*s", field, "");
			}
		}
		else {
			printk(" %0*lx", field, regs->regs[i]);
			if (kes_mem_print_handle) {
				kes_mem_print_handle(" %0*lx", field, regs->regs[i]);
			}
		}
	...
	printk("Acx    : %0*lx\n", field, regs->acx);
	if (kes_mem_print_handle) {
		kes_mem_print_handle("Acx    : %0*lx\n", field, regs->acx);
	}
#endif
	printk("Hi    : %0*lx\n", field, regs->hi);
	printk("Lo    : %0*lx\n", field, regs->lo);
	if (kes_mem_print_handle) {
		kes_mem_print_handle("Hi    : %0*lx\n", field, regs->hi);
		kes_mem_print_handle("Lo    : %0*lx\n", field, regs->lo);
	}
	...
	if (kes_mem_print_handle) {
		kes_mem_print_handle("epc   : %0*lx %pS\n", field, regs->cp0_epc,
							(void *) regs->cp0_epc);
		kes_mem_print_handle("    %s\n", print_tainted());
		kes_mem_print_handle("ra    : %0*lx %pS\n", field, regs->regs[31],
							(void *) regs->regs[31]);
		kes_mem_print_handle("Status: %08x    ", (uint32_t) regs->cp0_status);
	}
	...
	if (current_cpu_data.isa_level == MIPS_CPU_ISA_I) {
		if (regs->cp0_status & ST0_KUO) {
			printk("KUo ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("KUo ");
			}
		}
		if (regs->cp0_status & ST0_IEO) {
			printk("IEo ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("IEo ");
			}
		}
		if (regs->cp0_status & ST0_KUP) {
			printk("KUp ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("KUp ");
			}
		}
		if (regs->cp0_status & ST0_IEP) {
			printk("IEp ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("IEp ");
			}
		}
		if (regs->cp0_status & ST0_KUC) {
			printk("KUc ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("KUc ");
			}
		}
		if (regs->cp0_status & ST0_IEC) {
			printk("IEc ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("IEc ");
			}
		}
	} else {
		if (regs->cp0_status & ST0_KX) {
			printk("KX ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("KX ");
			}
		}
		if (regs->cp0_status & ST0_SX) {
			printk("SX ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("SX ");
			}
		}
		if (regs->cp0_status & ST0_UX) {
			printk("UX ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("UX ");
			}
		}
		...
		case KSU_USER:
			printk("USER ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("USER ");
			}
			break;
		case KSU_SUPERVISOR:
			printk("SUPERVISOR ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("SUPERVISOR ");
			}
			break;
		case KSU_KERNEL:
			printk("KERNEL ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("KERNEL ");
			}
			break;
		default:
			printk("BAD_MODE ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("BAD_MODE ");
			}
			break;
		}
		if (regs->cp0_status & ST0_ERL) {
			printk("ERL ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("ERL ");
			}
		}
		if (regs->cp0_status & ST0_EXL) {
			printk("EXL ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("EXL ");
			}
		}
		if (regs->cp0_status & ST0_IE) {
			printk("IE ");
			if (kes_mem_print_handle) {
				kes_mem_print_handle("IE ");
			}
		}
	}
	printk("\n");
	...
	printk("Cause : %08x\n", cause);
	if (kes_mem_print_handle) {
		kes_mem_print_handle("Cause : %08x\n", cause);
	}
	...
	if (1 <= cause && cause <= 5) {
		...
		if (kes_mem_print_handle) {
			kes_mem_print_handle("BadVA : %0*lx\n", field, regs->cp0_badvaddr);
		}
	}
	...
	printk("PrId  : %08x (%s)\n", read_c0_prid(),
	       cpu_name_string());
	if (kes_mem_print_handle) {
		kes_mem_print_handle("PrId  : %08x (%s)\n", read_c0_prid(),
						       cpu_name_string());
	}
}
...
void show_registers(struct pt_regs *regs)
{
	...
	if (kes_mem_print_handle && print_current_time_handle) {
		print_current_time_handle();
		kes_mem_print_handle("Process %s (pid: %d, threadinfo=%p, task=%p, tls=%0*lx)\n",
						       current->comm, current->pid, current_thread_info(), current,
						      field, current_thread_info()->tp_value);
	}
	if (cpu_has_userlocal) {
		...
		if (tls != current_thread_info()->tp_value) {
			printk("*HwTLS: %0*lx\n", field, tls);
			if (kes_mem_print_handle) {
				kes_mem_print_handle("*HwTLS: %0*lx\n", field, tls);
			}
		}
	}
...
void __noreturn die(const char * str, struct pt_regs * regs)
{
	...
	if (panic_on_oops) {
		...
		if (kes_mem_print_handle) {
			kes_mem_print_handle("Fatal exception: panic in 5 seconds\n");
		}
		ssleep(5);
...
asmlinkage void do_be(struct pt_regs *regs)
{
	...
	if (kes_mem_print_handle && print_current_time_handle) {
		print_current_time_handle();
		kes_mem_print_handle("%s bus error, epc == %0*lx, ra == %0*lx\n",
						       data ? "Data" : "Instruction",
						       field, regs->cp0_epc, field, regs->regs[31]);
	}
...
EXPORT_SYMBOL(kes_mem_print_handle);
