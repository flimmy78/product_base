				      ...
				      u8 slot, u8 pin)
{
	...
	if(autelan_product_info.board_type == AUTELAN_BOARD_AX71_CRSMU){
	    ...
	    switch (dev_num)
		{
			case 1: if (1 == pin) irq_num = OCTEON_IRQ_PCI_INT2; else { if (2 == pin) irq_num = OCTEON_IRQ_PCI_INT3; }; break;
			case 2: irq_num = OCTEON_IRQ_PCI_INT0; break;
			case 3: irq_num = OCTEON_IRQ_PCI_INT1; break;
			default: break; 
	...
	printk("devnum = %d, irqnum = %d, slot = %u, pin = %u,OCTEON_IRQ_PCI_INT0 = %d \n",dev_num,irq_num,slot,pin,OCTEON_IRQ_PCI_INT0);
	return irq_num;
}
...
static int __init octeon_pci_setup(void)
{
	...
	if (octeon_dma_bar_type == OCTEON_DMA_BAR_TYPE_BIG) {
		...
		/* Because we reduced swiotlb memory from 64M to 1MB, so we can remap
			1M~128MB+1MB use bar1. to modified with dma-octeon.c line 283. 
			sush@autelan.com 11/Feb/2012
		*/
		octeon_bar1_pci_phys = 0x1000000;
