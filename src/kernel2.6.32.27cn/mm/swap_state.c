...
void show_swap_cache_info(void)
{
	...
	if (kes_mem_print_handle && print_current_time_handle) {
		...
		kes_mem_print_handle("%lu pages in swap cache\n", total_swapcache_pages);
		kes_mem_print_handle("Swap cache stats: add %lu, delete %lu, find %lu/%lu\n",
							swap_cache_info.add_total, swap_cache_info.del_total,
							swap_cache_info.find_success, swap_cache_info.find_total);
		kes_mem_print_handle("Free swap  = %ldkB\n", nr_swap_pages << (PAGE_SHIFT - 10));
		kes_mem_print_handle("Total swap = %lukB\n", total_swap_pages << (PAGE_SHIFT - 10));
	}
}
