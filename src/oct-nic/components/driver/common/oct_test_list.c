...
struct test_list *
initialize_test_list(int  count, int  extra_bytes)
{
	int                i, size;
	struct test_list  *tl;
	struct test_node  *tn;
...
int
cleanup_test_list(struct test_list  *tl, int  count)
{
	int               empty = 0, i=0;
	...
	if(count > tl->avail_count) {
		PRINT_ERR("%s test list (%p) called with count: %d available: %d\n",
	...
	while(tn != NULL) {
		...
		tl->node_count--;
		tl->avail_count--;
		if((i != count) && ((tl->node_count == 0) || (tl->avail_count == 0))) {
			PRINT_ERR(" %s Error! Node Count: %d Available Count: %d after %d operations\n", __FUNCTION__, tl->node_count, tl->avail_count, i);
			oct_test_list_unlock(&tl->lock);
			return -EINVAL;
		}
		tn = tnext;
...
void
print_test_list(struct test_list  *tl)
{
	int                i=0;
	struct test_node  *tn;
...
struct test_node *
__get_next_node(struct test_list *tl)
{
	...
	if(tl->tail != NULL) {
		...
		if(tn->node_status != NODE_IS_FREE) {
			PRINT_DBG(" %s Error: Node in test list (%p) has status %d\n",
...
int
__put_free_node(struct test_list  *tl, struct test_node *tn)
{
	if(tn->node_status == NODE_IS_FREE) {
...
/* $Id: oct_test_list.c 53786 2010-10-08 22:09:32Z panicker $ */
