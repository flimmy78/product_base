...
cn56xx_alloc_peeriq_memory(void)
{
	...
	for(i = 0; i < MAX_PEERS; i++) {
		...
		size = sizeof(octeon_instr_32B_t) * PEER_CMDQ_SIZE;
...
{
    cvm_drv_register_op_handler(PCIE_MAP_OP,  cn56xx_process_pci_map);
}
...
cn56xx_process_pci_map(cvmx_wqe_t  *wqe)
{
	...
	cvm_drv_print_data(data, (wqe->len - CVM_RAW_FRONT_SIZE));
...
cn56xx_create_ep_gather_list(cn56xx_ep_packet_t  *pkt)
{
	...
	sg = cvmx_fpa_alloc(CVMX_FPA_WQE_POOL);
	if(sg == NULL)
		return g;
...
cn56xx_send_ep_packet(cn56xx_ep_packet_t  *pkt)
{
	...
	memset(&iqcmd, 0, sizeof(octeon_instr_32B_t));
	...
	memcpy(&piq->my_cmdq[piq->my_wr_idx], &iqcmd, sizeof(octeon_instr_32B_t));
...
cn56xx_process_ep_test_packet(cvmx_wqe_t   *wqe)
{
	...
	printf("%s Received test pkt (pkt len: %d bufs: %d)\n",
	       __FUNCTION__, wqe->len, wqe->word2.s.bufs);
	printf("EP Test Pkt: Param: %x\n", front->irh.s.param);

	cvm_drv_print_data(CVM_DRV_GET_PTR(wqe->packet_ptr.s.addr), wqe->len);
#endif
	cvm_free_host_instr(wqe);
...
/* $Id: cn56xx_ep_comm.c 53786 2010-10-08 22:09:32Z panicker $  */
