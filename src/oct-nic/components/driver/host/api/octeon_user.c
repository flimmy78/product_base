...
int
octeon_send_request(int oct_id, octeon_soft_request_t  *soft_req)
{
	...
	if(SOFT_REQ_IS_RAW(soft_req))  {
		...
		if(resp_order == OCTEON_RESP_ORDERED) {
			printf("octeon_send_request: Error! ORDERED (%d) mode not supported \n", resp_order);
			return EINVAL;
		}
	}  else {
		/* Non-RAW mode data (Packet mode) should not expect a RESPONSE */
		if(resp_order != OCTEON_RESP_NORESPONSE) {
			printf("octeon_send_request: Error! Unsupported Response Order (%d) in non-RAW mode\n", resp_order);
			return EINVAL;
		}
	}
	if((resp_order == OCTEON_RESP_NORESPONSE) && (resp_mode == OCTEON_RESP_BLOCKING)) {
		printf("octeon_send_request: Error! Blocking Mode not supported for NoResponse Packet\n");
		return EINVAL;
	}
...
int
octeon_query_request(int oct_id, octeon_query_request_t  *query)
{
...
int
octeon_get_stats(int oct_id, oct_stats_t  *stats)
{
	stats->oct_id = oct_id;
                                 ...
                                 void *data, int cmd)
{
   ...
   switch(cmd) {
      case IOCTL_OCTEON_WRITE32: rw_buf.val = *((uint32_t *)data); break;
      case IOCTL_OCTEON_WRITE16: rw_buf.val = *((uint16_t *)data); break;
      case IOCTL_OCTEON_WRITE8:  rw_buf.val = *((uint8_t *)data); break;
      default: rw_buf.val = 0; break;  // Command could be a read */
   }
   ...
   switch(cmd) {
      case IOCTL_OCTEON_READ32: *((uint32_t *)data) = (uint32_t)rw_buf.val; break;
      case IOCTL_OCTEON_READ16: *((uint16_t *)data) = (uint16_t)rw_buf.val; break;
      case IOCTL_OCTEON_READ8:  *((uint8_t *)data)  = (uint8_t)rw_buf.val; break;
      default: break;
   }
...
{
	return __octeon_rw_mapped_core_memory(oct_id, address, data, IOCTL_OCTEON_READ32);
}

int octeon_read16(int oct_id, unsigned long address, uint16_t *data)
{
	return __octeon_rw_mapped_core_memory(oct_id, address, data, IOCTL_OCTEON_READ16);
}

int octeon_read8(int oct_id, unsigned long address, uint8_t *data)
{
	return __octeon_rw_mapped_core_memory(oct_id, address, data, IOCTL_OCTEON_READ8);
}

int octeon_write32(int oct_id, unsigned long address, uint32_t data)
{
	return __octeon_rw_mapped_core_memory(oct_id, address, &data, IOCTL_OCTEON_WRITE32);
}

int octeon_write16(int oct_id, unsigned long address, uint16_t data)
{
	return __octeon_rw_mapped_core_memory(oct_id, address, &data, IOCTL_OCTEON_WRITE16);
}

int octeon_write8(int oct_id, unsigned long address, uint8_t data)
{
	return __octeon_rw_mapped_core_memory(oct_id, address, &data, IOCTL_OCTEON_WRITE8);
}
...
int open (const char *filename, int mode) {
  ...
  if ((handle = CreateFile (filename, FILE_READ_DATA | FILE_WRITE_DATA, FILE_SHARE_READ | FILE_SHARE_WRITE,
...
int ioctl (int fd, int request, void *ptr) {
  ...
  if (!DeviceIoControl ((HANDLE) fd, request, (LPVOID) inout, size, (LPVOID) inout, size, &ret, NULL)) {
    errno = -1;
    return -1;
  }
...
#endif
/* $Id: octeon_user.c 53786 2010-10-08 22:09:32Z panicker $ */
