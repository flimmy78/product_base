...
int                     signal_to_catch=SIGINT, non_stop=1;
...
void
print_count_freq(struct test_stats  *s)
{
	int  i;
...
void
print_test_stats(struct test *t)
{
...
void
add_thread_stats(struct test *t)
{
	int i, j;
	...
	for(i = 0; i < TEST_THREAD_COUNT; i++) {
		...
		if(t->total.maxdatareceived < t->perthread[i].maxdatareceived)
			t->total.maxdatareceived = t->perthread[i].maxdatareceived;
	}
}
...
*/
void
wait_for_thread_completion(struct test *t, int timeval)
{
	...
	do {
		...
		for(i = 0, t_exists = 0; i < TEST_THREAD_COUNT; i++)
...
void
wait_for_unordered_requests(struct test_stats *s)
{
	int i, r, max_retries=10;

	__print("\n(pid: %d) pending requests: %d\n", getpid(), s->reqs_pending);
		...
		for(i = 0; i < MAX_NB_REQUESTS; i++) {
			...
			if(r != OCTEON_REQUEST_PENDING) {
				if(r == OCTEON_REQUEST_DONE) {
					verify_output(s, s->nbreqs[i].sr, s->nbreqs[i].verify_size,
					              s->nbreqs[i].outsize);
					if(s->nbreqs[i].outsize > s->maxdatareceived)
						s->maxdatareceived = s->nbreqs[i].outsize;
				}
	...
	if(s->reqs_pending) {
		printf("Quitting! There are still %d pending requests for thread %d\n",
...
void  print_usage()
{
	printf("Usage: oct_req <oct_id> [-b] [-u] [-s] [-h] [-y] [-d dma_mode] [count]\n");
	...
	printf("    oct_id - Octeon device id (0 for 1st device, 1 for 2nd..)\n");
	printf(" Optional arguments\n");
	printf("   -d <dma_mode>: where dma mode can be \"direct\", \"gather\", \"scatter\" or \"scatter_gather\" \n");
...
void
printstring(int type, int value)
{
	switch(type) {
	case 0:
		switch(value) {
		case OCTEON_RESP_NORESPONSE: printf(" NORESPONSE (%d)\n", value); break;
		case OCTEON_RESP_UNORDERED: printf(" UNORDERED (%d)\n", value); break;
		default: printf(" UNKNOWN "); break;
		}
		break;
	case 1: 
		switch(value) {
		case OCTEON_RESP_BLOCKING: printf(" BLOCKING (%d)\n", value); break;
		case OCTEON_RESP_NON_BLOCKING: printf(" NON-BLOCKING (%d)\n", value); break;
		default: printf(" UNKNOWN "); break;
		}
		break;
	case 2:
		switch(value) {
		case OCTEON_DMA_DIRECT: printf(" DIRECT (%d)\n", value); break;
		case OCTEON_DMA_GATHER: printf(" GATHER (%d)\n", value); break;
		case OCTEON_DMA_SCATTER: printf(" SCATTER (%d)\n", value); break;
		case OCTEON_DMA_SCATTER_GATHER: printf(" SCATTER/GATHER (%d)\n", value); break;
		}
		break;
	default:
		break;
...
void 
print_data(uint8_t *data, uint32_t size)
{
	int i;
...
print_test_setup(OCTEON_DMA_MODE        dma_mode,
                 ...
                 OCTEON_RESPONSE_MODE   resp_mode)
{
	printf("  response order = "); printstring(0, resp_order);
	printf("  response mode  = "); printstring(1, resp_mode);
	printf("  dma mode       = "); printstring(2, dma_mode);
...
int
send_request(int oct_id, struct test_stats *s, octeon_soft_request_t *sr)
{
	...
	if(!retval) {
		...
		s->request_count++; s->incntfreq[sr->inbuf.cnt]++;
		if(sr->outbuf.cnt)
	...
	if(ok_to_print&& (req_info->req_mask.resp_order == OCTEON_RESP_UNORDERED)) {
 ...
 */
int
compare_bytes(uint8_t   *in, uint8_t  *out, int bytes, int check_sign)
{
	int i, j;
...
{
#ifdef  RANDOMIZE
	*incnt  = (random() % (MAX_INBUFS+1));
	*incnt  = (*incnt)?*incnt:1;
	if(outcnt) {
		*outcnt = (random() % (MAX_OUTBUFS+1));
		*outcnt = (*outcnt)?*outcnt:1;
	}
#else
	*incnt  = MAX_INBUFS;
	if(outcnt)
...
oct_request_thread(int  q_no, int count, int  tidx)
{
	...
	do {
		...
		if(TEST_THREAD_COUNT == 1) {
			printf("--Sending request[%lu] tag: 0x%x--%s",
			       s->request_count, tag, (ok_to_print)?"\n":"\r");
		}
		...
		if( (t->resp_order == OCTEON_RESP_UNORDERED) &&
			...
			switch(nbreqs[nbidx].status) {
				...
				if(r != OCTEON_REQUEST_PENDING) {
					...
					if(r == OCTEON_REQUEST_DONE) {
						if(nbreqs[nbidx].outsize > s->maxdatareceived)
				...
				case REQ_NONE:
				req_status = unordered_nonblocking_request(q_no, tag,s,
				                                   &nbreqs[nbidx], t->dma_mode);
				if(req_status == 0) {
					if(count) count--;
					tag++;
					s->reqs_pending++;
			...
			if(nbreqs[nbidx].status == REQ_PEND)
				nbidx = ((nbidx + 1) == MAX_NB_REQUESTS? 0 : (nbidx + 1));
...
int main(int argc, char **argv)
{
	...
	const char       *cvs_tag = "$Name: PCI_BASE_RELEASE_2_2_0_build_82 ";
	...
	while(arg_idx < argc) {
		...
		if(arg2[0] == '-') {
			...
			while(i < strlen(arg2)) {
				switch(arg2[i]) {
				case 'b': resp_mode = OCTEON_RESP_BLOCKING; break;
				case 'u': resp_order = OCTEON_RESP_UNORDERED; break;
				case 'd': dma_mode = get_dma_mode(argv[arg_idx++]);
				case 's': ok_to_print = 0; break;
				case 'y': prompt_user = 0; break;
	...
	if((resp_order != OCTEON_RESP_NORESPONSE) && (OUTBUF_SIZE  < 16)) {
		printf("Invalid output size for Response Order (%d)\n",
		       resp_order);
		exit(-EINVAL);
	}
...
/* $Id: oct_req.c 53786 2010-10-08 22:09:32Z panicker $ */
