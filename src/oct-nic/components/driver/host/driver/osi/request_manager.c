...
static inline int
IQ_INSTR_MODE_64B(octeon_device_t *oct, int iq_no)
{
	octeon_instr_queue_t *iq = (octeon_instr_queue_t *)oct->instr_queue[iq_no];
	return (iq->iqcmd_64B);
}
...
int
octeon_init_nr_free_list(octeon_instr_queue_t  *iq, int count)
{
	int    size;
	...
	if(iq->nr_free.q == NULL) {
		cavium_error("OCTEON: NoResponse Free list allocation failed\n");
...
int 
octeon_init_instr_queue(octeon_device_t       *oct, int  iq_no)
{
	...
	if(oct->chip_id <= OCTEON_CN58XX)
		conf = &(CHIP_FIELD(oct, cn3xxx, conf)->iq[iq_no]);

	if(oct->chip_id == OCTEON_CN56XX)
		conf = &(CHIP_FIELD(oct, cn56xx, conf)->iq[iq_no]);

	if(oct->chip_id == OCTEON_CN63XX)
		conf = &(CHIP_FIELD(oct, cn63xx, conf)->iq[iq_no]);

	if(oct->chip_id == OCTEON_CN66XX)
		conf = &(CHIP_FIELD(oct, cn66xx, conf)->iq[iq_no]);

	if(oct->chip_id == OCTEON_CN68XX)
		conf = &(CHIP_FIELD(oct, cn68xx, conf)->iq[iq_no]);
	...
	if(octeon_init_nr_free_list(iq, iq->max_count)) {
		octeon_pci_free_consistent(oct->pci_dev, q_size, iq->base_addr, iq->base_addr_dma);
	...
	poll_ops.ticks  = 1;
	strcpy(poll_ops.name, "Doorbell Timeout");
	poll_ops.rsvd   = 0xff;
...
int
octeon_delete_instr_queue(octeon_device_t     *oct, int  iq_no)
{
	...
	if(oct->chip_id <= OCTEON_CN58XX)
		desc_size  = (CHIP_FIELD(oct, cn3xxx, conf))->iq[iq_no].instr_type;

	if(oct->chip_id == OCTEON_CN56XX)
		desc_size  = (CHIP_FIELD(oct, cn56xx, conf))->iq[iq_no].instr_type;

	if(oct->chip_id == OCTEON_CN63XX)
		desc_size  = (CHIP_FIELD(oct, cn63xx, conf))->iq[iq_no].instr_type;
	
	if(oct->chip_id == OCTEON_CN66XX)
		desc_size  = (CHIP_FIELD(oct, cn66xx, conf))->iq[iq_no].instr_type;


	if(oct->chip_id == OCTEON_CN68XX)
		desc_size  = (CHIP_FIELD(oct, cn68xx, conf))->iq[iq_no].instr_type;
...
int
wait_for_instr_fetch(octeon_device_t  *oct)
{
...
static inline void
ring_doorbell(octeon_instr_queue_t  *iq)
{
...
static inline void
__copy_cmd_into_iq(octeon_instr_queue_t  *iq, uint8_t  *cmd)
{
...
static inline int 
__post_command(octeon_device_t       *octeon_dev,
               octeon_instr_queue_t  *iq,
               uint32_t               force_db,
               uint8_t               *cmd)
{
...
#ifdef CAVIUM_DEBUG
   cavium_atomic_check_and_inc(&iq->instr_pending, iq->max_count, __CVM_FILE__, __CVM_LINE__);
#else
...
__post_command2(octeon_device_t       *octeon_dev,
                octeon_instr_queue_t  *iq,
                uint32_t               force_db,
                uint8_t               *cmd)
{
...
post_command32B(octeon_device_t       *oct,
                octeon_instr_queue_t  *iq,
                uint32_t               force_db,
                octeon_instr_32B_t    *cmd32)
{
#ifdef CAVIUM_DEBUG
	if(iq->iqcmd_64B) {
		cavium_error("Cannot post 32 byte command in IQ[%d] (64-Byte IQ)\n",
		             iq->iq_no);
		return -1;
	}
#endif
...
post_command64B(octeon_device_t       *oct,
                octeon_instr_queue_t  *iq,
                uint32_t               force_db,
                octeon_instr_64B_t    *cmd64)
{
#ifdef CAVIUM_DEBUG
	if(!iq->iqcmd_64B) {
		cavium_error("Cannot post 64 byte command in IQ[%d] (32-Byte IQ)\n",
		             iq->iq_no);
		return -1;
	}
#endif
...
static int
__process_iq_noresponse_list(octeon_device_t  *oct, octeon_instr_queue_t  *iq)
{
	...
	while(old != iq->octeon_read_index)  {
		...
		cavium_print(PRINT_DEBUG,"Adding instr @ %p type %d to freelist @ idx %d\n", 
		      iq->nrlist[old].buf, iq->nrlist[old].buftype, put_idx);
...
check_db_timeout(void  *octptr, unsigned long  iq_no)
{
  ...
  if(!cavium_check_timeout(cavium_jiffies, (iq->last_db_time + iq->db_timeout))) {
                            ...
                            octeon_soft_request_t     *sr)
{
  ...
  print_soft_instr(oct, si);
   ...
   if(si->rptr) {
      if(!si->irh.scatter) {
         cmd->rptr = (uint64_t)octeon_pci_map_single(oct->pci_dev,
                  (void *)si->rptr, si->irh.rlenssz, CAVIUM_PCI_DMA_FROMDEVICE);
      } else {
         cmd->rptr = (uint64_t)octeon_pci_map_single(oct->pci_dev,
                      (void *)si->rptr,
                      ((ROUNDUP4(si->irh.rlenssz) >> 2) * OCT_SG_ENTRY_SIZE),
                      CAVIUM_PCI_DMA_TODEVICE);/* scatterlist PCI is TODEVICE */
      }
   }
   ...
   if(iq->iqcmd_64B && !(si->alloc_flags & OCTEON_DIRECT_GATHER) &&
      ...
      cavium_memcpy(cmd->exhdr, si->exhdr, (SOFT_INSTR_EXHDR_COUNT(si) * 8) );
   }
   ...
   if(!iq->iqcmd_64B) {
      ...
      q_index = post_command64B(oct, iq,
                (si->alloc_flags & OCTEON_SOFT_INSTR_DB_NOW), cmd);
   }
   ...
   if(q_index >= 0) {
      ...
      cavium_print(PRINT_DEBUG, "OCTEON[%d]: No space in IQ[%d] to post instruction\n",
                   oct->octeon_id, iq_no);
...
/** Allocate a sglist components based on the number of scatter or gather 
  ...
  */
static inline  octeon_sg_entry_t  *
octeon_alloc_sglist(octeon_device_t            *octeon_dev,
                    ...
                    uint32_t                    flags)
{
	...
	if(sg_list) {
		cavium_print(PRINT_DEBUG,"alloc_sg_list:sg_count: %d sg_list at 0x%p\n", sg_count, sg_list );
		...
		if((unsigned long)sg_list & 0x07)
			sg_list = (uint8_t *)(((unsigned long)sg_list + 7) & ~(7UL));
...
static void *
octeon_create_direct_gather(octeon_device_t            *oct,
                            octeon_buffer_t            *buf,
                            octeon_soft_instruction_t  *si)
{
	...
	for(i = 1; i < buf->cnt; i++) {
		...
		sg->ptr[i-1] = octeon_pci_map_single(oct->pci_dev, OCT_SOFT_REQ_BUFPTR(buf, i), buf->size[i], CAVIUM_PCI_DMA_TODEVICE);
	}
   ...
   int                 tmpcnt = 0;
    ...
    if(cnt == 0) {
       cavium_error("OCTEON: %s buffer count cannot be 0\n",
                    (flags == OCTEON_DPTR_GATHER)?"Gather":"Scatter");
       return NULL;
    }
    ...
    if(flags == OCTEON_DPTR_GATHER) { /* If its a gather list. */
       ...
       cavium_print(PRINT_DEBUG,"iq_no: %d instr_mode: %d exhdr_size: %d\n",
                    iq_no, IQ_INSTR_MODE_32B(oct, iq_no)?32:64, exhdr_size);
       ...
       for(i = 0; i < cnt; i++) {
           if(buf->size[i] == 0) {
              cavium_error("OCTEON: Gather buffer size cannot be 0\n");
              return NULL;
           }
       ...
       if(IQ_INSTR_MODE_32B(oct, iq_no) && exhdr_size ) {
          cavium_print(PRINT_DEBUG,"create gather: exhdr to be added\n");
       ...
       if(total_size > OCT_MAX_GATHER_DATA_SIZE) {
          cavium_error("OCTEON: Total Size (%d) exceeds max (%d) for Gather\n",
       ...
       if(cnt > MAX_SCATTER_PTRS) {
          cavium_error("Found %d scatter ptrs; Max supported is %d\n",
       ...
       for(i = 0, total_size=0; i < cnt; i++) {
           if(buf->size[i] == 0) {
              cavium_error("OCTEON: Scatter buffer size cannot be 0\n");
              return NULL;
           }
           /* CN38XX Errata PCI-500: Component length should <= 65528 */
           if(buf->size[i] > OCT_MAX_COMP_BUF_LEN) {
              cavium_error("Scatter buffer size (%d) exceeds max (%d)\n",
       ...
       */
       if(total_size > OCT_MAX_SCATTER_DMA_SIZE) {
          cavium_error("Buffer Size (%d) exceeds max (%d) for Scatter\n",
...
octeon_process_request_inbuf(octeon_device_t            *octeon_dev,
                             octeon_soft_instruction_t  *soft_instr,
                             octeon_soft_request_t      *soft_req)
{
   ...
   */
    if( (soft_req->inbuf.cnt > 1)
       ...
       if(total_size == 0) {
           cavium_error("OCTEON: Input Size cannot be 0 when bufcnt is %d\n",
                        soft_req->inbuf.cnt);
           return NULL;
       }
       ...
       if(total_size > OCT_MAX_DIRECT_INPUT_DATA_SIZE) {
           cavium_error("Input Size (%d) exceeds max (%d) for DIRECT mode\n",
                       ...
                       octeon_soft_request_t      *soft_req)
{
   ...
   */
   if((soft_req->inbuf.cnt > MAX_BUFCNT)||(soft_req->outbuf.cnt > MAX_BUFCNT)) {
   ...
   */
   if(resp_order == OCTEON_RESP_NORESPONSE) {
       if(dma_mode != OCTEON_DMA_DIRECT && dma_mode != OCTEON_DMA_GATHER) {
          cavium_error("OCTEON: NoResponse request don't support %s dma mode\n",
                        OCTEON_DMA_MODE_STRING(dma_mode));
       }
       if(soft_req->outbuf.cnt != 0) {
          cavium_error("OCTEON: Outbuf count should be 0 for resp_order: %s\n",
                       OCTEON_RESP_ORDER_STRING(resp_order));
       }
   } else {
       if( (soft_req->outbuf.cnt == 0) || (SOFT_REQ_OUTBUF(soft_req, 0) == NULL) ) {
          cavium_error("OCTEON: Invalid buffer count for resp_order: %s\n",
                       OCTEON_RESP_ORDER_STRING(resp_order));
          cavium_error("OCTEON: outbuf.cnt = %d outbuf.ptr0 = %p\n",
                       soft_req->outbuf.cnt, SOFT_REQ_OUTBUF(soft_req, 0));
          return OCTEON_REQUEST_INVALID_BUFCNT;
       }
   }
   ...
   if( (dma_mode == OCTEON_DMA_DIRECT || dma_mode == OCTEON_DMA_GATHER) &&
       ...
       cavium_error("OCTEON: Invalid outbuf.cnt (%d) for dma_mode %s\n",
   ...
   if(dma_mode == OCTEON_DMA_DIRECT) {
        ...
        */
        if( (resp_order != OCTEON_RESP_NORESPONSE) 
           ...
           cavium_error("Output Size (%d) exceeds max (%d) for DIRECT mode\n",
                     soft_req->outbuf.size[0], OCT_MAX_DIRECT_DMA_SIZE);
        ...
        if( (soft_req->inbuf.cnt) ||
            ...
            soft_instr->dptr =  octeon_process_request_inbuf(oct,
                                                      soft_instr, soft_req);
            if(!soft_instr->dptr) {
                cavium_error("OCTEON: (%s) Input buffer creation failed\n",
                             __CVM_FUNCTION__);
          ...
          || (dma_mode == OCTEON_DMA_SCATTER_GATHER)) {
           ...
           if( (soft_instr->ih.raw)
               ...
               if(soft_req->outbuf.size[0] == 0) {
                  cavium_error("OCTEON: Output Size must be non-zero for %s response\n", OCTEON_RESP_ORDER_STRING(resp_order)) ;
                  return OCTEON_REQUEST_INVALID_BUFSIZE;
               }

               /* GATHER DMA uses a single Output buffer. */ 
               if(soft_req->outbuf.size[0] > OCT_MAX_DIRECT_DMA_SIZE) {
                  cavium_error("OCTEON: Output Size (%d) exceeds max (%d) for GATHER mode\n", soft_req->outbuf.size[0], OCT_MAX_DIRECT_DMA_SIZE);
                  return OCTEON_REQUEST_INVALID_BUFSIZE;
               }
               soft_instr->rptr         = SOFT_REQ_OUTBUF(soft_req, 0);
               soft_instr->irh.rlenssz  = soft_req->outbuf.size[0];
           ...
           if(!soft_instr->dptr)  {
               cavium_error("OCTEON: Gather list for request failed\n");
               return OCTEON_REQUEST_NO_MEMORY;
           }
...
__do_request_processing(octeon_device_t        *oct,
                        octeon_soft_request_t  *sr)
{
	octeon_soft_instruction_t   *si;
	...
	if(si == NULL)  {
		cavium_error("OCTEON[%d]: Allocation failed in processing request\n",
		             oct->octeon_id);
#ifndef OCTEON_USE_OLD_REQ_STATUS
	...
	for(i = 0; i < hdr_cnt; i++)  {
		...
		switch(hdr_info)  {
			...
			case OCTEON_EXHDR_ENDIAN_SWAP:
				...
				break;
			default: 
				cavium_error("OCTEON: Invalid extra header info in request\n");
				break;
		}
	}
	...
	if((retval.s.status = octeon_create_data_buf(oct, si, sr)))  {
		...
		cavium_error("OCTEON: Data buffer creation failed in request\n");
	...
	if(retval.s.error) {
		cavium_error("OCTEON: Instruction send failed to process request status: %x\n", retval.s.status);
...
octeon_instr_status_t
octeon_process_request(uint32_t                octeon_id,
                       octeon_soft_request_t  *soft_req)
{
	octeon_instr_status_t   retval;
	octeon_device_t        *oct;
	...
	if(!SOFT_REQ_IS_RAW(soft_req))  {
		...
		if(resp_order != OCTEON_RESP_NORESPONSE) {
			cavium_error("%s: Error! Packet Mode request must be NORESPONSE\n",
			             __CVM_FUNCTION__);
#ifndef OCTEON_USE_OLD_REQ_STATUS
                          ...
                          int               rsize)
{
	...
	if( (cavium_atomic_read(&oct->status) != OCT_DEV_RUNNING)  &&
	    ...
	    /*wangchong 2013-01-19 for review CID 13502*/
	    //delete_soft_instr_buffers(oct, instr);
		return 1;
	}
...
{
	if(!iq->iqcmd_64B)
		return post_command32B(oct, iq, force_db, cmd);
	else
		return post_command64B(oct, iq, force_db, cmd);
}
...
octeon_flush_iq(octeon_device_t   *oct, octeon_instr_queue_t  *iq)
{
	...
	if(cavium_atomic_read(&iq->instr_pending) >= 8) {//(iq->max_count - 2)) {
		flush_instr_queue(oct, iq);
	}
}
...
void
octeon_perf_flush_iq(octeon_device_t   *oct, octeon_instr_queue_t  *iq)
{
	uint32_t    inst_processed=0;
...
/* $Id: request_manager.c 66446 2011-10-25 02:31:59Z mchalla $ */
