...
void cn66xx_dump_dpi_regs(octeon_device_t   *oct);




static  int
cn66xx_soft_reset(octeon_device_t   *oct)
{
	octeon_write_csr64(oct, CN63XX_WIN_WR_MASK_REG, 0xFF);

	cavium_print_msg("OCTEON[%d]: BIST enabled for CN66XX soft reset\n",
	                 oct->octeon_id);
...
void
cn66xx_enable_error_reporting(octeon_device_t   *oct)
{
	uint32_t   regval;
...
static void
cn66xx_setup_pcie_mps(octeon_device_t   *oct, enum octeon_pcie_mps  mps)
 {
	uint32_t            regval;
...
static void
cn66xx_setup_pcie_mrrs(octeon_device_t   *oct, enum octeon_pcie_mrrs mrrs)
{
	uint32_t            regval;
	volatile uint64_t   r64;
...
uint32_t
cn66xx_core_clock(octeon_device_t  *oct)
{
...
static uint32_t
cn66xx_coprocessor_clock(octeon_device_t  *oct)
{
...
uint32_t
cn66xx_get_oq_ticks(octeon_device_t  *oct, uint32_t  time_intr_in_us)
{
	/* This gives the SLI clock per microsec */
	uint32_t  oqticks_per_us = cn66xx_coprocessor_clock(oct);
...
void
cn66xx_setup_global_input_regs(octeon_device_t  *oct)
{
...
void
cn66xx_setup_global_output_regs(octeon_device_t  *oct)
{
	...
	octeon_write_csr(oct, CN63XX_SLI_OQ_INT_LEVEL_PKTS, cn66xx->conf->oq_intr_pkt);
	time_threshold = cn66xx_get_oq_ticks(oct, cn66xx->conf->oq_intr_time);
	
	octeon_write_csr(oct, CN63XX_SLI_OQ_INT_LEVEL_TIME, time_threshold);

#ifdef CVMCS_DMA_IC
	/* This gives the SLI clock per microsec */
	time_threshold = cn66xx_coprocessor_clock(oct);
	/* Now Calculate the DMA timer interrupt threshold */
	time_threshold *= cn66xx->conf->dma_intr_time;
	octeon_write_csr64(oct, CN63XX_DMA_INT_LEVEL_START, 
			  	((uint64_t)time_threshold << 32) | 
			   	cn66xx->conf->dma_intr_pkt);
...
static int
cn66xx_setup_device_regs(octeon_device_t  *oct)
{
	cn66xx_setup_pcie_mps(oct, PCIE_MPS_DEFAULT);
	cn66xx_setup_pcie_mrrs(oct, PCIE_MRRS_512B);

	cn66xx_enable_error_reporting(oct);

	cn66xx_setup_global_input_regs(oct);
...
static void
cn66xx_setup_iq_regs(octeon_device_t   *oct, int   iq_no)
{
...
static void
cn66xx_setup_oq_regs(octeon_device_t   *oct, int   oq_no)
{
	uint32_t        intr;
...
static void
cn66xx_enable_io_queues(octeon_device_t  *oct)
{
...
static void
cn66xx_disable_io_queues(octeon_device_t  *oct)
{
...
static void
cn66xx_handle_pcie_error_intr(octeon_device_t  *oct, uint64_t intr64)
{
...
int
cn66xx_droq_intr_handler(octeon_device_t  *oct)
{
	octeon_droq_t   *droq;
...
cvm_intr_return_t
cn66xx_interrupt_handler(void  *dev)
{
	...
	intr64 = OCTEON_READ64(cn66xx->intr_sum_reg64);
	...
	if(intr64 & CN63XX_INTR_ERR)
		cn66xx_handle_pcie_error_intr(oct, intr64);

	if(intr64 & CN63XX_INTR_PKT_DATA)
		cn66xx_droq_intr_handler(oct);
	...
	OCTEON_WRITE64(cn66xx->intr_sum_reg64, intr64);
...
static void
cn66xx_reinit_regs(octeon_device_t  *oct)
{
	int               i;
...
static void
cn66xx_bar1_idx_setup(octeon_device_t  *oct, uint64_t core_addr, int idx, int valid)
{
...
static void
cn66xx_bar1_idx_write(octeon_device_t  *oct, int idx, uint32_t  mask)
{
...
static uint32_t
cn66xx_bar1_idx_read(octeon_device_t  *oct, int idx)
{
...
static uint32_t
cn66xx_update_read_index(octeon_instr_queue_t  *iq)
{
...
static void
cn66xx_enable_interrupt(void  *chip)
{
	octeon_cn66xx_t  *cn66xx = (octeon_cn66xx_t  *)chip;

	/* Enable Interrupt */
#ifdef CVMCS_DMA_IC
	OCTEON_WRITE64(cn66xx->intr_enb_reg64, cn66xx->intr_mask64);
#else
	OCTEON_WRITE64(cn66xx->intr_enb_reg64,
		(cn66xx->intr_mask64 | CN63XX_INTR_DMA0_FORCE));
#endif
}
...
static void
cn66xx_disable_interrupt(void  *chip)
{
	octeon_cn66xx_t  *cn66xx = (octeon_cn66xx_t  *)chip;

	/* Disable Interrupts */
	OCTEON_WRITE64(cn66xx->intr_enb_reg64, 0);
}
...
static void
cn66xx_get_pcie_qlmport(octeon_device_t  *oct)
{
	if(oct->rev_id >= 8) {
	...
	cavium_print_msg("OCTEON: CN66xx uses PCIE Port %d\n", oct->pcie_port);
}
...
static void
cn66xx_setup_reg_address(octeon_device_t   *oct)
{
	octeon_cn66xx_t *cn66xx = (octeon_cn66xx_t *)oct->chip;
	...
	cn66xx_get_pcie_qlmport(oct);

	cn66xx->intr_sum_reg64 = bar0_pciaddr + CN63XX_SLI_INT_SUM64;
	cn66xx->intr_mask64    = CN63XX_INTR_MASK;
	cn66xx->intr_enb_reg64 = bar0_pciaddr + CN63XX_SLI_INT_ENB64(oct->pcie_port);
}



int
setup_cn66xx_octeon_device(octeon_device_t  *oct)
{
	octeon_cn66xx_t *cn66xx = (octeon_cn66xx_t *)oct->chip;
	...
	cn66xx->conf  = (cn66xx_config_t  *)oct_get_config_info(oct);
	if(cn66xx->conf == NULL) {
	...
	oct->fn_list.setup_iq_regs      = cn66xx_setup_iq_regs;
	oct->fn_list.setup_oq_regs      = cn66xx_setup_oq_regs;

	oct->fn_list.interrupt_handler  = cn66xx_interrupt_handler;
	oct->fn_list.soft_reset         = cn66xx_soft_reset;
	oct->fn_list.setup_device_regs  = cn66xx_setup_device_regs;
	oct->fn_list.reinit_regs        = cn66xx_reinit_regs;
	oct->fn_list.update_iq_read_idx = cn66xx_update_read_index;

	oct->fn_list.bar1_idx_setup     = cn66xx_bar1_idx_setup;
	oct->fn_list.bar1_idx_write     = cn66xx_bar1_idx_write;
	oct->fn_list.bar1_idx_read      = cn66xx_bar1_idx_read;

	oct->fn_list.enable_interrupt   = cn66xx_enable_interrupt;
	oct->fn_list.disable_interrupt  = cn66xx_disable_interrupt;

	oct->fn_list.enable_io_queues   = cn66xx_enable_io_queues;
	oct->fn_list.disable_io_queues  = cn66xx_disable_io_queues;

	cn66xx_setup_reg_address(oct);
...
int
validate_cn66xx_config_info(cn66xx_config_t  *conf66xx)
{
	int i, total_instrs = 0;

	if(conf66xx->c.num_iqs > CN63XX_MAX_INPUT_QUEUES) {
		cavium_error("%s: Num IQ (%d) exceeds Max (%d)\n", __CVM_FUNCTION__,
		             conf66xx->c.num_iqs, CN63XX_MAX_INPUT_QUEUES);
		return 1;
	}


	if(conf66xx->c.num_oqs > CN63XX_MAX_OUTPUT_QUEUES) {
		cavium_error("%s: Num IQ (%d) exceeds Max (%d)\n", __CVM_FUNCTION__,
		             conf66xx->c.num_iqs, CN63XX_MAX_INPUT_QUEUES);
		return 1;
	}
	...
	for(i = 0; i < conf66xx->c.num_iqs; i++) {
		if( (conf66xx->iq[i].instr_type != 32) &&
		    (conf66xx->iq[i].instr_type != 64) ) {
			cavium_error("%s: Invalid instr type (%d) for IQ[%d]\n",
			              __CVM_FUNCTION__, conf66xx->iq[i].instr_type, i);
			return 1;
		}
		if( !(conf66xx->iq[i].num_descs) || !(conf66xx->iq[i].db_min)
		    || !(conf66xx->iq[i].db_timeout) ) {
			cavium_error("%s: Invalid parameter for IQ[%d]\n",
			             __CVM_FUNCTION__, i);
			return 1;
		}
		total_instrs += conf66xx->iq[i].num_descs;
	}

	if(conf66xx->c.pending_list_size < total_instrs) {
		cavium_error("%s Pending list size (%d) should be >= total instructions queue size (%d)\n",
		           __CVM_FUNCTION__, conf66xx->c.pending_list_size, total_instrs);
		return 1;
	}

	for(i = 0; i < conf66xx->c.num_oqs; i++) {
		if(!(conf66xx->oq[i].info_ptr)  || !(conf66xx->oq[i].pkts_per_intr) ||
		   !(conf66xx->oq[i].num_descs) || !(conf66xx->oq[i].refill_threshold) ||
		   !(conf66xx->oq[i].buf_size)   ){
...
void
cn66xx_dump_dpi_regs(octeon_device_t   *oct)
{
	int i;
...
void
cn66xx_dump_iq_regs(octeon_device_t  *oct)
{
	uint32_t  regval;
...
/* $Id: cn66xx_device.c 47454 2010-02-18 22:33:46Z panicker $ */
