...
int 
octeon_setup_response_list(octeon_device_t *oct)
{
	int                 i, ret;
...
void
octeon_delete_response_list(octeon_device_t *oct)
{
                            ...
                            octeon_query_request_t  *query)
{
	...
	if(octeon_dev == NULL) {
		cavium_error("OCTEON: Request query for invalid octeon device: %d\n",
		              query->octeon_id);
		return 1;
	}
                         ...
                         uint32_t            type)
{
	...
	dir = (type == OCTEON_DMA_GATHER)?CAVIUM_PCI_DMA_TODEVICE:CAVIUM_PCI_DMA_FROMDEVICE;

	for(i = 0; i < sg_cnt; i++) {
		if(sg[i/4].ptr[i%4]) {
			octeon_pci_unmap_single(oct->pci_dev, sg[i/4].ptr[i%4],
			                    CAVIUM_GET_SG_SIZE(&sg[i/4], (i%4)), dir);
        }
	}
	...
	octeon_pci_unmap_single(oct->pci_dev, hw_addr,
	             ((ROUNDUP4(sg_cnt) >> 2) * OCT_SG_ENTRY_SIZE), dir);
}
...
void
delete_soft_instr_buffers(octeon_device_t             *octeon_dev,
                   ...
                   octeon_req_status_t         status)
{
	...
	if(soft_instr->dptr) {
	 	if(!soft_instr->ih.gather) {
			octeon_pci_unmap_single(octeon_dev->pci_dev,soft_instr->command.dptr,
                               soft_instr->ih.dlengsz, CAVIUM_PCI_DMA_TODEVICE);
		}  else  {
			octeon_pci_unmap_sg_list(octeon_dev,
			                       (octeon_sg_entry_t *)soft_instr->dptr,
	...
	if(soft_instr->rptr) {
		if(!soft_instr->irh.scatter) {
			octeon_pci_unmap_single(octeon_dev->pci_dev, soft_instr->command.rptr,
			               soft_instr->irh.rlenssz, CAVIUM_PCI_DMA_FROMDEVICE);
		} else {
			octeon_pci_unmap_sg_list(octeon_dev,
			                       (octeon_sg_entry_t *)soft_instr->rptr,
	...
	if(SOFT_INSTR_CALLBACK(soft_instr)) {
		SOFT_INSTR_CALLBACK(soft_instr)(status,
                                        SOFT_INSTR_CALLBACK_ARG(soft_instr));
	}
...
static __inline octeon_pending_entry_t * 
get_list_head(octeon_device_t         *octeon_dev,
...
static __inline void
release_from_response_list(octeon_device_t          *octeon_dev,
...
void 
push_response_list(octeon_response_list_t *response_list, octeon_pending_entry_t *entry)
{
...
int 
process_ordered_list(octeon_device_t *octeon_dev)
{
	octeon_response_list_t      *response_list;
	octeon_pending_entry_t      *pending_entry;
	octeon_soft_instruction_t   *soft_instr;
...
#ifdef CVMCS_DMA_IC
	/* Process as many requests noted in interrupt handler */
	resp_to_process = cavium_atomic_read(&octeon_dev->dma_cnt_to_process);
	cavium_atomic_sub(resp_to_process, &octeon_dev->dma_cnt_to_process);
#endif
	...
	do {
		...
		if(status != OCTEON_REQUEST_PENDING)  {
			...
			cavium_print(PRINT_REGS, "process_ordered_list: releasing entry with status: %x\n", status);
                       ...
                       octeon_query_request_t  *query)
{
	...
	if( query->request_id > octeon_dev->pend_list_size) {
		cavium_error("OCTEON: process_unordered_list: No request with id: %d\n", query->request_id);
	...
	if (pending_entry->request_id != query->request_id) {
		cavium_error("OCTEON: process_unordered_list: pending entry req_id (%d) does not match query req_id: %d\n", pending_entry->request_id, query->request_id);
		query->status = OCTEON_REQUEST_TIMEOUT;
		return 1;
	}
	...
	if( SOFT_INSTR_RESP_ORDER(soft_instr) != OCTEON_RESP_UNORDERED) {
		cavium_error("OCTEON: process_unordered_list: request not unordered\n");
		query->status = OCTEON_REQUEST_TIMEOUT;
		return 1;
	}
	...
	if(query->status == OCTEON_REQUEST_INTERRUPTED) {
		status = query->status; goto unordered_finish;
	}
...
unordered_finish:
	if(status != OCTEON_REQUEST_PENDING)  {
		...
		cavium_print(PRINT_DEBUG, "process_unordered_list: release entry with status: %x\n", status);
...
void
process_noresponse_list(octeon_device_t       *oct,
                        octeon_instr_queue_t  *iq)
{
	...
	while(get_idx != iq->nr_free.put_idx) {
		cavium_print(PRINT_DEBUG, "Removing buf @ %p type %d from idx %d\n", 
	      iq->nr_free.q[get_idx].buf, iq->nr_free.q[get_idx].buftype, get_idx);
	...
	cavium_list_for_each_safe(tmp, tmp2, &instr_list) {
		octeon_soft_instruction_t *instr = (octeon_soft_instruction_t *)tmp;
		cavium_list_del(tmp);
                      ...
                      uint32_t                    iq_no)
{
	...
	GET_RESPONSE_LIST(SOFT_INSTR_RESP_ORDER(pe->instr), SOFT_INSTR_RESP_MODE(pe->instr), resp_list);

	cavium_spin_lock_softirqsave(&(oct->response_list[resp_list].lock));
	release_from_response_list(oct, pe);
	cavium_spin_unlock_softirqrestore(&(oct->response_list[resp_list].lock));
...
*/
void
check_unordered_blocking_list(octeon_device_t  *oct)
{
	...
	cavium_list_for_each_safe(curr, tmp, &response_list->head) {
		...
		if(status != OCTEON_REQUEST_PENDING)  {
			...
			cavium_print(PRINT_DEBUG, "process_unordered_list: release entry with status: %x\n", status);
			if(SOFT_INSTR_CALLBACK(soft_instr)) {
				SOFT_INSTR_CALLBACK(soft_instr)(status, SOFT_INSTR_CALLBACK_ARG(soft_instr));
...
oct_poll_check_unordered_list(void *octptr, unsigned long  arg)
{
	...
	cavium_list_for_each_safe(curr, tmp, &response_list->head) {
		...
		if(pe->status == OCTEON_PENDING_ENTRY_TIMEOUT) {
			cavium_print(PRINT_DEBUG,"Removing pending entry req_id:%d, opcode: %x\n",
		...
		if(pe->status == OCTEON_PENDING_ENTRY_USED) {
			if(cavium_check_timeout(cavium_jiffies, si->timeout)) {
				cavium_print(PRINT_DEBUG,"Found pending entry @ %p, opcode: %x\n",
				             pe, si->irh.opcode);
...
/* $Id: response_manager.c 56301 2011-01-15 00:41:25Z skoteshwar $ */
