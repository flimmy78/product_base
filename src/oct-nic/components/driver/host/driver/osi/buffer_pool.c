                        ...
                        octeon_bufpool_config_t  *gconfig)
{
        ...
        if (malloc_buffer(octeon_dev, large, gconfig->large_buffer_max,
            ...
            cavium_error("OCTEON init_buffer_pool: failed to alloc large\n");
            goto failed;
        }
        ...
        if (malloc_buffer(octeon_dev, medium, gconfig->medium_buffer_max,
            ...
            cavium_error("OCTEON init_buffer_pool: failed to alloc medium\n");
            goto failed;
        }
        ...
        if (malloc_buffer(octeon_dev, small, gconfig->small_buffer_max,
            ...
            cavium_error("OCTEON init_buffer_pool: failed to alloc small\n");
            goto failed;
        }
        ...
        if (malloc_buffer(octeon_dev, ex_tiny, gconfig->ex_tiny_buffer_max,
            ...
            cavium_error("OCTEON init_buffer_pool: failed to alloc ex_tiny\n");
            goto failed;
        }
 ...
 ***************************************************/
void 
octeon_delete_buffer_pool(octeon_device_t * octeon_dev)
{
              ...
              int               size)
{
        ...
        for (i = 0; i < buf->chunks; i++) {
            buf->address[i] = (uint8_t *)cavium_malloc_dma(buf->real_size, __CAVIUM_MEM_ATOMIC);
            if (!buf->address[i]) {
                 cavium_error("OCTEON malloc_buffer: failed for chunk=%d\n",i);
                 goto failed;
            }
 ...
 ***************************************************/
void 
free_buffer(octeon_device_t * octeon_dev, OCTEON_BUFPOOL p)
{
        int i;
        cavium_buffer_t *buf = &octeon_dev->buf[p];
...
get_free_fragment(octeon_device_t *octeon_dev)
{
...
static void
put_fragment(octeon_device_t *octeon_dev, cavium_frag_buf_t frag)
{
        octeon_dev->fragment_free_list_index --;
        octeon_dev->fragment_free_list[octeon_dev->fragment_free_list_index] = (uint16_t) frag.index;
}
...
get_buffer(void *pdev, cavium_buffer_t * b)
{
        ...
        cavium_list_for_each(tmp, &b->frags_list) {
           cavium_frag_buf_t *entry = (cavium_frag_buf_t *)(&(((cavium_frag_buf_t *)tmp)->list));
...
{
        buffer_tag* t = (buffer_tag*)((unsigned long)buf - sizeof(buffer_tag));
        ...
        switch (t->pool) {
           case ex_tiny:
                cavium_error("OCTEON: buffer pools fragmenting from tiny buffer\n");
 ...
 ******************************************************/
static uint32_t
grow_buffers(octeon_device_t *octeon_dev, OCTEON_BUFPOOL p)
{
        uint8_t *buf = NULL;
        unsigned long flags;
        ...
        switch (p) {
           case ex_tiny:
                buf = get_buffer_from_init_pool(octeon_dev, &octeon_dev->buf[tiny]);
                if (buf) {
                    other_pools[tiny]++;
           ...
           case tiny:
                buf = get_buffer_from_init_pool(octeon_dev,&octeon_dev->buf[small]);
                if (buf) {
                    other_pools[small]++;
           ...
           case small:
                buf =get_buffer_from_init_pool(octeon_dev,&octeon_dev->buf[medium]);
                if (buf) {
                    other_pools[medium]++;
           ...
           case medium:
                buf =get_buffer_from_init_pool(octeon_dev,&octeon_dev->buf[large]);
                if (buf) {
                    other_pools[large]++;
           ...
           case large:
                buf = get_buffer_from_init_pool(octeon_dev, &octeon_dev->buf[huge]);
                if (buf) {
                    other_pools[huge]++;
...
get_buf:
        if (size <= EX_TINY_BUFFER_CHUNK_SIZE) {
            ...
            buf = cavium_malloc_dma(size + sizeof(buffer_tag),__CAVIUM_MEM_ATOMIC);
            if (buf) {
               ...
               cavium_error("OCTEON: Out of memory get_buffer_from_pool %d\n",size);
               return NULL;    
            }
        }
...
#endif
               {
                   cavium_error("OCTEON: Alloc failed (get_buffer_from_pool)\n");
                   return buf;
               }
            }
            goto get_buf;
...
check_in_fragmented_pool(octeon_device_t *octeon_dev, uint8_t *b)
{
        ...
        cavium_list_for_each_safe(tmp, tmp1, &allocated_list_head) {
          ...
          frag  = (cavium_frag_buf_t *)(&(((cavium_frag_buf_t *)tmp)->alloc_list));
          t  = (buffer_tag*)((unsigned long)(frag->big_buf) - sizeof(buffer_tag));
          for (i = 0; i < frag->frags_count; i++) {
              if (frag->address[i] == b) {
                 ...
                 if (frag->free_list_index == 0) {
                     ...
                     CAVIUM_INIT_LIST_HEAD(&frag->alloc_list);
                 ...
                 if (big_buf_to_be_freed) {
                      put_buffer(&octeon_dev->buf[t->pool], t->index);
...
void 
put_buffer_in_pool(void *dev, uint8_t *b)
{
        ...
        default:
                /* bad, very bad! this should never happen. */
                cavium_error("OCTEON: Unsupported pool %lu in put_buffer_in_pool\n",
                              t->pool);
                return;
        }
        alloc_buffer_stats[t->pool] --;
...
void
oct_get_buffer_pool_stats(oct_stats_t   *stats)
{
	int i;
...
/* $Id: buffer_pool.c 42960 2009-05-28 19:45:43Z panicker $ */
