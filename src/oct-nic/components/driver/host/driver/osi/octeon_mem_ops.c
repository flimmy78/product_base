...
uint32_t
octeon_map_device_range(octeon_device_t   *oct,
                        uint64_t    start,
                        uint64_t    end)
{
   ...
   if(start > end) {
      cavium_error("OCTEON: Invalid address range, start: 0x%llx end: 0x%llx\n", CVM_CAST64(start), CVM_CAST64(end));
     return INVALID_MAP;
   }
   ...
   if((oct->map_count + maps_reqrd) > MAX_OCTEON_MAPS)  {
       cavium_error("OCTEON: current map count: %d, new maps_reqrd: %d will exceed max allowed: %d\n", oct->map_count, maps_reqrd, MAX_OCTEON_MAPS);
       return  INVALID_MAP;
   }
   ...
   for (i = old_map_count; i < (old_map_count + maps_reqrd); i++) {
      ...
      oct->range_table[i].mapped_addr =
         (uint8_t *)oct->mmio[1].hw_addr + ((old_map_count + i) << 22);
...
uint32_t
octeon_get_mapped_idx(octeon_device_t  *oct,
                      uint64_t          core_addr)
{
	uint32_t  i;
	...
	for(i = 0; i < oct->map_count; i++) {
		if( (oct->range_table[i].core_addr <= core_addr)
		    &&  (core_addr < (oct->range_table[i].core_addr + (1 << 22)) ) )
			return i;
	}
...
void *
octeon_get_mapped_addr(octeon_device_t  *oct, uint64_t  core_addr)
{
	uint32_t            range_idx;
...
uint64_t
octeon_read_device_mem64(octeon_device_t   *oct, uint64_t  core_addr)
{
...
static inline void 
octeon_toggle_bar1_swapmode(octeon_device_t   *oct, int idx)
{
...
static void
octeon_pci_fastwrite(octeon_device_t   *oct, uint8_t *mapped_addr, uint8_t  *hostbuf, int len)
{
	...
	octeon_toggle_bar1_swapmode(oct, 1);

	while(len >= 8) {
		OCTEON_WRITE64(mapped_addr, *((uint64_t *)hostbuf));
		mapped_addr += 8; hostbuf += 8; len -= 8;
	}
...
static void
octeon_pci_fastread(octeon_device_t   *oct, uint8_t *mapped_addr, uint8_t  *hostbuf, int len)
{
	while(((unsigned long)mapped_addr) & 7) {
...
#define MEMOPS_IDX   MAX_BAR1_MAP_INDEX
	...
	cavium_print(PRINT_DEBUG, "%s Transfer %llu bytes %s core addr %llx %s host buffer @ %p\n",
	       __CVM_FUNCTION__, CVM_CAST64(len), (op)?"from":"to", CVM_CAST64(addr), (op)?"to":"from", hostbuf);
	...
	do {
		...
		if( ((addr + len - 1) & ~(0x3fffff)) != (addr & ~(0x3fffff)) ) {
			copy_len = ((addr & ~(0x3fffff)) + (MEMOPS_IDX << 22)) - addr;
		} else {
			copy_len = len;
		}

		cavium_print(PRINT_DEBUG, "hostbuf: %p corebuf: %llx copy_len: %d\n",
...
/* $Id: octeon_mem_ops.c 53785 2010-10-08 22:08:08Z panicker $ */
