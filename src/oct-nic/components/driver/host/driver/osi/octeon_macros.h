 ...
 */
static inline void *
cavium_alloc_aligned_memory(uint32_t        size,
                            uint32_t       *alloc_size,
...
#define OCTEON_MAX_ALLOC_RETRIES     1
   do  {
      ptr = (void *)cavium_alloc_pages(__CAVIUM_MEM_GENERAL,
                                       cavium_get_order(size));
      if((unsigned long)ptr & 0x07) {
         cavium_free_pages((unsigned long)ptr , cavium_get_order(size));
         ptr = NULL;
...
/** Free the buffers from a recv_pkt structure. The buffer type determines the
  ...
  * @param - pointer to a recv_pkt structure.
  */
static inline void
cavium_free_recv_pkt_buffers(octeon_recv_pkt_t  *recv_pkt)
{
	int i;
...
octeon_get_dispatch(octeon_device_t  *octeon_dev,
                    uint16_t          opcode)
{
   int                    idx;
   ...
   if(octeon_dev->dispatch.dlist[idx].opcode == opcode)  {
      ...
      cavium_list_for_each(dispatch, &(octeon_dev->dispatch.dlist[idx].list))  {
         if(((octeon_dispatch_t *)dispatch)->opcode == opcode)  {
           cavium_print(PRINT_DEBUG, " found dispatch in list\n");
           fn = ((octeon_dispatch_t *)dispatch)->dispatch_fn;
           break;
         }
      }
   }
 ...
 */
static inline void *
octeon_get_dispatch_arg(octeon_device_t  *octeon_dev,
                        uint16_t          opcode)
{
   int             idx;
   cavium_list_t  *dispatch;
...
/* $Id: octeon_macros.h 59212 2011-04-14 18:46:46Z panicker $ */
