...
*/
void
print_octeon_regs(octeon_device_t  *octeon_dev)
{
   int i;
   uint32_t  val;
   ...
   cavium_print(PRINT_REGS,"NPI[%016llx]: 0x%016llx\n", CVM_CAST64(startaddr + 0x10), CVM_CAST64(OCTEON_PCI_WIN_READ(octeon_dev, startaddr + 0x10)));
   for(i = 0x50; i < 0x1FF; i += 8)
       cavium_print(PRINT_REGS,"NPI[%016llx]: 0x%016llx\n", CVM_CAST64(startaddr + i), CVM_CAST64(OCTEON_PCI_WIN_READ(octeon_dev, startaddr + i)));
...
void 
print_queue(octeon_device_t   *oct_dev,
            int                iq_no)
{
   ...
   if(octeon_debug_level >= PRINT_DEBUG) {
       ...
       if(iq->iqcmd_64B) {
          ...
          for (i = 0; i < iq->max_count; i++)
              cavium_print(PRINT_DEBUG,
                "Queue[%d] dptr:0x%llx ih:0x%llx irh:0x%llx rptr:0x%llx \n",
                 i, CVM_CAST64(queue64[i].dptr), CVM_CAST64(*((uint64_t *)&queue64[i].ih)),
                      CVM_CAST64(queue64[i].rptr), CVM_CAST64(*((uint64_t *)&queue64[i].irh)));
       } else {
          ...
          for (i = 0; i < iq->max_count; i++) 
              cavium_print(PRINT_DEBUG,
                "Queue[%d] dptr:0x%llx ih:0x%llx irh:0x%llx  rptr:0x%llx\n",
                 i,CVM_CAST64(queue32[i].dptr), CVM_CAST64(*((uint64_t *)&queue32[i].ih)),
                      CVM_CAST64(queue32[i].rptr), CVM_CAST64(*((uint64_t *)&queue32[i].irh)));
       }
   }
}
...
void
octeon_print_command(OCTEON_IQ_INSTRUCTION_MODE   iq_mode, 
              void                         *arg) 
{
   ...
   if(octeon_debug_level >= PRINT_DEBUG) {
       ...
       cavium_print_msg("Mode: %s %s %s dlengsz: %d fsz: %d grp: %d qos: %d tag: 0x%08x tagtype: %d\n",
          (ih->raw?"RAW":"Packet"),
          (ih->gather?(ih->dlengsz?"InDirect":"Direct"):""),
          (ih->gather?"Gather":"Direct"),
						ih->dlengsz,
						ih->fsz,
						ih->grp,
						ih->qos,
          ...
          case IQ_MODE_64:
               ...
               for(i = 0; i < 4; i++)
                  cavium_print_msg("gh[%d]: 0x%016llx\n",
                               i, CVM_CAST64(command64->exhdr[i]));
               break;
          case IQ_MODE_32:     
               break;
      }
...
void
print_pending_list(octeon_device_t *octeon_dev)
{
   if(octeon_debug_level >= PRINT_DEBUG) {
       uint32_t  i;
       uint32_t  pl_index;
       ...
       cavium_print(PRINT_DEBUG,"\n\n----#  Pending List  #----\n");
       cavium_print(PRINT_DEBUG,"Total used entries: %d\n",
                    octeon_dev->plist->free_index);

       for (i = 0; i < octeon_dev->plist->free_index; i++)  {
            pl_index = octeon_dev->plist->free_list[i];
            cavium_print(PRINT_DEBUG," pending entry at index %d in pending_list\n",
                                  pl_index);
            cavium_print(PRINT_DEBUG," request id: %d\n",
                              octeon_dev->plist->list[pl_index].request_id);
            cavium_print(PRINT_DEBUG," queue_index: %d\n",
                             octeon_dev->plist->list[pl_index].queue_index);
            cavium_print(PRINT_DEBUG," ih in the instr: 0x%llx\n", 
                      CVM_CAST64(*((uint64_t*)&octeon_dev->plist->list[pl_index].instr->ih)));
            cavium_print(PRINT_DEBUG," irh in the instr: 0x%llx\n", 
                    CVM_CAST64(*((uint64_t*)&octeon_dev->plist->list[pl_index].instr->irh)));
       }
   }
}
...
void
print_response_list(octeon_device_t    *octeon_dev,
...
void
print_sg_list(octeon_sg_entry_t  *sg, 
              uint32_t              count)
{
...
void
print_req_info(octeon_request_info_t *req_info)
{
...
void
print_soft_instr(octeon_device_t           *octeon_dev, 
   ...
   if(octeon_debug_level >= PRINT_DEBUG) {
      ...
      octeon_request_info_t *req_info = &soft_instr->req_info;
       ...
       cavium_print(PRINT_DEBUG,"ih: 0x%016llx  irh: 0x%016llx\n",
                        CVM_CAST64(*((volatile uint64_t *)ih)), CVM_CAST64(*((volatile uint64_t *)irh)));
       cavium_print(PRINT_DEBUG," %s %s dlen:%d fsz:%d qos:%d grp:%d %s\n",
              (ih->raw)?"Raw":" ", (ih->gather)?" Gather ":" ",ih->dlengsz,
                 ih->fsz, ih->qos, ih->grp,(ih->rs)?" raw short ": " ");
       cavium_print(PRINT_DEBUG,"opcode:0x%x param:%d dport:%d rlen:%d rid:%d %s\n",
                            irh->opcode, irh->param, irh->dport, irh->rlenssz,
                                     irh->rid, (irh->scatter)?" scatter ":" ");

       for (i = 0; i < 4; i++) 
          cavium_print(PRINT_DEBUG,"exhdr[%d]:0x%016llx\n", i, CVM_CAST64(soft_instr->exhdr[i]));
...
void
octeon_print_stamp(void)
{
	int i;
	for(i = 0; i < oct_stamp_cnt; i++) {
		octeon_swap_8B_data(&oct_stamps[i], 1);
		cavium_print_msg("stamp[%d]: 0x%016llx index: %d\n",i,  CVM_CAST64(oct_stamps[i]), oct_stamp_index[i]);
	}
}


void
octeon_add_stamp(uint64_t *dptr, uint32_t q_index)
{
...
/* $Id: octeon_debug.c 53785 2010-10-08 22:08:08Z panicker $ */
