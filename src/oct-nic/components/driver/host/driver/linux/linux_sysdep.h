...
/*
 * To format and print into /proc page
 */
...
static inline int
cavium_kthread_create(cvm_kthread_t  *t)
{
	t->id = kernel_thread(t->fn, t->fn_arg, CLONE_FS|CLONE_FILES|CLONE_SIGHAND);
	return (t->id < 0);
}

static inline int
cavium_kthread_signalled(void)
{
	return signal_pending(current);
}

static inline void
cavium_kthread_destroy(cvm_kthread_t  *t)
{
	if(t->id >= 0) {
...
static inline void
cavium_kthread_set_cpu_affinity(cvm_kthread_t  *t, int cpu)
{
	cavium_error("%s: Nothing implemented\n", __CVM_FUNCTION__);
}

static inline void
cavium_kthread_run(cvm_kthread_t  *t)
{
	cavium_error("%s: Nothing implemented\n", __CVM_FUNCTION__);
}
...
static inline void
cavium_thread_daemonize(char *name)
{
	daemonize(name);
...
static inline int
cavium_atomic_check_and_sub(int val, cavium_atomic_t *ptr, char *file, int line)
{
	if( (cavium_atomic_read((ptr)) - val) < 0) {
		cavium_error("OCTEON: %s:%d Underflow in atomic value (%d) (attempt to subtract %d)\n", file, line, cavium_atomic_read((ptr)), val);
		return 1;
	}
	atomic_sub((val),(ptr));
	return 0;
...
static inline int
cavium_atomic_check_and_inc(cavium_atomic_t *ptr, int check_val, char *file, int line)
{
	if( (cavium_atomic_read((ptr)) + 1) > check_val) {
		cavium_error("OCTEON: %s:%d Overflow in atomic value (%d) (max: %d)\n",
...
static inline int
cavium_atomic_check_and_add(int val, cavium_atomic_t *ptr, int check_val, char *file, int line)
{
	if( (cavium_atomic_read((ptr)) + val) > check_val) {
		cavium_error("OCTEON: %s:%d Overflow in atomic value (%d) (attempt to add %d, max: %d)\n", file, line, cavium_atomic_read((ptr)), val, check_val);
		return 1;
	}
	atomic_add((val), (ptr));
	return 0;
...
/* Gives up the CPU for a timeout period.
   ...
   timeout period.  */
static inline  void 
cavium_sleep_timeout_cond(cavium_wait_channel  *wait_queue, int  *condition, int timeout)
{
	cavium_wait_entry      we;
...
#if !defined(writeq)
static inline void
OCTEON_WRITE64(void *addr, uint64_t val)
{
...
static inline void *
octeon_pci_alloc_consistent(cavium_pci_device_t *pci_dev, uint32_t  size,
...
static inline void *
get_new_recv_buffer(uint32_t size)
{
#if 1
...
#define SKB_ADJUST       (SKB_ADJUST_MASK + 1)
	...
	if( (unsigned long)skb->data & SKB_ADJUST_MASK) {
		uint32_t  r = SKB_ADJUST - ((unsigned long)skb->data & SKB_ADJUST_MASK);
		skb_reserve(skb, r);
	}
	/*if( (unsigned long)skb->data & SKB_ADJUST_MASK) {
		printk("skb->data @ %p\n", skb->data);
...
cavium_free_recv_pkt_buf(void *buf, uint32_t  buf_type)
{
	if(buf_type == OCT_BUFFER_TYPE_1)  {
		...
		if(buf_type == OCT_BUFFER_TYPE_2)
			...
			cavium_error("OCTEON: Unknown recv buf type. Buffer not freed\n");
	}
}
...
/** Prints contents of a memory location when an error occurs.
  ...
  */
static inline void
cavium_error_print(uint8_t *data, uint32_t  size)
{
	uint32_t  i;
...
static inline void
cavium_error_print_8B_data(uint64_t *data, uint32_t  size)
{
	uint32_t  i, blocks;
...
int
octeon_add_proc_entry(int octeon_id, octeon_proc_entry_t  *entry);

int
octeon_delete_proc_entry(int octeon_id, char *name);
...
/* $Id: linux_sysdep.h 63353 2011-08-10 17:28:11Z panicker $ */
