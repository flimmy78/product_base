...
int
octeon_setup_poll_fn_list(octeon_device_t   *oct)
{
...
void
octeon_delete_poll_fn_list(octeon_device_t   *oct)
{
...
int 
octeon_delete_poll_thread(void)
{
	...
	if(CVM_KTHREAD_EXISTS(&oct_poll_id)) {
		...
		cavium_print(PRINT_DEBUG," free_poll_thread: thread exited normally\n");
	}
...
int
octeon_register_poll_fn(int                 oct_id,
                        octeon_poll_ops_t  *ops)
{
...
int
octeon_unregister_poll_fn(int                oct_id,
                          octeon_poll_fn_t   fn,
                          unsigned long      fn_arg)
{
	octeon_device_t        *oct = get_octeon_device(oct_id);
	...
	for(i = 0; i < MAX_POLL_FN; i++) {
		...
		if( (n->fn == fn)  && (n->fn_arg == fn_arg) )  {
			...
			oct_poll_fn_print_action(oct_id, n, OCT_POLL_FN_UNREGISTERED);
			cavium_memset(n, 0, sizeof(octeon_poll_fn_node_t));
			ret = 0;
			break;
		}
	...
	if(i == MAX_POLL_FN) {
		cavium_error("%s: oct[%d]:  Poll Function (@ %p; arg: %lx) not found\n",
...
static void
oct_process_poll_list(octeon_device_t  *oct)
{
	int                      i;
...
oct_poll_thread(void* arg)
{
	...
	cavium_print_msg("\n-- OCTEON: Poll_thread quitting now\n");
...
/* $Id: octeon_poll.c 53785 2010-10-08 22:08:08Z panicker $ */
