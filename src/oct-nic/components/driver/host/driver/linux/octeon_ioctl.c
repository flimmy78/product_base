...
static inline void
octeon_user_req_copyout_response(octeon_copy_buffer_t  *copy_buf, octeon_req_status_t  status)
{
	int      i, ret;
	...
	if(status == OCTEON_REQUEST_DONE) {
		cavium_print(PRINT_DEBUG, "Copying %d buffers from kernel to user ..", copy_buf->user_bufcnt);
		for( i = 0; i < copy_buf->user_bufcnt; i++)  {
			if((ret = cavium_copy_out(copy_buf->user_ptr[i], mem, copy_buf->user_size[i]))){
				cavium_error("OCTEON: Failed to copy out %d of %d bytes to user address %p\n", ret, copy_buf->user_size[i], copy_buf->user_ptr[i]);
				return;
			}
		...
		cavium_error("copy_user_buffer: No copy done, status: %d (0x%x)\n", status, status);
	}
	return;
}
...
octeon_alloc_user_req_complete(octeon_device_t   *oct_dev)
{
        octeon_user_req_complete_t   *comp;
                              ...
                              uint32_t                gather)
{
	...
	if(!gather) {
		if(total_size > OCT_MAX_DIRECT_INPUT_DATA_SIZE) {
			cavium_error("Input size (%d) exceeds max direct dma size (%d)\n",
			             total_size, OCT_MAX_DIRECT_INPUT_DATA_SIZE);
			return -EINVAL;
		}
	}  else {
		if(total_size > OCT_MAX_GATHER_DATA_SIZE) {
			cavium_error("Input size (%d) exceeds max gather data size (%d)\n",
	...
	if(mem == NULL) {
		cavium_error("OCTEON: Memory allocation failed for direct buffers\n");
		return -ENOMEM;
	}
                                 ...
                                 uint32_t                scatter)
{
	...
	if(cnt > MAX_BUFCNT) {
		cavium_error("OCTEON: Octeon supports <= %d buffers, request had %d\n",
		             MAX_BUFCNT, cnt);
		return -EINVAL;
	}
	...
	if(!scatter) {
		if(total_size > OCT_MAX_DIRECT_OUTPUT_DATA_SIZE) {
			cavium_error("Output size (%d) exceeds max direct dma size (%d)\n",
			             total_size, OCT_MAX_DIRECT_OUTPUT_DATA_SIZE);
			return -EINVAL;
		}
	} else {
		if(total_size > OCT_MAX_SCATTER_DATA_SIZE) {
			cavium_error("Output size (%d) exceeds max scatter dma size (%d)\n",
	...
	while (size < total_size) {
		...
		size += buf_size;
		cavium_print(PRINT_DEBUG, "addr: %p bufsize[%d]: %d total: %d\n", SOFT_REQ_OUTBUF(soft_req, i), i, buf_size, size);
		i++;
	}
                              ...
                              void          *arg)
{
	...
	if( !(cavium_access_ok(VERIFY_WRITE, (void *)arg, _IOC_SIZE(cmd))) ) {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST: User data not ready\n");
		return -EFAULT;
	}

	soft_req = (octeon_soft_request_t *)cavium_alloc_buffer(octeon_dev, OCT_SOFT_REQUEST_SIZE);
	if(soft_req == NULL) {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST: Memory allocation failed (1)\n");
		return -ENOMEM;
	}
	cavium_memset(soft_req, 0, OCT_SOFT_REQUEST_SIZE);

	if(cavium_copy_in(soft_req, (void *)arg, OCT_SOFT_REQUEST_SIZE)) {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST:Copy in failed for soft_req\n");
		retval = -EFAULT;
		goto free_soft_req;
	}

	if(soft_req->inbuf.cnt > MAX_BUFCNT || soft_req->outbuf.cnt > MAX_BUFCNT) {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST: Max buffers allowed is %d\n",
		             MAX_BUFCNT);
		retval = -EINVAL;
		goto free_soft_req;
	}
	...
	if(req_info == NULL) {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST: Memory allocation failed (2)\n");
		retval = -ENOMEM;
		goto free_soft_req;
	}
	cavium_memset(req_info, 0, OCT_REQ_INFO_SIZE);

	if(cavium_copy_in((void *)req_info, (void *)user_req_info, OCT_USER_REQ_INFO_SIZE))  {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST: Copy in failed for req_info\n");
		retval = -EFAULT;
		goto free_req_info;
	}
	...
	if(octeon_dev == NULL) {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST: Octeon device (%d) not found\n", GET_REQ_INFO_OCTEON_ID(req_info));
		retval = -EINVAL;
		goto free_req_info;
	}
	...
	if(GET_REQ_INFO_IQ_NO(SOFT_REQ_INFO(soft_req)) >= octeon_dev->num_iqs) {
	    ...
	    /*wangchong 2013-01-19 for review CID 13500*/
	    //cavium_free_buffer(octeon_dev, OCT_REQ_INFO_SIZE);
		return retval;
	}
	...
	if (resp_mode == OCTEON_RESP_BLOCKING) {
		...
		if(comp == NULL) {
			cavium_error("IOCTL_OCTEON_SEND_REQUEST: Alloc failed for wait channel\n");
			retval = -ENOMEM;
			goto free_req_info;
		}
	}
	...
	if(soft_req->inbuf.cnt) {
		if((dma_mode == OCTEON_DMA_DIRECT) || (dma_mode == OCTEON_DMA_SCATTER))
			retval = octeon_copy_input_dma_buffers(octeon_dev, soft_req, 0);
		else
	...
	if(copy_buf == NULL) {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST: Memory allocation failed (5)\n");
		retval = -ENOMEM;
		goto free_inbuf;
	}
	...
	if(GET_REQ_INFO_RESP_ORDER(req_info) == OCTEON_RESP_NORESPONSE)  {
		...
		if((dma_mode == OCTEON_DMA_DIRECT)||(dma_mode == OCTEON_DMA_GATHER)) {
			retval = octeon_create_output_dma_buffers(octeon_dev, soft_req,
			                                          copy_buf, 0);
		}  else  {
			retval = octeon_create_output_dma_buffers(octeon_dev, soft_req,
			                                          copy_buf, 1);
	...
	if(status.s.error) {
		cavium_error("OCTEON: SEND REQUEST ioctl failed status: %x\n", status.s.status);
		retval = -ENOMEM;
		goto free_outbuf;
	}
	...
	if (resp_mode == OCTEON_RESP_BLOCKING) {
		...
		if(resp_order == OCTEON_RESP_UNORDERED) {
			...
			do {
				...
				retval = octeon_query_request_status(query.octeon_id,&query);
			...
			cavium_error("IOCTL_OCTEON_SEND_REQUEST: ORDERED mode not supported\n");
			retval = -EINVAL;
			goto free_outbuf;
		}
	}
	...
	retval = 0;
	cavium_print(PRINT_REGS, "ioctl: Status of request: 0x%x req_id is %d\n", 
	             SOFT_REQ_INFO(soft_req)->status, SOFT_REQ_INFO(soft_req)->request_id);

	if(cavium_copy_out( user_req_info, SOFT_REQ_INFO(soft_req), OCT_USER_REQ_INFO_SIZE)) {
		cavium_error("IOCTL_OCTEON_SEND_REQUEST: Request Info copyback failed\n");
		retval = -EFAULT;
		goto free_outbuf;
	} 
...
free_soft_req:
    ...
    /*wangchong 2013-01-19 for review CID 13499*/
	//cavium_free_buffer(octeon_dev, OCT_REQ_INFO_SIZE);
	return retval;
}
...
int octeon_ioctl_query_request(unsigned int   cmd,
                               void          *arg)
{
	octeon_query_request_t   query;
	...
	if(octeon_query_request_status(query.octeon_id, &query)) {
		...
		if(cavium_copy_out((void *)arg, &query, OCT_QUERY_REQUEST_SIZE))
			return -EFAULT;
	}
...
int
octeon_ioctl_hot_reset(unsigned int cmd, void *arg)
{
...
int octeon_ioctl_get_mapping_info(unsigned int cmd, void *arg)
{
   int ret=0, mmio_idx;
   octeon_rw_reg_buf_t rw_buf;
   octeon_device_t *oct_dev;
   ...
   if(oct_dev->mmio[mmio_idx].done) {
      rw_buf.addr = (uint64_t) ((unsigned long)oct_dev->mmio[mmio_idx].hw_addr);
...
int octeon_ioctl_read(unsigned int cmd, 
                      void        *arg)
{
   octeon_rw_reg_buf_t rw_buf;
   octeon_device_t *oct_dev;
   ...
   switch(cmd) {
       case IOCTL_OCTEON_READ32:
            rw_buf.val = (uint32_t)OCTEON_READ32((void *)((unsigned long)rw_buf.addr));
            break;
       case IOCTL_OCTEON_READ16:
            rw_buf.val = (uint16_t)OCTEON_READ16((void *)((unsigned long)rw_buf.addr));
            break;
       case IOCTL_OCTEON_READ8:
            rw_buf.val = (uint8_t)OCTEON_READ8((void *)((unsigned long)rw_buf.addr));
            break;
       case IOCTL_OCTEON_WIN_READ:
            rw_buf.val = OCTEON_PCI_WIN_READ(oct_dev,rw_buf.addr);
            break;
       case IOCTL_OCTEON_READ_PCI_CONFIG:
            OCTEON_READ_PCI_CONFIG(oct_dev, (uint32_t)rw_buf.addr, (uint32_t *)&rw_buf.val);
            break;
       default:
   ...
   cavium_print(PRINT_FLOW,"%s read reg addr=0x%llx, val=0x%llx swap: 0x%llx\n",
                __CVM_FILE__, CVM_CAST64(rw_buf.addr), CVM_CAST64(rw_buf.val),
...
int octeon_ioctl_write(unsigned int cmd, 
                       void        *arg)
{
   octeon_rw_reg_buf_t rw_buf;
   octeon_device_t *oct_dev;
   ...
   switch(cmd) {
       case IOCTL_OCTEON_WRITE32:
            OCTEON_WRITE32((void *)((unsigned long)rw_buf.addr), (uint32_t)rw_buf.val);
            break;
       case IOCTL_OCTEON_WRITE16:
            OCTEON_WRITE16((void *)((unsigned long)rw_buf.addr), (uint16_t)rw_buf.val);
            break;
       case IOCTL_OCTEON_WRITE8:
            OCTEON_WRITE8((void *)((unsigned long)rw_buf.addr), (uint8_t)rw_buf.val);
            break;
       case IOCTL_OCTEON_WIN_WRITE:
            OCTEON_PCI_WIN_WRITE(oct_dev,rw_buf.addr,rw_buf.val);
            break;
       case IOCTL_OCTEON_WRITE_PCI_CONFIG:
            OCTEON_WRITE_PCI_CONFIG(oct_dev, (uint32_t)rw_buf.addr, (uint32_t)rw_buf.val);
            break;
       default:
   ...
   cavium_print(PRINT_FLOW,"%s:%d write: addr=0x%llx, val=0x%llx\n",
                        __CVM_FILE__, __CVM_LINE__, CVM_CAST64(rw_buf.addr), CVM_CAST64(rw_buf.val));
...
int octeon_ioctl_get_dev_count(unsigned int cmd, void *arg)
{
...
int octeon_ioctl_read_core_mem(unsigned int   cmd,
                               void          *arg)
{
   ...
   if(!data) {
      cavium_error("%s:%d Data alloc failed in core mem write ioctl\n", 
                        __CVM_FILE__, __CVM_LINE__);
      return -EINVAL;
   }
   octeon_pci_read_core_mem(octeon_dev, core_mem.addr, data, core_mem.size, core_mem.endian);
...
int octeon_ioctl_write_core_mem(unsigned int   cmd,
                                void          *arg)
{
   octeon_core_mem_rw_t   core_mem;
   octeon_device_t       *octeon_dev;
   ...
   octeon_pci_write_core_mem(octeon_dev, core_mem.addr, data, core_mem.size, core_mem.endian);
   cavium_free_virt(data);
...
int
octeon_ioctl_stats(unsigned int   cmd,
                   void          *arg)
{
	octeon_device_t       *oct;
	oct_stats_t           *stats;
	...
	for(i = 0; i < oct->num_iqs; i++)  {
		cavium_memcpy(&stats->iq[i], &oct->instr_queue[i]->stats, OCT_IQ_STATS_SIZE);
	}

	for(i = 0; i < oct->num_oqs; i++)  {
		cavium_memcpy(&stats->droq[i], &oct->droq[i]->stats, OCT_DROQ_STATS_SIZE);
	}


	if(oct->dma_ops.read_statsb)
		oct->dma_ops.read_statsb(oct->octeon_id, stats);
...
long octeon_unlocked_ioctl (struct file *f,
                            unsigned int   cmd,
                            unsigned long  arg)
{
	return (long) octeon_ioctl (NULL, f, cmd, arg);
...
#ifdef CAVIUM_DEBUG
static inline void
octeon_print_copy_buf_data(uint8_t *buf, uint32_t datasize)
{
 	int i;
...
void octeon_copy_user_buffer(octeon_req_status_t   status,
                             void                 *arg)
{
	octeon_copy_buffer_t  *copy_buf;
	octeon_device_t       *octeon_dev;
	...
	cavium_print(PRINT_FLOW,"----#  octeon_copy_user_buffer; status: %x \n", status);
...
/* $Id: octeon_ioctl.c 63621 2011-08-18 19:21:07Z panicker $ */
