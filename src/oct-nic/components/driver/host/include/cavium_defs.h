...
#define    OCT_MAX_DIRECT_INPUT_DATA_SIZE    (OCT_MAX_DIRECT_DATA_SIZE - 24)
...
*/
#define    OCT_MAX_GATHER_DATA_SIZE          (OCT_MAX_GATHER_DMA_SIZE - 24)
...
octeon_swap_8B_data(uint64_t *data, uint32_t blocks)
{
...
/** Swap 4B blocks */
static __inline void
octeon_swap_4B_data(uint32_t *data, uint32_t blocks)
{
#ifdef OCTEON_SWAP_DATA_BYTES
  while(blocks) {
...
/** Swap 2B blocks */
static __inline void
octeon_swap_2B_data(uint16_t *data, uint32_t blocks)
{
#ifdef OCTEON_SWAP_DATA_BYTES
  while(blocks) {
...
({                                                                          \
    char  str[16];                                                          \
	switch(resp_order) {                                                    \
		case OCTEON_RESP_BLOCKING: strcpy(str, "BLOCKING"); break;          \
		case OCTEON_RESP_NON_BLOCKING: strcpy(str, "NON-BLOCKING"); break;  \
...
({                                                                     \
    char  str[16];                                                     \
	switch(resp_order) {                                               \
		case OCTEON_RESP_ORDERED: strcpy(str, "ORDERED"); break;       \
		case OCTEON_RESP_UNORDERED: strcpy(str, "UNORDERED"); break;   \
		case OCTEON_RESP_NORESPONSE: strcpy(str, "NORESPONSE"); break; \
		default: strcpy(str, "UNKNOWN"); break;                        \
	}                                                                  \
	str;                                                               \
})
...
({                                                                            \
    char  str[16];                                                            \
	switch(dma_mode) {                                                        \
		case OCTEON_DMA_DIRECT: strcpy(str, "DIRECT"); break;                 \
		case OCTEON_DMA_SCATTER: strcpy(str, "SCATTER"); break;               \
		case OCTEON_DMA_GATHER: strcpy(str, "GATHER"); break;                 \
		case OCTEON_DMA_SCATTER_GATHER: strcpy(str, "SCATTER_GATHER"); break; \
		default: strcpy(str, "UNKNOWN"); break;                               \
	}                                                                         \
	str;                                                                      \
})
...
#if  !defined(CAVIUM_IN_KERNEL_MODE)
...
GET_SOFT_REQ_EXHDR_INFO(octeon_soft_request_t *soft_req_ptr,
                        uint32_t                hdr_no)
{
   switch(hdr_no)  {
       case 0:   return((OCTEON_EXHDR_FMT)soft_req_ptr->exhdr_info.exhdr1_op);
       case 1:   return((OCTEON_EXHDR_FMT)soft_req_ptr->exhdr_info.exhdr2_op);
       case 2:   return((OCTEON_EXHDR_FMT)soft_req_ptr->exhdr_info.exhdr3_op);
       case 3:   return((OCTEON_EXHDR_FMT)soft_req_ptr->exhdr_info.exhdr4_op);
   }
 ...
 */
static __inline void
SET_REQ_INFO_IQ_NO(octeon_request_info_t  *req_info,
                   uint32_t                   iq_no)
{
 ...
 */
static __inline void
SET_REQ_INFO_OCTEON_ID(octeon_request_info_t  *req_info,
                       uint32_t                  octeon_id  )
{
 ...
 */
static __inline void
SET_REQ_INFO_TIMEOUT(octeon_request_info_t  *req_info,
                     uint32_t                  timeout)
{
...
#if  defined(CVM_SUPPORT_DEPRECATED_API) || defined(CAVIUM_IN_KERNEL_MODE)
...
static __inline void
SET_REQ_INFO_REQUEST_ID(octeon_request_info_t  *req_info,
                        uint32_t                  req_id)
{
 ...
 */
static __inline void
SET_SOFT_REQ_EXHDR_CNT(octeon_soft_request_t   *soft_req_ptr,
                       uint32_t                  count )
{
...
SET_SOFT_REQ_EXHDR_INFO(octeon_soft_request_t   *soft_req_ptr,
                        uint32_t                  hdr_no,
                        OCTEON_EXHDR_FMT        hdr_fmt)
{
   switch(hdr_no)  {
       case 0:   soft_req_ptr->exhdr_info.exhdr1_op = hdr_fmt;
                 break;
       case 1:   soft_req_ptr->exhdr_info.exhdr2_op = hdr_fmt;
                 break;
       case 2:   soft_req_ptr->exhdr_info.exhdr3_op = hdr_fmt;
                 break;
       case 3:   soft_req_ptr->exhdr_info.exhdr4_op = hdr_fmt;
                 break;
   }
}
 ...
 */
static __inline void
SET_SOFT_REQ_OPCODE_PARAM(octeon_soft_request_t   *soft_req_ptr,
                          uint8_t                   param)
{
 ...
 */ 
static __inline void
SET_SOFT_REQ_DESTPORT(octeon_soft_request_t   *soft_req_ptr,
                      uint8_t                   dport)
{
 ...
 */
static __inline void
SET_SOFT_REQ_RAW(octeon_soft_request_t   *soft_req_ptr)
{
 ...
 */
static __inline void
SET_SOFT_REQ_SHORTRAW(octeon_soft_request_t   *soft_req_ptr)
{
   soft_req_ptr->ih.raw = 1;
...
static __inline void
SET_SOFT_REQ_FRONTDATA(octeon_soft_request_t   *soft_req_ptr,
                       uint8_t                   fsz)
{
 ...
 */
static __inline void
SET_SOFT_REQ_GROUP(octeon_soft_request_t   *soft_req_ptr,
                   uint8_t                   group)
{
 ...
 */
static __inline void
SET_SOFT_REQ_QOS(octeon_soft_request_t   *soft_req_ptr,
                 uint8_t                   qos)
{
...
typedef enum {
  ...
  PCI_BAR0_MAPPED = 1, /**< BAR0 has the CSR's */
  PCI_BAR2_MAPPED = 2, /**< BAR2 indirectly maps Octeon core memory */
  PCI_BAR4_MAPPED = 3, /**< BAR4 maps the Octeon core memory directly */
...
typedef struct {
   ...
   uint32_t   bar1_index;  /**< The BAR1 index register to use for this op. */
...
/* $Id: cavium_defs.h 63638 2011-08-19 01:06:48Z panicker $ */
