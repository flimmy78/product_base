...
void
print_as_binary_32(uint32_t  data)
{
  int i = 0;
...
int main(int argc, const char* argv[])
{
   ...
   {
       printf("Illegal oct_id (%u) passed. It should be >= 0 and < %u\n", oct_id, oct_dev_count);
       goto cleanup;
   }
...
#if 0
            case 'w':
               ret = octeon_write16(oct_id, address, (uint16_t)data);
               data = wdata;
               break;

            case 'b':
               ret = octeon_write8(oct_id, address, (uint8_t)data);
               data = bdata;
               break;
#else
			/*wangchong 2013-01-24 for review CID 14318&14319  and no test*/
            case 'w':
               ret = octeon_write16(oct_id, address, (uint16_t)wdata);
               data = wdata;
               break;

            case 'b':
               ret = octeon_write8(oct_id, address, (uint8_t)bdata);
               data = bdata;
               break;
#endif
      ...
      case 'i':
       {
         ...
         ll_data=0;
         if(!octeon_win_read(oct_id, ll_address, &ll_data))
         {
             printf("\nValue is 0x%llx ", CVM_CAST64(ll_data));
      ...
      case 'o':
       {
         ...
         printf("\nWriting Windowed register at addr 0x%llx val=0x%llx\n",
                    CVM_CAST64(ll_address), CVM_CAST64(ll_data));

         if(octeon_win_write(oct_id, ll_address, ll_data))
         {
...
start_core_read:
         ...
         switch(c) {
            case 'l':
               ret = octeon_read_core(oct_id, DATA32, address, &data);
               break;
            case 'w':
               ret = octeon_read_core(oct_id, DATA16, address, &data);
               break;
            case 'b':
               ret = octeon_read_core(oct_id, DATA8, address, &data);
               break;
            default:
...
start_core_write:
         ...
         switch(c) {
            case 'l':
               ret = octeon_write_core(oct_id, DATA32, address, &data);
               break;
            case 'w':
               ret = octeon_write_core(oct_id, DATA16, address, &data);
               break;
            case 'b':
               ret = octeon_write_core(oct_id, DATA8, address, &data);
               break;
            default:
      ...
      case 'n':
         if(oct_dev_count < 2)
              break;
         printf("\n Please enter Octeon Id (currently %d): ", oct_id);
         scanf("%s", s);
         data = strtoul(s,NULL,16);
...
void get_mapping_info()
{
   if(octeon_get_mapping_info(oct_id, PCI_BAR0_MAPPED, &maddr[0], &msize[0]))
   {
      printf("%s:%d get_mapping_info failed for PCI_BAR0_MAPPED\n", __FILE__, __LINE__);
      goto cleanup;
   }

   if(octeon_get_mapping_info(oct_id, PCI_BAR2_MAPPED, &maddr[1], &msize[1]))
   {
      printf("%s:%d get_mapping_info failed for PCI_BAR2_MAPPED\n", __FILE__, __LINE__);
      goto cleanup;
   }

   if(octeon_get_mapping_info(oct_id, PCI_BAR4_MAPPED, &maddr[2], &msize[2]))
   {
      printf("%s:%d get_mapping_info failed for PCI_BAR4_MAPPED\n", __FILE__, __LINE__);
      goto cleanup;
   }
cleanup:
   return;
...
void display_mapping_info()
{
  ...
  for(i=0;i<3;i++)
  {
     printf("%02d: mapped memory address = 0x%lx, mapped_size = %d\n",
                     i,maddr[i], msize[i]);
  }
}
...
/*
 * $Id: oct_dbg.c 53786 2010-10-08 22:09:32Z panicker $
 */
